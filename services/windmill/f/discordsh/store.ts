...
async function buy(rest: string[], discord: Discord) {
	...
	if (!discordId) {
		return errEmbed(
			'Missing Discord identity',
			'Could not read your Discord id.',
		);
	}
	...
	if (!resp.ok) {
		return errEmbed(
			'Purchase failed',
			`Store returned ${resp.status}. You were not charged.`,
		);
	}
	...
	return {
		embed: {
			...
			url: STORE_URL,
			description: `It's yours. Reveal your card at [kbve.com/store](${STORE_URL}) — sign in with Discord and it'll be waiting in your collection.`,
			color: OK,
