...
serve(async (req) => {
  ...
  try {
    ...
    const isUserToken = typeof claims.role === "string" &&
    ...
    if (!isValidSegment(body.owner) || !isValidSegment(body.repo)) {
      return jsonResponse({
        error: "owner and repo are required and must match [A-Za-z0-9._-]",
      }, 400);
    }
    ...
    let maxPages = Math.min(
      Math.max(body.max_pages ?? DEFAULT_MAX_PAGES, 1),
      50,
    );

    const guildId =
      (body.guild_id ?? Deno.env.get("GH_BACKFILL_DEFAULT_GUILD_ID") ?? "")
        .trim();
        ...
        return jsonResponse(
          {
            error: rate.reason === "cooldown"
              ...
              }s between smoke runs`
              : `Hourly smoke-test limit of ${USER_SMOKE_MAX_PER_HOUR} per guild exceeded — retry in ${
                Math.ceil(rate.retryAfterMs / 60_000)
              }m`,
    ...
    if (tokenErr) {
      ...
      return jsonResponse({
        error: "Failed to resolve GitHub token from vault",
      }, 500);
    }
    if (!githubToken || typeof githubToken !== "string") {
      return jsonResponse(
        {
          error:
            `No GitHub PAT found in vault for guild ${guildId} (service=${VAULT_GITHUB_SERVICE}).`,
        },
        404,
      );
    }
