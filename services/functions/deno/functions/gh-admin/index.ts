...
async function handleList(
  ...
  const expectedUrl =
    `${WEBHOOK_PUBLIC_BASE}/functions/v1/gh-webhook/${serverId}`;
...
async function upsertGuildSecret(
  ...
  if (!row || !row.success) {
    return {
      ok: false,
      error: row?.message ?? "service_set_guild_token failed",
    };
  }
  return { ok: true };
}
          ...
          return jsonResponse(
            {
              error: rate.reason === "cooldown"
                ...
                : `Hourly rotation limit exceeded — retry in ${
          ...
          return jsonResponse(
            {
              error: rate.reason === "cooldown"
                ...
                : `Hourly ping limit of ${PING_RATE_MAX} per repo exceeded — retry in ${
                  Math.ceil(rate.retryAfterMs / 60_000)
                }m`,
              retry_after_ms: rate.retryAfterMs,
            },
