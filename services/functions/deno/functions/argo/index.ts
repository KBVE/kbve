  ...
  try {
    ...
    if (!command || typeof command !== "string") {
      return jsonResponse(
        {
          error:
            'command is required ("applications", "app-status", or "health")',
        },
        400,
      );
    }
    ...
    switch (command) {
      ...
      case "app-status": {
        ...
        if (!name || typeof name !== "string") {
          return jsonResponse(
            { error: "name is required for app-status" },
            400,
          );
        }
      ...
      default:
        return jsonResponse(
          {
            error:
              `Unknown command: ${command}. Use "applications", "app-status", or "health"`,
          },
          400,
        );
    }
