...
serve(async (req) => {
  ...
  if (!ALLOWED_EVENTS.has(githubEvent)) {
    return jsonResponse(
      { ok: true, skipped: githubEvent, guild: guildId },
      200,
    );
  }
  ...
  if (allowlist.size > 0 && !allowlist.has(fullName)) {
    return jsonResponse(
      {
        ok: true,
        skipped: `repo not allowlisted: ${fullName}`,
        guild: guildId,
      },
      200,
    );
  ...
  const eventActor = payload.sender?.login ?? payload.comment?.user?.login ??
    null;
