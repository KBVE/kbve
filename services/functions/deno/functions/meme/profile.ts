...
import { safeRpcError, validateSafeUrl } from "../_shared/validators.ts";
  ...
  async update({ body }) {
    ...
    if (
      ...
      return jsonResponse(
        {
          error:
            "At least one of display_name, avatar_url, or bio must be provided",
        },
        400,
      );
    }
    ...
    const supabase = createServiceClient();
    const { error } = await supabase.schema("meme").rpc(
...
export async function handleProfile(
  ...
  if (!handler) {
    return jsonResponse(
      {
        error: `Unknown profile action: ${memeReq.action}. Use: ${
