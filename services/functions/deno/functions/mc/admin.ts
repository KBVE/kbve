  ...
  try {
    ...
    const loginResp = await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(
          () => reject(new Error("RCON login timeout")),
          RCON_TIMEOUT_MS,
        )
      ),
    ]);
...
const handlers: Record<string, Handler> = {
  ...
  async execute({ claims, token, body }) {
    ...
    if (!config) {
      return jsonResponse(
        {
          error: `Unknown server: ${server}. Available: ${
            Object.keys(SERVER_ENV_MAP).join(", ")
          }`,
        },
        400,
      );
  ...
  async give({ claims, token, body }) {
    ...
    if (!item || typeof item !== "string") {
      return jsonResponse(
        { error: "item is required (e.g. diamond_sword)" },
        400,
      );
    }
    ...
    if (!/^[a-z_][a-z0-9_]*(?::[a-z_][a-z0-9_]*)?$/.test(item as string)) {
      return jsonResponse(
        {
          error:
            "item must be a valid MC item ID (e.g. diamond, minecraft:diamond_sword)",
        },
        400,
      );
    }

    const amt = count !== undefined
      ? Math.min(Math.max(Number(count) || 1, 1), 64)
      : 1;
...
export async function handleAdmin(mcReq: McRequest): Promise<Response> {
  ...
  if (!handler) {
    return jsonResponse(
      {
        error: `Unknown admin action: ${mcReq.action}. Use: ${
          ADMIN_ACTIONS.join(", ")
        }`,
      },
      400,
