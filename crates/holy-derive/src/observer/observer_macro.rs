...
pub fn impl_observer_macro(ast: &DeriveInput) -> Result<TokenStream, syn::Error> {
    ...
    let companion_name = syn::Ident::new(&format!("{}Observers", struct_name), struct_name.span());

    let storage_fields = observed.iter().map(|f| {
        let name = f.ident.as_ref().unwrap();
        let observer_field = syn::Ident::new(&format!("{}_observers", name), name.span());
        quote! {
    ...
    let add_methods = observed.iter().map(|f| {
        let name = f.ident.as_ref().unwrap();
        let observer_field = syn::Ident::new(&format!("{}_observers", name), name.span());
        let method_name = syn::Ident::new(&format!("add_{}_observer", name), name.span());
        quote! {
    ...
    let notify_methods = observed.iter().map(|f| {
        let name = f.ident.as_ref().unwrap();
        let observer_field = syn::Ident::new(&format!("{}_observers", name), name.span());
        let method_name = syn::Ident::new(&format!("notify_{}_observers", name), name.span());
        quote! {
