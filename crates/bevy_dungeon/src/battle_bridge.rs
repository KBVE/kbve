...
pub struct EntityMap {
    ...
    pub players: Vec<(PlayerId, Entity)>,
...
impl EntityMap {
    ...
    pub fn player_entity(&self, uid: PlayerId) -> Option<Entity> {
        self.players
            .iter()
            .find(|(u, _)| *u == uid)
            .map(|(_, e)| *e)
    ...
    pub fn player_uid(&self, entity: Entity) -> Option<PlayerId> {
        self.players
            .iter()
            .find(|(_, e)| *e == entity)
            .map(|(u, _)| *u)
...
pub struct CombatWorld {
    ...
    pub inventories: HashMap<PlayerId, Inventory<ProtoItemKind>>,
}
...
impl CombatWorld {
    ...
    pub fn consume_item(&mut self, uid: PlayerId, game_id: &str) -> bool {
    ...
    #[allow(dead_code)]
    pub fn add_item(&mut self, uid: PlayerId, game_id: &str, qty: u32) -> u32 {
        let Some(inv) = self.inventories.get_mut(&uid) else {
            return qty;
        };
        let Some(kind) = pb::game_id_to_proto_item_kind(game_id) else {
    ...
    #[allow(dead_code)]
    pub fn item_count(&self, uid: PlayerId, game_id: &str) -> u32 {
