...
mod tests {
    ...
    // Edition 2024 made these unsafe: mutating the environment is
    // process-global and unsound while another thread reads it. Every test
    // below that touches the environment is #[serial], so nothing else in this
    // binary runs alongside them, and the one test that is not serial does not
    // read the environment at all. Wrapped once here rather than at each call
    // so the argument is stated in a single place.
    ...
    fn static_config_defaults() {
        remove_var("STATIC_DIR");
        remove_var("STATIC_PRECOMPRESSED");
    ...
    fn static_config_custom_dir() {
        set_var("STATIC_DIR", "/tmp/my-static");
        remove_var("STATIC_PRECOMPRESSED");
    ...
    fn static_config_precompressed_false_variants() {
        ...
        for v in ["false", "0", "off", "no"] {
            set_var("STATIC_PRECOMPRESSED", v);
