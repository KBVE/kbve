...
use crate::{spellbook_generate_ulid_bytes, spellbook_pool_conn};
...
pub async fn authorized_character_data_to_json(
    ...
    let _conn = match state.db_pool.get() {
        Ok(conn) => conn,
        Err(e) => {
    ...
    let user_id = match ValidatorBuilder::<String, String>::new()
        .clean_or_fail()
        .ulid()
    ...
    let character_data_result =
        hazardous_blocking_get_characters_by_userid(byte_ulid.clone(), state.db_pool.clone()).await;
...
pub async fn character_creation_handler(
    ...
    let name = match ValidatorBuilder::<String, String>::new()
        .clean_or_fail()
        .username()
    ...
    let description = match ValidatorBuilder::<String, String>::new()
        .clean_or_fail()
        .general_input()
    ...
    match hazardous_blocking_boolean_character_name_slot_open(name.clone(), state.db_pool.clone())
        .await
    {
        Ok(true) => {
    ...
    let check_character_limit =
        hazardous_blocking_check_user_character_limit(byte_ulid.clone(), state.db_pool.clone())
            .await;
