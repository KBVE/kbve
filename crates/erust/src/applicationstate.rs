use crate::img::{ImageError, create_egui_texture_from_image, load_image_from_url};
use base64::Engine;
...
impl AppState {
    ...
    pub fn load(storage: Option<&dyn eframe::Storage>) -> Option<Self> {
        if let Some(storage) = storage
            && let Some(serialized) = storage.get_string(eframe::APP_KEY)
        {
        ...
        match base64::engine::general_purpose::STANDARD.decode(base64_string) {
            Ok(image_data) => match image::load_from_memory(&image_data) {
                ...
                Err(error) => {
                    ...
                    *error_flag.lock().expect("load_error mutex poisoned") =
                        Some(format!("Failed to process image: {:?}", error));
                }
            },
            Err(error) => {
                warn!("Error decoding Base64: {:?}", error);
                *error_flag.lock().expect("load_error mutex poisoned") =
                    Some(format!("Failed to decode Base64: {:?}", error));
            }
        }
