use base64::Engine;
use egui::{ColorImage, Context as EguiContext, TextureFilter, TextureHandle, TextureOptions};
...
where
    F: FnOnce(Result<DynamicImage, ImageError>) + Send + 'static,
{
    let request = Request::get(url);
    ehttp::fetch(request, move |result| match result {
        Ok(response) => on_image_loaded(response, callback),
        Err(error) => {
            callback(Err(ImageError::NetworkError(format!(
                "HTTP request failed: {}",
                error
            ))));
        }
...
pub fn darken_image(image: &DynamicImage, factor: f32) -> RgbaImage {
    ...
    // Process 4 bytes at a time (R, G, B, A) — skip alpha. as_chunks_mut
    // yields [u8; 4] rather than a slice, so the indexing below is what the
    // doc comment above claims: no per-pixel bounds check.
    for chunk in raw.as_chunks_mut::<4>().0 {
...
fn on_image_loaded(response: Response, callback: impl FnOnce(Result<DynamicImage, ImageError>)) {
    if response.status == 200 {
        if response.bytes.is_empty() {
            callback(Err(ImageError::NetworkError("Empty response".to_string())));
        } else {
            let result = image::load_from_memory(&response.bytes).map_err(ImageError::from);
            callback(result);
        }
    } else {
        callback(Err(ImageError::NetworkError(format!(
...
pub fn create_egui_texture_from_image(egui_ctx: &EguiContext, image: RgbaImage) -> TextureHandle {
    ...
    egui_ctx.load_texture("loaded_image", color_image, texture_options)
}
