...
use crate::supabase::{Session, SupabaseConfig, SupabaseError, auth, functions, oauth};
...
impl SupabaseClient {
    ...
    pub fn is_authenticated(&self) -> bool {
        self.get_session().map(|s| !s.is_expired()).unwrap_or(false)
    }
        ...
        F: FnOnce(Result<Session, SupabaseError>) + Send + 'static,
    {
        ...
        *error_handle.lock().expect("last_error mutex poisoned") = None;
        ...
        auth::sign_in_with_password(&self.config, email, password, move |result| {
            match &result {
                Ok(session) => {
                    *session_handle.lock().expect("session mutex poisoned") = Some(session.clone());
                }
                Err(err) => {
        ...
        auth::sign_out(&self.config, &token, move |result| {
            *session_handle.lock().expect("session mutex poisoned") = None;
            if let Err(ref err) = result {
                *error_handle.lock().expect("last_error mutex poisoned") = Some(format!("{}", err));
            }
            loading_flag.store(false, Ordering::Release);
            callback(result);
        });
    ...
    pub fn invoke_function<F>(&self, function_name: &str, body: &serde_json::Value, callback: F)
    where
