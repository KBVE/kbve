...
- The shape of `storage.*` tables changes (e.g. `storage.orc_rep` gains
  per-player breakdown).
- A prototype name is renamed / removed (use `surface.find_entities_filtered`
    - `entity.destroy()` to clean stale entities).
- A new event handler depends on `storage.*` state that didn't exist on older
  saves.
