...
// =============================================================================
// Tile palettes — map a TileRole to render gid variants (mapdb TilePalette).
// Presentation only; gids are empirically sampled from cloud_tileset usage in
// cloud_city.tilemap.json so every gid is a known-good tile in the atlas.
// =============================================================================

export type TilePalette = Record<number, number[]>;

const GROUND_GIDS = [368];

/** Cloud-city biome — stone streets/plaza, grey-stone walls, banded roofs. */
export const CITY_PALETTE: TilePalette = {
	[Role.GROUND]: GROUND_GIDS,
	[Role.PLAZA]: GROUND_GIDS,
	[Role.ROAD]: GROUND_GIDS,
	[Role.GRASS]: GROUND_GIDS,
	[Role.DOOR]: GROUND_GIDS,
	[Role.PROP]: GROUND_GIDS,
	[Role.WALL]: [518, 519, 524, 526],
	[Role.ROOF]: [484, 485, 486],
	[Role.PROP_SOLID]: [1112],
	[Role.WATER]: [108],
	[Role.VOID]: [0],
	[Role.UNSPECIFIED]: [0],
};

/** Dungeon biome — carved stone floor, solid grey-stone walls. */
export const DUNGEON_PALETTE: TilePalette = {
	[Role.GROUND]: GROUND_GIDS,
	[Role.PLAZA]: GROUND_GIDS,
	[Role.ROAD]: GROUND_GIDS,
	[Role.GRASS]: GROUND_GIDS,
...
export function resolveLayer(
	...
	return roles.map((role, i) =>
		variant(
			palette[role] ?? palette[Role.GROUND],
			i % width,
			Math.floor(i / width),
		),
	);
