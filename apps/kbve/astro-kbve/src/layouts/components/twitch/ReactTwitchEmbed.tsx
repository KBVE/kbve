...
import { twitchService, type TwitchTheme, type TwitchEmbedType } from './ServiceTwitch';
...
const ReactTwitchEmbed: React.FC<ReactTwitchEmbedProps> = ({
  ...
  useEffect(() => {
    ...
    const unsubscribe = twitchService.subscribeToThemeChanges((newTheme) => {
  ...
  useEffect(() => {
    const container = containerRef.current?.closest('[data-twitch-embed]') as HTMLElement | null;
    if (!container) return;

    const allLoaded = (type === 'stream' && streamLoaded) ||
          ...
          <div className="flex items-center gap-2">
            ...
            {type === 'both' && (
              <button
                ...
                className="p-2 rounded-lg bg-white/10 hover:bg-white/20 transition-colors"
                aria-label={chatEnabled ? 'Hide chat' : 'Show chat'}
              >
                <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
