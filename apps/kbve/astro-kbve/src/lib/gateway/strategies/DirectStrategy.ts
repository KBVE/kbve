...
export class DirectStrategy implements ISupabaseStrategy {
	...
	async connectWebSocket(wsUrl?: string): Promise<WebSocketStatus> {
		...
		this.ws.onclose = (event) => {
			console.log(
				'[DirectStrategy] WebSocket closed:',
				event.code,
				event.reason,
			);
			this.ws = null;
			...
			if (
				...
				console.log(
					`[DirectStrategy] Reconnecting (${this.wsReconnectAttempts}/${this.WS_MAX_RECONNECT_ATTEMPTS})...`,
				);

				this.wsReconnectTimer = setTimeout(() => {
					this.connectWebSocket(wsUrl).catch((err) => {
						console.error(
							'[DirectStrategy] Reconnection failed:',
							err,
						);
					});
	...
	private startHeartbeat() {
		...
		this.wsHeartbeatTimer = setInterval(() => {
			...
			if (
				...
				now - this.wsLastPongTime > this.WS_HEARTBEAT_TIMEOUT_MS
			) {
