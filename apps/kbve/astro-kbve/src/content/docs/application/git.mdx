...
- Retention: 7-day TTL + soft (80%) / hard (95%) size limits. A whitelist
  ConfigMap (`registry-mirror-ghcr-whitelist`) protects pinned tags under the
  soft limit; the hard limit overrides it to avoid a full disk.
...
2. **Pull cache-first in the job.** Swap the registry host to the cache, pull,
   ...
   ghcr fallback so a cache outage degrades gracefully:
