...
tags:
    - cloud
bento:
    badge: application · cloud · vcs
    title: 'Every line of history,'
    accent: Git.
    lede: >-
        The distributed version control system behind nearly all modern
        software — fast, flexible, and built for collaboration. This guide
        collects the command cheatsheet, conventional commits, hosting
        platforms from GitHub to Forgejo, and the CI/CD automation patterns
        the KBVE monorepo runs on.
    ctas:
    ...
    aside:
        ...
        body: >-
            Every clone is a full repository with complete history — commit
            offline, branch freely, and reconcile with remotes when you are
            ready. Git tracks changes, resolves conflicts, and keeps every
            contribution accounted for.
        points:
            - <strong>Cheatsheet</strong> — the commands you reach for daily.
            - <strong>Hosting</strong> — GitHub, GitLab, Gitea, and Forgejo.
            - <strong>Automation</strong> — fanout CI, auto-merge, cleanup.
    stats:
        - { label: Model, value: Distributed, icon: network }
        - { label: First release, value: '2005', icon: clock }
        - { label: License, value: GPLv2, icon: license }
        - { label: Created by, value: Linus Torvalds, icon: code }
    features:
        - title: Command cheatsheet
          icon: code
          href: '#cheatsheet'
          body: The full quick-reference table — init, branch, rebase, stash, bisect, reflog, and everything between.
        - title: GitHub Actions & fanout CI
          icon: deploy
          href: '#github'
          body: Pages, Actions, the fanout Docker pipeline, package publishing, queue guards, and failure tracking.
        - title: Forgejo OAuth with Supabase
          icon: lock
          href: '#forgejo'
          body: Wire the self-hosted forge into Supabase OIDC for automatic login and account creation.
        - title: Auto-merge & registry cleanup
          icon: gears
          href: '#auto-merge-bot-prs'
          body: Gated bot PR merging for version bumps plus weekly GHCR pruning that never touches releases.
    related:
        - title: Docker
          href: /application/docker/
          copy: The container images that Git-driven CI builds and publishes.
          icon: package
        - title: Kubernetes
          href: /application/kubernetes/
          copy: Where the manifest-update PRs from CI land and deploy.
          icon: deploy
        - title: Linux
          href: /application/linux/
          copy: The command-line foundation every Git workflow lives on.
          icon: server
    theme:
        hero_bg: https://images.unsplash.com/photo-1618401479427-c8ef9465fbe1?q=80&w=2400&auto=format&fit=crop
        accent: '#34d399'
        accent2: '#22d3ee'
        btn_fg: '#06251b'
...
<BentoProse id="cheatsheet" eyebrow="Command reference" heading="Cheatsheet">
...
<BentoProse id="conventional-commits" eyebrow="Commit style" heading="Conventional Commits Cheatsheet">
...
<BentoProse id="github" eyebrow="Hosting & CI/CD" heading="Github">
...
These are brief notes on the `utils-rust-publish-crate.yml` within our monorepo, which is used to publish Rust crates to crates.io.
...
publish_crates:
    ...
    uses: KBVE/kbve/.github/workflows/utils-rust-publish-crate.yml@main
    with:
        package: ${{ matrix.name }}
    secrets:
...
3. **Test** (conditional) — calls `utils-docker-test-app.yml` which builds a `ci-{sha}` tagged image in GHCR and runs the app's e2e test suite against it. Skipped for apps without tests (e.g., `kilobase`).
...
| discordsh | `kbve/discordsh` | ubuntu-latest | discordsh-web-e2e | Yes |
| mc | `kbve/mc` | ubuntu-latest | mc | Yes |
| edge | `kbve/edge` | ubuntu-latest | edge | Yes |
...
#### Docker Image Pull-Through Cache

Large base images (e.g. the ~20 GB `ghcr.io/epicgames/unreal-engine` used by the
Unreal pipeline) are otherwise re-pulled from the internet by every ephemeral
`arc-runner-*` job, ~25 min each. An in-cluster `registry:2` running in **proxy
mode** caches those layers on a Longhorn PVC so the first pull fills it once and
every later pull is LAN speed.

- Manifest: `apps/kube/github/runners/manifests/registry-mirror-ghcr.yaml`
- Service: `registry-mirror-ghcr.arc-runners.svc.cluster.local:5000`
- Upstream: `https://ghcr.io` (auth via the in-cluster `github-arc-token`)
- Proxy revalidation: 24h TTL — UE `dev-*` tags are mutable, so the cache
  re-checks the tag→manifest mapping daily rather than serving a stale image for
  a week after an upstream re-push (blob bytes are immutable, so this is cheap).
- Retention: soft (80%) / hard (95%) size limits. A whitelist ConfigMap
  (`registry-mirror-ghcr-whitelist`) protects pinned tags under the soft limit;
  the hard limit overrides it to avoid a full disk.
- Access: an Ingress NetworkPolicy (`registry-mirror-ghcr-ingress`) restricts
  `:5000` to the `arc-runners` namespace, since the cache serves the private UE
  image unauthenticated over HTTP and has the delete API enabled.

##### Use the cache from another runner job

Routing another CI job's image pulls through the cache is two steps:

1. **Opt the runner in.** Add the cache host to that runner's dind args so Docker
   trusts it (the cache serves over HTTP), then bump that file's
   `restart-trigger` to roll the pods. `arc-runner-ue` already has this:

   ```yaml
   # apps/kube/github/runners/manifests/values-<runner>.yaml (dind command)
   exec dockerd ... --insecure-registry=registry-mirror-ghcr.arc-runners.svc.cluster.local:5000
   ```

2. **Pull cache-first in the job.** Swap the registry host to the cache, pull,
   and retag to the canonical name (so the rest of the job is unchanged), with a
   ghcr fallback so a cache outage degrades gracefully. Log in to ghcr
   **unconditionally** first — a cache hit must still leave a valid ghcr session
   for any later step that touches ghcr:

   ```yaml
   - name: Pull image (cache-first, ghcr fallback)
     env:
       CACHE_HOST: registry-mirror-ghcr.arc-runners.svc.cluster.local:5000
     run: |
...
Only `ghcr.io` images are served by this instance (its single upstream). To keep
a tag from being pruned under the soft limit, add its cache path
(`<org>/<image>:<tag>`) to the `registry-mirror-ghcr-whitelist` ConfigMap.

##### Cache a different registry (Docker Hub, quay.io, …)

Stand up a second proxy instance: copy `registry-mirror-ghcr.yaml`, change
`REGISTRY_PROXY_REMOTEURL` to that registry, give it its own PVC + Service name,
and point image refs at the new service. One `registry:2` proxy serves exactly
one upstream.
...
<BentoProse id="gitlab" eyebrow="DevOps platform" heading="GitLab">
...
<BentoProse id="gitea" eyebrow="Self-hosted Git" heading="Gitea">
...
<BentoProse id="forgejo" eyebrow="Community fork" heading="Forgejo">

Forgejo is a community-driven fork of Gitea focused on self-hosted Git service with enhanced features and governance.
Built on the foundation of Gitea, Forgejo maintains the same lightweight and efficient architecture while ensuring long-term community ownership and development.

### Forgejo OAuth/OIDC Setup with Supabase

This section describes how to configure Forgejo to use Supabase (GoTrue) as an OAuth2/OIDC provider for automatic login with KBVE infrastructure.

#### Overview

Users can log into Forgejo using their KBVE/Supabase credentials automatically through the following flow:

<Steps>
1. User visits `forgejo.kbve.com` (routes through kbve-gate)
2. Gate validates Supabase JWT and checks staff role
3. If authenticated, user accesses Forgejo and is automatically logged in via OAuth
4. User account is auto-created in Forgejo on first login
</Steps>

#### Prerequisites

##### 1. Create OAuth Application in Supabase

Register Forgejo as an OAuth client in Supabase:

**Option A: Via Supabase Dashboard**
- Navigate to Authentication > URL Configuration or Providers
- Add a new OAuth client for Forgejo

**Option B: Via Database (SQL)**
```sql
-- Insert into auth.saml_providers or equivalent table
-- This depends on your Supabase version and configuration
```

**Required Configuration:**
- **Client Name**: `forgejo` or `forgejo-kbve`
- **Redirect URI**: `https://forgejo.kbve.com/user/oauth2/kbve/callback`
- **Scopes**: `openid email profile`
- **PKCE**: Optional but recommended

After creation, you'll receive:
- **Client ID**: Save this value
- **Client Secret**: Save this value securely

##### 2. Create Kubernetes Secret

Create a sealed secret with the OAuth credentials:

```bash
# Create the secret YAML
kubectl create secret generic forgejo-oauth-config \
  --from-literal=client-id='YOUR_CLIENT_ID' \
  --from-literal=client-secret='YOUR_CLIENT_SECRET' \
  --namespace=forgejo \
  --dry-run=client -o yaml > /tmp/forgejo-oauth-config.yaml

# Seal it
kubeseal --format=yaml \
  --cert=<path-to-pub-cert>.pem \
  < /tmp/forgejo-oauth-config.yaml \
  > apps/kube/forgejo/manifest/sealed-oauth-config.yaml

# Clean up
rm /tmp/forgejo-oauth-config.yaml
```

#### Configuration Methods

##### Method 1: CLI Configuration (Recommended for Automation)

Create a Kubernetes Job that runs after Forgejo is deployed:
...
##### Method 2: Web GUI Configuration

<Steps>
1. Log in to Forgejo as admin
2. Navigate to: Site Administration (gear icon) > Authentication Sources
3. Click "Add Authentication Source"
4. Configure the following settings:
   - **Authentication Type**: OAuth2
   - **Authentication Name**: `kbve` (must match redirect URI)
   - **OAuth2 Provider**: OpenID Connect
   - **Client ID**: `<from secret>`
   - **Client Secret**: `<from secret>`
   - **OpenID Connect Auto Discovery URL**: `https://supabase.kbve.com/auth/v1/.well-known/openid-configuration`
   - **Icon URL**: (optional) `https://kbve.com/favicon.ico`
   - **Scopes**: `openid email profile`
   - **Required Claim Name**: (optional) leave empty
   - **Required Claim Value**: (optional) leave empty
   - **Group Claim Name**: (optional) `groups`
   - **Admin Group**: (optional) `staff`
   - **Restricted Group**: (optional) leave empty
...
The Forgejo `application.yaml` includes the following OAuth settings:

```yaml
service:
    DISABLE_REGISTRATION: false  # Allow OAuth registration
    REQUIRE_SIGNIN_VIEW: true    # Still require login to view
    ALLOW_ONLY_EXTERNAL_REGISTRATION: true  # Only via OAuth
    SHOW_REGISTRATION_BUTTON: false  # Hide manual registration
openid:
    ENABLE_OPENID_SIGNIN: true
    ENABLE_OPENID_SIGNUP: true
oauth2_client:
    ENABLE_AUTO_REGISTRATION: true  # Auto-create accounts
    ACCOUNT_LINKING: auto          # Auto-link existing accounts
    UPDATE_AVATAR: true            # Sync avatars from OAuth
    USERNAME: preferred_username   # Use preferred_username claim
```

#### Verification

Test the OAuth flow after setup:

<Steps>
1. Log out of Forgejo (if logged in)
2. Visit `https://forgejo.kbve.com`
3. You should see "Sign in with kbve" button
4. Click it - should redirect to Supabase login
5. After Supabase authentication, redirects back to Forgejo
6. User account is auto-created and logged in
</Steps>
...
**Issue: No "Sign in with kbve" button**
- Check authentication source was added correctly
- Verify the authentication source name matches redirect URI
...
- Verify `ENABLE_AUTO_REGISTRATION` is `true`
- Check Supabase returns required claims (email, preferred_username)
- Review Forgejo logs for registration errors

</Aside>

</BentoProse>

<BentoProse id="submodules" eyebrow="Nested repos" heading="Submodules">
...
<BentoProse id="scripts" eyebrow="Shell helpers" heading="Scripts">
...
<BentoProse id="devops" eyebrow="Culture & practice" heading="DevOps">
...
<BentoProse id="flyio" eyebrow="Quick deploys" heading="FlyIO">
...
<BentoProse id="monorepo" eyebrow="Unified codebases" heading="Monorepo">
...
<BentoProse id="auto-merge-bot-prs" eyebrow="Gated automation" heading="Auto-Merge Bot PRs">

The `ci-auto-merge-bot-prs.yml` workflow provides automated, gated merging for bot-generated PRs that update version files and Kubernetes deployment manifests.
It is never triggered directly by code pushes — only by explicit `workflow_dispatch` or `workflow_call` from other CI workflows.

### Dispatch Chain

The auto-merge bot sits at the end of a two-stage dispatch chain:

1. **`ci-docker.yml` / `ci-publish.yml`** — after a successful image push or package publish, these workflows call the utility workflows below.
2. **`utils-update-version-toml.yml`** — creates a PR on `dev` that updates the `version.toml` to match the just-published version, then dispatches `ci-auto-merge-bot-prs.yml` with the new PR number.
3. **`utils-update-kube-manifest.yml`** — creates a PR on `dev` that updates the `deployment.yaml` image tag to the new version, then dispatches `ci-auto-merge-bot-prs.yml` with the new PR number.

### Security Gates

The bot enforces five checks before approving and merging:

| Gate | Rule | Purpose |
|------|------|---------|
| **Author** | Must be `github-actions` | Prevents human PRs from being auto-merged |
| **Base branch** | Must target `dev` | Never auto-merges to `main` |
| **Labels** | Must have `atomic` or `auto-pr` | Ensures PR was created by an authorized workflow |
| **Title prefix** | Must match an allowlist | Limits which apps can auto-merge |
| **File allowlist** | Only `*/version.toml`, `*/deployment.yaml`, `*/manifest/*.yaml` | Prevents code changes from sneaking through |

### Allowed Title Prefixes

The following prefixes are permitted for auto-merge:

- `chore(ows-*` — OWS service version updates
- `chore(axum-*` — Axum service version updates
- `chore(edge*` — Edge function updates
- `chore(discordsh*` — Discord bot updates
- `chore(herbmail*` — Herbmail updates
- `chore(cryptothrone*` — Cryptothrone updates
- `chore(cityvote*` — CityVote updates
- `Atomic: kube*` — Kubernetes manifest updates
...
<BentoProse id="registry-cleanup-ghcr" eyebrow="Image hygiene" heading="Registry Cleanup (GHCR)">
