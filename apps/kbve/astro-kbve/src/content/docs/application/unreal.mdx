...
tags:
    ...
    - software
bento:
    badge: application · game-engine · software
    title: 'Real-time worlds,'
    accent: Unreal Engine.
    lede: >-
        Epic Games' real-time 3D platform behind high-fidelity games,
        simulations, and interactive experiences. This guide covers the engine
        from Blueprints and C++ through Nanite, Lumen, and multiplayer — plus
        the KBVE plugin collection and the CI/CD pipeline that builds and
        ships Unreal projects from the monorepo.
    ctas:
    ...
    aside:
        icon: render
        title: One engine, every platform
        body: >-
            From Nanite virtualized geometry to Lumen global illumination,
            Unreal Engine 5 pairs film-quality rendering with tools for
            animation, physics, AI, and networking — deployable to desktop,
            console, mobile, and XR from a single project.
        points:
            - <strong>Scripting</strong> — C++ for depth, Blueprints for speed.
            - <strong>Rendering</strong> — Nanite, Lumen, and Virtual Shadow Maps.
            - <strong>Pipeline</strong> — RunUAT builds shipped via KBVE CI/CD.
    stats:
        - { label: Developer, value: Epic Games, icon: crown }
        - { label: First release, value: '1998', icon: clock }
        - { label: Languages, value: 'C++ / Blueprints', icon: code }
        - { label: Royalty, value: '5% after $1M', icon: license }
    features:
        - title: Blueprints
          icon: route
          href: '#blueprints'
          body: Visual scripting for gameplay logic — Blueprint types, communication patterns, and hybrid C++ workflows.
        - title: C++ framework
          icon: code
          href: '#cpp-development'
          body: The Gameplay Framework, reflection macros, and the module-based build system behind every Unreal project.
        - title: Nanite & Lumen
          icon: render
          href: '#rendering'
          body: Virtualized geometry, dynamic global illumination, Virtual Shadow Maps, and the render pipeline options.
        - title: KBVE CI/CD & plugins
          icon: deploy
          href: '#cicd'
          body: RunUAT automation, Agones server builds, and the open-source KBVE plugin collection at packages/unreal.
    related:
        - title: Unity
          href: /application/unity/
          copy: The DOTS-based engine powering Rareicon.
          icon: box
        - title: Godot
          href: /application/godot/
          copy: The open-source engine alternative with Rust bindings.
          icon: sword
        - title: Blender
          href: /application/blender/
          copy: Asset creation and sprite baking for game pipelines.
          icon: camera
    theme:
        hero_bg: https://images.unsplash.com/photo-1542751371-adc38448a05e?q=80&w=2400&auto=format&fit=crop
        accent: '#34d399'
        accent2: '#22d3ee'
        btn_fg: '#06251b'
...
<BentoProse id="getting-started" eyebrow="First steps" heading="Getting Started">
...
<BentoProse id="blueprints" eyebrow="Visual scripting" heading="Blueprints">
...
<BentoProse id="cpp-development" eyebrow="Native code" heading="C++ Development">
...
<BentoProse id="rendering" eyebrow="Nanite & Lumen" heading="Rendering">
...
<BentoProse id="world-building" eyebrow="Large worlds" heading="World Building">
...
<BentoProse id="animation" eyebrow="Motion tools" heading="Animation">
...
<BentoProse id="audio" eyebrow="Sound systems" heading="Audio">
...
<BentoProse id="physics" eyebrow="Chaos engine" heading="Physics">
...
<BentoProse id="ai" eyebrow="Behavior systems" heading="AI">
...
<BentoProse id="multiplayer" eyebrow="Networking" heading="Multiplayer">
...
<BentoProse id="platform" eyebrow="Deploy anywhere" heading="Platform">
...
<BentoProse id="cicd" eyebrow="Automated builds" heading="CICD">
...
### KBVE Unreal CI/CD Pipeline

The KBVE monorepo runs two Unreal Engine workflows:

- **`.github/workflows/ci-unreal.yml`** — Dispatch router (receives workflow_dispatch calls)
- **`.github/workflows/utils-unreal-build.yml`** — Worker (game, server, win-setup, mac-setup modes)

#### Server Build Flow

Server builds produce dedicated server binaries for deployment to Agones fleets.

<Steps>

1. **Extract version** from `version.toml` and `manifest_version` input
2. **Clone external repo** (if `external_repo_url` set) or use monorepo path
3. **Build server target** via UBT (`-Server -TargetType=Server -Configuration=Shipping`)
4. **Package to `/tmp/ue5-build-output/LinuxServer/`**
5. **Deploy to PVC** at `/mnt/longhorn/ows-server/${TARGET}/${VERSION}/`
6. **Symlink latest** (`ln -sfn ${VERSION} /mnt/longhorn/ows-server/${TARGET}/latest`)
7. **Prune old versions** — keep 3 newest by semver, protect `latest` target

</Steps>

**Prune Strategy:**

The prune step uses `sort -V -r` (semver descending) to keep the 3 newest versions for rollback.
It protects the `latest` symlink target and skips versions with `.nfs*` files (active servers).

```bash
# Example: versions 0.3.24, 0.3.25, 0.3.26, 0.3.27, 0.3.28
# Keep: 0.3.28, 0.3.27, 0.3.26
# Prune: 0.3.25, 0.3.24 (unless in use)
```

**Fleet Integration:**

Agones gameserver pods mount the PVC at `/server` with `subPath: ${TARGET}` (e.g., `chuckServer`).
The pod looks for `/server/latest/LinuxServer/${TARGET}.sh` to start the server.

#### Game Build Flow

Game builds produce packaged client binaries for Windows, macOS, Linux, and mobile platforms.

<Steps>

1. **Extract version** from `version.toml`
2. **Clone external repo** or use monorepo path
3. **Build client target** for each platform in `build_targets` (Win64, Linux, Mac)
4. **Cook content** for each target configuration
5. **Package to artifacts** (zip per platform)
6. **Upload to itch.io** (if `deploy_to_itch` enabled)
7. **Create GitHub release** with platform zips

</Steps>

**Version Sync:**

Client and server versions are kept in sync via `ci-dispatch-manifest.json`.
Each Unreal game entry has a single `version` field used for both client and server builds.
...
Projects using external repos (e.g., `chuck` at `github.com/kbve/chuck`) store LFS blobs on Forgejo (`git.kbve.com/KBVE/chuck.git`).
The `kbve.sh -lfs` router injects `-c lfs.url=...` to override `.lfsconfig` per project.
...
<BentoProse id="plugins" eyebrow="Extend the engine" heading="Plugins">
...
<BentoProse id="kbve-plugins" eyebrow="In-house tooling" heading="KBVE Plugins">
...
Third-party plugins extending the Unreal Editor with specialized workflows:

#### SelShader

**[envieous/UnrealEngine-SelShader](https://github.com/envieous/UnrealEngine-SelShader)** — Editor shader selection and inspection toolkit.

Provides advanced shader debugging and visualization tools for material artists and rendering engineers.
Allows selecting shaders directly in the viewport, inspecting compiled shader bytecode, and visualizing pipeline state objects (PSOs).

**Features:**
- Viewport shader picker (click to select active shader stage)
- PSO inspector with draw call metadata
- HLSL bytecode viewer and disassembler integration
- Material instance parameter override visualization
- Shader complexity heatmaps per pass

**Use Case:** Debugging performance hotspots, validating shader permutations, and profiling material complexity in large scenes.

**License:** Check repository for licensing terms.

#### Angelscript

**[Hazelight/UnrealEngine-Angelscript](https://github.com/Hazelight/UnrealEngine-Angelscript)** — AngelScript scripting language integration for Unreal Engine.

Alternative to Blueprints and C++ with text-based scripting, hot reload, and editor integration.
Used in production by Hazelight Studios (*It Takes Two*, *A Way Out*) for rapid gameplay iteration without full C++ recompilation.

**Features:**
- Text-based scripting with syntax similar to C++/C#
- Instant hot reload of script changes in PIE (Play In Editor)
- Full access to UE reflection system (UCLASS, UPROPERTY, UFUNCTION)
- Editor integration with syntax highlighting and autocomplete
- Debugging support with breakpoints and watches
- Lower compile times than C++ for iteration-heavy workflows

**Use Case:** Gameplay scripting for teams needing faster iteration than C++ without Blueprint's visual overhead. Ideal for narrative-heavy or systems-driven games where designers write code.

**Supported Versions:** UE5.1+ (check repo for latest compatibility)
...
<BentoProse id="windows-vm-tooling" eyebrow="Headless Windows" heading="Windows VM Tooling (KubeVirt)">
...
<BentoProse id="macos-vm-tooling" eyebrow="Headless macOS" heading="macOS VM Tooling (KubeVirt)">
...
The `ci-unreal.yml` workflow (`task: mac-setup`, routes to `utils-unreal-build.yml` mode `mac-setup`) follows the same two-job pattern as Windows:
