...
- **`.github/workflows/ci-unreal-build.yml`** — Worker (game, server, win-setup, mac-setup modes)
...
The `ci-unreal.yml` workflow (`task: mac-setup`, routes to `ci-unreal-build.yml` mode `mac-setup`) follows the same two-job pattern as Windows:
