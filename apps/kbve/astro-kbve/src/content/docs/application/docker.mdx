...
bento:
    ...
    lede: >-
        ...
        that runs identically on any machine. This working reference covers the
        day-to-day CLI cheatsheet, every Dockerfile instruction, GPU
        passthrough, BuildX multi-arch builds, WatchTower auto-updates, and
    ...
    aside:
        ...
        body: >-
            A Dockerfile describes how to build an immutable, layered image;
            docker run launches isolated containers from it. Containers share
            the host kernel, so they start in seconds and use far less memory
            than full virtual machines.
        points:
            - <strong>Cheatsheet</strong> — the commands you reach for daily.
            - <strong>Dockerfile</strong> — every instruction, explained.
            - <strong>Automation</strong> — BuildX, WatchTower, and scripts.
    stats:
        - { label: Build engine, value: BuildKit, icon: gears }
        - { label: Image recipe, value: Dockerfile, icon: code }
        - { label: Multi-arch, value: BuildX, icon: deploy }
        - { label: Multi-container, value: Compose, icon: box }
    features:
        - title: CLI cheatsheet
          icon: code
          href: '#cheatsheet'
          body: Container management, inspection, networks, Compose, Swarm, and prune — the full command tables in one place.
        - title: Dockerfile instructions
          icon: box
          href: '#dockerfile'
          body: FROM, RUN, CMD, ENTRYPOINT, COPY vs ADD, HEALTHCHECK, and every other instruction with usage tables.
        - title: BuildX multi-arch
          icon: deploy
          href: '#buildx'
          body: Extended builds on BuildKit — cache export, builder instances, and multi-platform images for ARM and x86.
        - title: WatchTower auto-updates
          icon: clock
          href: '#watchtower'
          body: Automatically pull new images, restart containers, and notify your team when deployments update.
    related:
        - title: Kubernetes
          href: /application/kubernetes/
          copy: Orchestrate containers across clusters once one host is not enough.
          icon: deploy
        - title: Linux
          href: /application/linux/
          copy: The kernel and command-line foundation containers run on.
          icon: server
        - title: Portainer
          href: /application/portainer/
          copy: A web UI for managing Docker containers, images, and stacks.
          icon: settings
    theme:
        hero_bg: https://images.unsplash.com/photo-1605745341112-85968b19335b?q=80&w=2400&auto=format&fit=crop
        accent: '#34d399'
        accent2: '#22d3ee'
        btn_fg: '#06251b'
...
<BentoProse id="tools" eyebrow="In the workshop" heading="Tools">
...
<BentoProse id="orb" eyebrow="macOS runtime" heading="Orb">

OrbStack is a lightweight container runtime designed specifically for macOS.
It provides full Docker CLI compatibility while using significantly less CPU and memory than Docker Desktop.
Containers launch in roughly one second, and the application includes built in support for Linux virtual machines, Kubernetes, and volume mounts with native file system performance.

OrbStack can be installed through Homebrew:

```bash
brew install --cask orbstack
```

After installation, the `docker` and `docker-compose` commands work exactly as they do with Docker Desktop.
No additional configuration is required to switch between the two runtimes.

OrbStack ships with a built in [Kubernetes](/application/kubernetes/) cluster that can be enabled through the settings panel.
The cluster runs a single node locally and exposes `kubectl` to your terminal without any extra tooling.
This makes it straightforward to test manifests, Helm charts, and service meshes before deploying to a remote cluster.
For more details on Kubernetes concepts and cluster management, see the [Kubernetes documentation](/application/kubernetes/).

OrbStack also pairs well with [Talos](/application/kubernetes/) based workflows.
Talos is a minimal, immutable Linux distribution purpose built for running Kubernetes nodes with no shell access and no package manager.
When prototyping Talos configurations locally, OrbStack can serve as the container runtime for spinning up lightweight test clusters before pushing changes to bare metal or cloud infrastructure.
See the [Kubernetes](/application/kubernetes/) page for notes on Talos worker nodes and production cluster design.

For teams that need to run containers locally during development, OrbStack is a practical alternative that reduces overhead without sacrificing compatibility.
See the [Getting Started](/guides/getting-started/) guide for full environment setup instructions.
...
<BentoProse id="dockerfile" eyebrow="Image recipes" heading="DockerFile">
...
<BentoProse id="gpu" eyebrow="Hardware acceleration" heading="GPU">
...
<BentoProse id="setup" eyebrow="Getting started" heading="Setup">
...
<BentoProse id="buildx" eyebrow="Multi-arch builds" heading="BuildX">
...
<BentoProse id="watchtower" eyebrow="Auto updates" heading="WatchTower">
...
<BentoProse id="scripts" eyebrow="Automation" heading="Scripts">
...
<BentoProse id="docker-compose" eyebrow="Multi-container apps" heading="Docker Compose">
...
<BentoProse id="volumes-and-networks" eyebrow="Persistence and isolation" heading="Volumes and Networks">
...
<BentoProse id="best-practices" eyebrow="Do it right" heading="Best Practices">
