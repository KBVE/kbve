...
begin
  ...
  -- Lookup the vault key from the user_secrets table
  select vault_key
    into v_vault_key
  ...
  v_secret := vault.get_secret(v_vault_key);
  return v_secret;
end;
$function$;
...
begin
  ...
  perform vault.delete_secret(v_email_key);
  perform vault.delete_secret(v_password_key);
