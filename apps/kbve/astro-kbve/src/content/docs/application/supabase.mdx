...
begin
  ...
  -- Store secret using Supabase Vault
  insert into vault.secrets (name, description, secret)
  values (v_vault_key, format('user secret for %s', p_key), p_value)
...
begin
  ...
  -- Lookup the vault key from user_secrets
  select vault_key
    into v_vault_key
  ...
  -- Fetch secret using Supabase Vault view
...
begin
  ...
  -- Delete secrets from Vault (via name)
...
begin
  ...
  if p_notes is not null then
    -- First pass: strip dangerous characters (e.g., HTML/script-like syntax)
    v_clean_notes := trim(
      regexp_replace(p_notes, '[<>\$\[\]{}\=\*\`"''\\]', '', 'g')
    );

    -- Second pass: restrict to safe printable characters
    v_clean_notes := regexp_replace(v_clean_notes, '[^\\w\\s.,!?()\\-]', '', 'g');
...
-- Permissions
revoke all on function private.upsert_osrs_account_info_internal(uuid, text, text, text)
  from public, anon, authenticated;

grant execute on function private.upsert_osrs_account_info_internal(uuid, text, text, text)
  to service_role;
...
begin
  ...
  -- Validate account name format early
  if p_account_name !~ '^[a-z0-9_]{3,64}$' then
    raise exception 'Invalid account name format. Only a-z, 0-9, and underscores (3–64 characters) allowed.';
  end if;

  -- Call internal secure function
...
-- Permissions
revoke all on function public.update_osrs_account_info(text, text, text)
  from public, anon;

grant execute on function public.update_osrs_account_info(text, text, text)
  to authenticated;
