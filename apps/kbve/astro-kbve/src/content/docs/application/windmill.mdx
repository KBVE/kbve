...
3. **CI on merge to main**
   `.github/workflows/windmill-sync.yml` triggers on a push to `main` that touches `services/windmill/**`. It runs `windmill-cli` **pinned to the deployed server version** (the CLI and server protocol are version-matched — a mismatch breaks the push).
