...
- [`.github/workflows/utils-blender-flow.yml`](.github/workflows/utils-blender-flow.yml) — reusable `workflow_call` builder. Pulls the target object from LFS, runs the task on `[self-hosted, macOS, ARM64, mac-blender]`, and uploads artifacts.
...
jobs:
  render:
    uses: KBVE/kbve/.github/workflows/utils-blender-flow.yml@dev
    with:
      task: render
      lfs_backend: forgejo
