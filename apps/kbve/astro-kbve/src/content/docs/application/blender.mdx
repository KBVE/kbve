...
sheet whose frame index maps directly to a facing. The component lives at
`apps/agones/arpg/web/scripts/gen-model-sprites.py` with an interactive angle
finder beside it.

<FileTree>
- apps/agones/arpg/web/scripts/
  - gen-model-sprites.py        Blender headless baker (OBJ/FBX → sheet + strip)
  - sprite-postprocess.py       Bakes a soft ground shadow, then stitches the sheet
  - gen-skin-variant.py         Kills a glow color → "powered-down" skin variant
...
2. **Bake the sheet** — paste the copied flags:
   ...
   ```bash
   blender -b -P gen-model-sprites.py -- --model fighter1.obj --skin idolknight.jpg \
...
- **Real (`--real-shadow`)** — the proper one. A ground plane under the hull is flagged
  ...
  frames is seconds).
- **Fake (default)** — `sprite-postprocess.py` derives a shadow from each frame's own
...
A prop can ship multiple skins for different states. `gen-skin-variant.py` masks a
glow color in the skin and desaturates + darkens it, so e.g. the parked ship's green
engines read as **off**:

```bash
python3 gen-skin-variant.py --in idolknight.jpg --out idolknight_off.jpg --hue green
```
