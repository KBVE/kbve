...
**What is the Memes application?**
Memes is the KBVE web service serving meme.sh and www.meme.sh. It is built from
`apps/memes/axum-memes` (a Rust backend with Askama templates) plus the
`apps/memes/astro-memes` static frontend, packaged into the `kbve/memes` image.

**How does traffic reach the Memes service?**
Public traffic enters via Cloudflare, CNAMEs to `gateway.kbve.com`, hits the
Cilium gateway LB, then the `kbve-gateway` `https-memes` listener (cert
`memes-tls` covering meme.sh and www.meme.sh), routes through `memes-route`, and
lands on `memes-service:4321`.

**Why should apex and www not be split across gateway listeners?**
Cilium derives the Envoy filter chain SNI list from the cert SAN, not the
listener hostname. Two listeners sharing one multi-SAN cert emit duplicate filter
chain matches, which Envoy rejects, causing a fall-through 404. Use one listener
with the apex hostname and let the HTTPRoute enumerate every hostname.

## Related

- [Firecracker Node Web](/docs/project/firecracker-node-web) — sibling Docker-pipeline service in the KBVE stack.
