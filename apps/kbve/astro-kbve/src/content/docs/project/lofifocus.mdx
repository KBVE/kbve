...
**What is LofiFocus?**
LofiFocus is a Chrome extension that automatically generates lo-fi music while you browse articles, blogs, and other sites, creating an ambient atmosphere to help you focus while reading.

**How does LofiFocus generate music?**
The extension sends the current URL to an N8N workflow that extracts page metadata, uses ChatGPT to build music prompts, then calls a custom AudioCraft MusicGen model via a Python Flask API on an RTX 4090 GPU; the resulting track is stored in S3 and played back in the browser.

**Does LofiFocus use a custom model?**
Yes. The team fine-tuned Facebook's MusicGen small model on a dataset of non-copyright lo-fi clips paired with descriptive text prompts, training for 100 epochs to specialize it for generating lo-fi hip-hop from text.

## Related

- [Charles](/docs/project/charles) — the KBVE GPU machine-learning build behind model training.