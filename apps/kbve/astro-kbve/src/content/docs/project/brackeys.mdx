---
title: "Brackeys Game Jam — KBVE Godot + Rust Idle Space Shooter"
description: KBVE's Brackeys Game Jam entry — an idle Asteroids-style space shooter built in Godot with Rust (GDExtension) around the theme Nothing Can Go Wrong.
sem: 1
sidebar:
...
jsonld:
    faq:
        - question: What is KBVE's Brackeys Game Jam entry?
          answer: It is KBVE's submission to Brackeys Game Jam 2025.1, built in Godot with Rust via GDExtension. The game is an idle Asteroids-style space shooter where you pilot a mining ship, harvest scraps from asteroids, and upgrade your ship — themed around Nothing Can Go Wrong.
        - question: What is the game's theme and premise?
          answer: The jam theme is Nothing Can Go Wrong. You wake alone on a mining spaceship as sparks appear across the ship, and you hope nothing goes wrong while piloting it. The ship is not automatically destroyed, so it plays as a low-pressure idle space game with optional challenges and bonuses.
        - question: Which engine and assets does the entry use?
          answer: The game uses Godot with Rust (GDExtension) and third-party assets including Kenney Space Shooter Redux (CC0), audioknobs from OpenGameArt (CC-BY-3.0), Foozlecc Void Fleet packs, Galaxy Pixel Art, and Norma 2D's Pixel Space background.
---
...
This page documents KBVE's entry for the **Brackeys Game Jam 2025.1**, an idle
Asteroids-style space shooter built in [Godot](/application/godot/) with
[Rust](/application/rust/) via GDExtension. The jam theme is **Nothing Can Go
Wrong** — you pilot a lone mining ship, harvest resources from asteroids, and
upgrade your vessel while anomalies keep things interesting. The notes below cover
the design flowchart, gameplay loop, resource system, layers, and asset credits.

## Key facts

| | |
|---|---|
| **Event** | Brackeys Game Jam 2025.1 |
| **Engine** | Godot + Rust (GDExtension) |
| **Genre** | Idle Asteroids-style space shooter |
| **Theme** | Nothing Can Go Wrong |
| **Core loop** | Mine asteroids, earn resources, upgrade the ship |
| **Key assets** | Kenney Space Shooter Redux (CC0), Foozlecc Void Fleet, OpenGameArt audioknobs |
...
**What is KBVE's Brackeys Game Jam entry?**
It is KBVE's submission to Brackeys Game Jam 2025.1, built in Godot with Rust via
GDExtension. The game is an idle Asteroids-style space shooter where you pilot a
mining ship, harvest scraps from asteroids, and upgrade your ship — themed around
Nothing Can Go Wrong.

**What is the game's theme and premise?**
The jam theme is Nothing Can Go Wrong. You wake alone on a mining spaceship as
sparks appear across the ship, and you hope nothing goes wrong while piloting it.
The ship is not automatically destroyed, so it plays as a low-pressure idle space
game with optional challenges and bonuses.

**Which engine and assets does the entry use?**
The game uses Godot with Rust (GDExtension) and third-party assets including Kenney
Space Shooter Redux (CC0), audioknobs from OpenGameArt (CC-BY-3.0), Foozlecc Void
Fleet packs, Galaxy Pixel Art, and Norma 2D's Pixel Space background.

## Related

- [Godot](/application/godot/) — the engine powering this jam entry.
- [Rust](/application/rust/) — the language used via GDExtension for the game logic.