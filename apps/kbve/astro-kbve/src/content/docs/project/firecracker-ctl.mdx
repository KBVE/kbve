...
**What does Firecracker CTL do?**
It is a Rust Axum REST API that manages Firecracker microVMs — creating, polling,
and destroying them — with per-VM timeout enforcement and in-memory state tracking
via DashMap.

**How is user code executed inside a VM?**
Code arrives via `env.CODE`, is written to a padded raw block attached as a second
drive (`/dev/vdb`), and the VM init reads it, writes `/tmp/code`, and execs the
entrypoint passed through `boot_args`.

**What are persistent endpoints?**
Long-lived VMs in the networked `-net` deployment that expose HTTP servers over
per-VM TAP networking, routed by name from the public edge and living until
explicitly deleted.

## Related

- [Firecracker Python Web](/docs/project/firecracker-python-web) — Python web rootfs image.
- [Firecracker Node Web](/docs/project/firecracker-node-web) — Node.js web rootfs image.
- [Firecracker Python Net](/docs/project/firecracker-python-net) — networked Python endpoint image.
