...
**What does the Agones Factorio relay do?**
It runs as a sidecar in the Factorio GameServer pod, tailing the console log,
speaking RCON to the game container, bridging chat to the kbve IRC `#general`
channel, and writing telemetry to the gameops ClickHouse database.

**Why is chat and telemetry logic in a sidecar instead of the Factorio image?**
Keeping it out-of-process lets the Factorio container stay vanilla and lets the
relay be iterated and redeployed without rebuilding the 1+ GB Factorio image.

**How does chat reach Discord?**
The relay forwards chat and join/leave events to the kbve IRC network, and the
existing `irc-gateway` service already relays IRC to Discord, so IRC is the
single integration point the game needs.

## Related

- [Factorio CTL](/docs/project/factorio-ctl) — REST control API that reads the telemetry this relay produces.
