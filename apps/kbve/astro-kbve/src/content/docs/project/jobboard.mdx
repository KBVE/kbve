...
> Status: **Phase 0 scaffolded** — `apps/jobboard` Axum service boots, first-party auth implemented (register/login/logout/me with Postgres-backed sessions), `jobboard.proto` wired into the build. **No SQL ships yet** — the `jobboard` schema + seed is deferred to a later dbmate migration (reference shape in the Deployment section). Verified locally against an ad-hoc Postgres. Phases 1–10 below remain spec.
...
| `sessions` | new (tower-sessions Postgres store) |
| `oauth_accounts` | new |
...
| Auth | **First-party** — `tower-sessions` (custom Postgres `SessionStore`), `argon2` hashing, optional `oauth2` (Discord/GitHub/Steam) later | No auth SaaS; transport-flexible (cookie + bearer) |
...
### Database schema (deferred — not committed yet)
No SQL ships in this scaffold by design; the schema lands later as a **dbmate** migration in `packages/data/sql/dbmate/migrations/`. The service queries the `jobboard` schema (`jobboard.users`, `jobboard.sessions`, `jobboard.verticals`, `jobboard.taxonomy`), so those tables + the game-dev seed must exist before it serves. Reference shape to migrate when ready:

```sql
create schema if not exists jobboard;

create table jobboard.users (
    id            uuid primary key default gen_random_uuid(),
    email         text not null unique,
    username      text not null unique,
    password_hash text not null,
    role          integer not null default 0,   -- Capability bitmask
...
-- tower-sessions backing store (custom PgSessionStore)
create table jobboard.sessions (
    id     text primary key,
    data   jsonb not null,
    expiry bigint not null              -- unix seconds
);
create index jobboard_sessions_expiry_idx on jobboard.sessions (expiry);
...
create table jobboard.taxonomy (
    ...
    kind        integer not null,        -- 1 discipline, 2 tool, 3 skill
    name        text not null check (name ~ '^[a-z0-9-]+$'),
    label       text not null,
    status      integer not null default 1,
    unique (vertical_id, kind, name)
);
-- seed: game-dev vertical + 15 disciplines / 13 tools / 6 skills (see spec taxonomy)
```
...
**Locked:** curated, both-sides vetted · gig→apply mechanic · lead-gen v1 (off-platform payments) · monetization-agnostic hooks · **game dev as the launch vertical, vertical-agnostic data model** (verticals + data-driven taxonomy, no hardcoded discipline enum) · decoupled architecture · Rust/Axum API · first-party auth (`tower-sessions` + `argon2`, custom pg `SessionStore`, no auth SaaS) · **jedi `PgCluster` (tokio-postgres + bb8) + dbmate migrations** against the KBVE cluster · standalone `jobboard.proto` (one package, standalone for Phase 0).
