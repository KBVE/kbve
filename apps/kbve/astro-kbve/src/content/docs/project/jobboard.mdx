...
- **RLS / privileges**: schema `create … authorization postgres`; RLS enabled on all 23 tables; `force` on the server-mediated ones (`engagements`, `reviews`, `member_applications`, `reports`, `audit_log`); `revoke all from anon, authenticated` + role-specific `alter default privileges for role postgres … revoke`; schema `revoke from public`, **no `authenticated` usage grant** (`authenticated` → `permission denied`, verified). The `set_updated_at()` fn is `revoke … from public` + `owner to postgres`. **No plpgsql/PostgREST RPC layer** — the Axum service is the single writer.
- **Dedicated least-privilege role** (not `service_role`/`postgres`): RLS **cannot** bound `service_role` (it's `BYPASSRLS`), so the service connects as a **dedicated non-owner `jobboard` role** (forgejo-pattern — `20260615130000_jobboard_dedicated_role.sql` + `schema/jobboard/jobboard_role.sql` + `init/00-roles.sql`). Because it doesn't own the schema, GRANTs + RLS actually constrain it. **Command-scoped grants encode invariants the DB can hold**: no `DELETE` on historical/moderation tables (gigs, engagements, applications, reviews, messages, reports), `reviews`/`messages` immutable (no `UPDATE`), `audit_log` **append-only** (INSERT/SELECT); lookups read-mostly. A permissive `for all to jobboard` policy per table lets the non-owner role pass RLS/`force`; the real limits are the GRANTs (multi-step invariants stay in the service). Prod password via a `jobboard-db` sealed secret → `KBVE_PG_RW_URL = postgres://jobboard:…`.
- Verified up + down on Postgres 17 (stub): 23 tables, 60 indexes; constraint negative battery all rejects; and **as the `jobboard` role**: INSERT/SELECT (with `gen_ulid` default) succeed while `DELETE audit_log/engagements/gigs` and `UPDATE audit_log/messages` are `permission denied`, yet `postgres` (admin) can still delete — role up+down cycles cleanly (`drop owned by` + drop role), tables intact.
