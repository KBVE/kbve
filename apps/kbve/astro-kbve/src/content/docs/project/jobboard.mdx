...
> Status: **Phase 0 scaffolded** — `apps/jobboard` Axum service boots, first-party auth works (register/login/logout/me with Postgres-backed sessions), DB seeded (game-dev vertical + taxonomy), `jobboard.proto` wired into the build. Phases 1–10 below remain spec.
...
| Migrations | **dbmate** (`packages/data/sql/dbmate/migrations`) | Repo-standard migration tool; one system across the monorepo |
