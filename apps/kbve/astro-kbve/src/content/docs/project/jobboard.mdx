...
No SQL ships in this scaffold by design; the schema lands later as a **dbmate** migration in `packages/data/sql/dbmate/migrations/`. The service queries the `jobboard` schema (`jobboard.users`, `jobboard.sessions`, `jobboard.verticals`, `jobboard.taxonomy`), so those tables + the game-dev seed must exist before it serves.

This is the full migration that was removed from the scaffold, parked here for review. When approved, drop it back into `packages/data/sql/dbmate/migrations/<timestamp>_jobboard_init.sql`:
