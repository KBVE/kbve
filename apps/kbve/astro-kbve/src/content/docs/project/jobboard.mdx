...
| Auth | **First-party** — `axum-login` + `tower-sessions` (Postgres store), `argon2` hashing, optional `oauth2` (Discord/GitHub/Steam) | No auth SaaS; transport-flexible (cookie + bearer) |
| DB access | `sqlx` (async, compile-time-checked) | Type-safe SQL; migrations via `sqlx migrate` |
| DB | Postgres (Supabase / Neon) | Relational fits gigs/applications/reviews |
...
### Deployment
- **API:** containerized Axum on Kubernetes (KBVE-native) / Fly.io / Railway (stateful service — not serverless).
- **DB:** Supabase/Neon Postgres (sqlx over TLS). **Object storage:** Cloudflare R2.
...
The platform's wire/storage contract is protobuf, reusing the existing KBVE proto collection (`packages/rust/jedi/proto/*` and `packages/kbve/src/kbveproto.proto`). **The rule: reuse first, extend second, add last.** We add exactly one new file — `jobboard.proto` (package `jobboard`) — for genuinely new domain entities.
...
**Locked:** curated, both-sides vetted · gig→apply mechanic · lead-gen v1 (off-platform payments) · monetization-agnostic hooks · **game dev as the launch vertical, vertical-agnostic data model** (verticals + data-driven taxonomy, no hardcoded discipline enum) · decoupled architecture · Rust/Axum API · first-party auth (axum-login + tower-sessions + argon2, no auth SaaS) · `sqlx` + Postgres · protobuf reuses KBVE protos + one new `jobboard` package.
...
- `sqlx` vs SeaORM/Diesel; cookie-only vs cookie+bearer; OAuth providers (Discord/GitHub/Steam) for v1.
