...
**What is the KBVE Job Board?**
It is a curated, both-sides-vetted freelance platform where sanctioned members post
gigs and other sanctioned members take them up. It launches in the game-dev vertical
but is architected so additional service verticals can be added as data, without
schema or code changes.

**What is the Job Board's technical architecture?**
It is a decoupled Rust/Axum REST API served to a separate frontend client. Auth is
Supabase (verifying the JWT `sub` as `auth.users.id`), DB access is via jedi
`PgCluster` over Postgres, migrations use dbmate, and the wire format is protobuf
over the jedi envelope.

**How does the Job Board add new verticals without schema changes?**
Disciplines, tools, and skills all live in a taxonomy table scoped by `vertical_id`
rather than a hardcoded enum, and gigs and profiles reference taxonomy ids. Launching
a new vertical is a seed-data operation, not a schema or code change.

* * *

## Related

- [rows](/project/rows/) — the KBVE multi-tenant Rust/Axum service whose auth and PgCluster patterns the Job Board reuses.
