...
- **RLS / privileges** locked to an **RPC-only** model: RLS enabled on all 23 tables; `force` on the strictly server-mediated tables (`engagements`, `reviews`, `member_applications`, `reports`, `audit_log`); `revoke all on all tables/sequences from anon, authenticated` + `alter default privileges … revoke` (so future tables inherit lockdown); schema `revoke from public` + `grant usage to authenticated`. The `set_updated_at()` trigger fn is `revoke … from public` + `owner to postgres` (default-privileges only affect later objects). No client policies yet — all writes go through transactional `SECURITY DEFINER` RPCs; per-phase SELECT policies (or PostgREST grants) land with each feature.
- Verified up + down on Postgres 17 (stub `auth.users`/`gen_ulid`): 23 tables; negative battery confirms cross-vertical tag, non-joined talent taxonomy, bad slug, empty label, unpublished-OPEN gig, COMPLETED-without-`completed_at`, dup review, empty message, cross-conversation cursor, `target_kind`/`target_id` reports all reject — while a full conversation delete cascades and a direct participant delete (with messages) is blocked, preserving history.
