...
- **Embed page** — `kbve.com/discord/arpg/` (`public/discord/arpg/index.html` + `arpg.js`, built by `nx run arpg:web-build-discord` — aliased as `astro-kbve:build-arpg-discord` — from `apps/agones/arpg/web/src/embed/discord.tsx`). Boots the Discord SDK, runs OAuth, fetches a session, then mounts `ReactIsoArpgApp` with `embedSession` (skips the Supabase gate + name prompt).
...
| `/arpg-assets` | `arpg.kbve.com` | game art (sprites/ground) served from the vite app's `/assets/...` (CORS-enabled) — the embed sets the arpg client asset base to `/arpg-assets` so absolute art paths resolve to the arpg origin through the proxy, not under the portal-root `/discord/arpg/` dir |
...
**Env (axum-kbve):** `DISCORD_CLIENT_ID`, `DISCORD_CLIENT_SECRET`, `SUPABASE_JWT_SECRET` (required); `GOTRUE_INTERNAL_URL` / `SUPABASE_URL` for fast-register; leave `DISCORD_REDIRECT_URI` **unset** (the RPC authorize flow rejects an explicit redirect_uri). **Build (embed):** `PUBLIC_DISCORD_CLIENT_ID` baked in by the `--mode discord` build in `apps/agones/arpg/web/vite.config.ts` — must be set or terser dead-code-eliminates the boot body. Register a separate Discord Activity application pointing its root at `kbve.com/discord/arpg/`.
