...
| **App** | `arpg-server` |
| **Version** | 0.1.22 |
| **Stack** | Rust (axum + tokio + Bevy headless via simgrid) |
| **Deploy** | Agones Fleet (`apps/kube/agones/arpg/manifests/fleet.yaml`) |
| **Image** | `kbve/arpg-server` |
| **Protocol** | simgrid JSON, `PROTOCOL_VERSION` 14 |
| **Default listen** | `0.0.0.0:7979` (`ARPG_SERVER_ADDR`) |
| **Source** | [`apps/agones/arpg/server`](https://github.com/KBVE/kbve/tree/main/apps/agones/arpg/server) |
...
**What is the ARPG Server?**
ARPG Server is the authoritative shared-world host for KBVE's ARPG, a Diablo-style
isometric action RPG. It pairs an `axum` WebSocket router with the `simgrid`
headless Bevy grid simulation, keeping movement validation, occupancy, and snapshot
framing server-owned.

**How does the ARPG Server stay authoritative?**
The server keeps everything in orthogonal tile `(x, y)` space with integer grid
math; collision is tile occupancy. It validates each move against occupancy, drives
the sim tick on a tokio multi-thread runtime, and fans out authoritative snapshots.
Isometric projection is a client-side render transform only.

**Which wire protocol does the ARPG Server speak?**
It speaks the shared simgrid JSON protocol (`PROTOCOL_VERSION` 14), handling `Join`,
`Step`, `MoveTo`, `Action`, `Face`, `Heartbeat`, and `Leave` frames and emitting
`Welcome`, `Snapshot`, and `Ephemeral` events for combat, projectiles, floor
changes, and stats.

## Related

- [ARPG Web](/docs/project/arpg-web) — the browser client that connects to this server over `/ws`.
- [Cryptothrone Server](/docs/project/cryptothrone-server) — the sibling grid-authoritative host sharing the simgrid crate.
