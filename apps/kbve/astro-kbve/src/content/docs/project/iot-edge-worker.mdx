...
**What is iot-edge-worker?**
A cloud-init template plus shell helper that joins a local dev machine (OrbStack,
Lima, or UTM VM) to the main KBVE Talos cluster as a remote worker over
WireGuard.

**Does it ship as a binary or Docker image?**
No. The artifact is the cloud-init template itself, rendered per-laptop and
consumed by the VM host at first boot — there is no runtime binary.

**How are edge workloads kept off normal nodes?**
The worker is tainted `edge=true:NoSchedule` and labeled `role=edge`, so only
pods that explicitly tolerate the taint and target the label schedule onto it.
