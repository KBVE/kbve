...
**What is DiscordSH?**
DiscordSH is a topsite for Discord servers that promotes them through two
mechanics — a voting system and a bump system — helping server owners increase
visibility and attract new members.

**How does DiscordSH voting differ from bumping?**
Voting is cumulative — users vote once every 12 hours, servers rank by total
votes, and votes reset monthly for long-term visibility. Bumping is a real-time
stream of recently promoted servers with no reset, aimed at short-term exposure.

**What is the DiscordSH tech stack?**
These unofficial notes describe an Astro frontend with Alpine.js and React, a
Rust and Axum backend as a thin layer over Supabase and Postgres with Redis for
ephemeral data, and Kubernetes for deployment, with real-time features over
WebSockets.

## Related

- [DiscordSH Bot](/docs/project/discordsh-bot) — the companion Discord bot for the topsite.
