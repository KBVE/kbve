...
ROWS deploys as a single Docker image (`ghcr.io/kbve/rows`) into the `ows` Kubernetes namespace. ArgoCD syncs the manifest from `apps/kube/rows/manifest/rows-deployment.yaml` with auto-prune and self-heal enabled.
