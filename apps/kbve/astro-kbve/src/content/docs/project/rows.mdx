---
title: ROWS — Single-Binary Rust Open World Server (REST + gRPC)
description: ROWS (Rust OWS) is a single-binary Rust game backend combining REST and gRPC, replacing the multi-service .NET OWS stack with one multi-tenant service.
sem: 1
sidebar:
    label: ROWS
    order: 66
...
jsonld:
    faq:
        - question: What is ROWS?
          answer: ROWS (Rust Open World Server) is a single-binary Rust reimplementation of the OWS game backend. It consolidates the five .NET microservices (PublicAPI, InstanceManagement, CharacterPersistence, GlobalData, Management) into one service exposing both REST (Axum) and gRPC (Tonic) on a single multiplexed port.
        - question: How is ROWS multi-tenant?
          answer: ROWS is multi-tenant by deployment but single-tenant by process — one process per game plus environment, pinning its customer_guid from OWS_API_KEY at boot. A shared Postgres DB is isolated by the customerguid column, and middleware rejects requests whose X-CustomerGUID differs from the process tenant.
        - question: How does authentication work in ROWS?
          answer: ROWS has three identities — tenant (customer_guid from OWS_API_KEY), player (Supabase UUID via a validated HS256 JWT), and trusted server (a service key argon2-verified against SUPABASE_SERVICE_KEY_HASH). Server-write endpoints require the service key; world-IP requests require a confirmed player login.
bento:
    badge: docker · rust · ows
    title: Single-binary Rust
    accent: Open World Server
    lede: >-
        A single-binary Rust reimplementation of the OWS game backend, combining
        REST (Axum) and gRPC (Tonic) on one multiplexed port — replacing the
        five-service .NET OWS stack with one multi-tenant service.
    ctas:
        - label: View source
          href: https://github.com/KBVE/kbve/tree/main/apps/rows
          primary: true
          external: true
    ...
    aside:
        icon: server
        title: One binary, five services
        body: >-
            ROWS consolidates the five .NET OWS microservices (PublicAPI,
            InstanceManagement, CharacterPersistence, GlobalData, Management)
            into a single multi-tenant process exposing REST (Axum) and gRPC
            (Tonic) on one multiplexed port.
        points:
            - <strong>Tenant</strong> — customer_guid pinned from OWS_API_KEY.
            - <strong>Isolation</strong> — X-CustomerGUID enforced per process.
    stats:
        - { label: App, value: rows, icon: server }
        - { label: Stack, value: 'Axum + Tonic', icon: rust }
        - { label: Port, value: '4322 (REST + gRPC)', icon: network }
        - { label: Tenancy, value: multi-tenant, icon: expand }
    features:
        - title: REST + gRPC on one port
          icon: network
          body: Axum player-facing REST and Tonic internal gRPC multiplexed on port 4322 for service-to-service and UE5 dedicated-server coordination.
        - title: Single binary, five services
          icon: package
          body: Consolidates PublicAPI, InstanceManagement, CharacterPersistence, GlobalData, and Management into one Docker image (ghcr.io/kbve/rows).
        - title: Multi-tenant by deployment
          icon: expand
          body: One process per game + environment pins its customer_guid from OWS_API_KEY at boot; a shared Postgres DB is isolated by the customerguid column.
        - title: Three-identity auth
          icon: key
          body: Tenant (OWS_API_KEY), player (Supabase JWT), and trusted server (argon2 service key) are distinct axes with per-gate enforcement.
    related:
        - title: KBVEROWS
          href: /project/kbverows/
          copy: The Unreal Engine REST client plugin for ROWS.
          icon: box
        - title: KBVEAgones
          href: /project/kbveagones/
          copy: Registers UE dedicated-server instances with ROWS.
          icon: server
...
<BentoProse id="overview" eyebrow="Architecture" heading="Rust Open World Server">
...
<Aside type="tip" title="Production">
    ...
    removed — all ChuckRPG traffic routes exclusively to ROWS via the per-environment hosts
    `api-dev.chuckrpg.com`, `api-beta.chuckrpg.com`, and `api-prod.chuckrpg.com`.
</Aside>
...
<BentoProse id="auth" eyebrow="Identity model" heading="Authentication & Identity">
...
<BentoProse id="procedural" eyebrow="Roadmap" heading="Procedural World — seed-based multi-zone">
