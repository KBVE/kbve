...
**What is ROWS?**
ROWS (Rust Open World Server) is a single-binary Rust reimplementation of the OWS game backend. It consolidates the five .NET microservices (PublicAPI, InstanceManagement, CharacterPersistence, GlobalData, Management) into one service exposing both REST (Axum) and gRPC (Tonic) on a single multiplexed port.

**How is ROWS multi-tenant?**
ROWS is multi-tenant by deployment but single-tenant by process — one process per game plus environment, pinning its `customer_guid` from `OWS_API_KEY` at boot. A shared Postgres DB is isolated by the `customerguid` column, and middleware rejects requests whose `X-CustomerGUID` differs from the process tenant.

**How does authentication work in ROWS?**
ROWS has three identities — tenant (`customer_guid` from `OWS_API_KEY`), player (Supabase UUID via a validated HS256 JWT), and trusted server (a service key argon2-verified against `SUPABASE_SERVICE_KEY_HASH`). Server-write endpoints require the service key; world-IP requests require a confirmed player login.

## Related

- [KBVEROWS](/docs/project/kbverows) — the Unreal Engine REST client plugin for ROWS.
- [KBVEAgones](/docs/project/kbveagones) — registers UE dedicated-server instances with ROWS.
