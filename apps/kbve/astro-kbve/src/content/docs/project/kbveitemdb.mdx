...
KBVEItemDB is a game-agnostic Unreal Engine plugin providing an item database
loader, a generic inventory model, and a dropped-item Mass fragment. It loads the
shared itemdb artifact into runtime item defs and looks them up by key or ref.

**How does KBVEItemDB relate to KBVEGameplay?**
KBVEItemDB is the data and inventory half; KBVEGameplay is the effect half. Item
defs carry consumable/food fields aligned to `FKBVEEffectSpec`, so a consumer
builds an effect spec from an item and applies it through KBVEGameplay's
`UKBVEEffectComponent`.

**How are consumables modeled in KBVEItemDB?**
`type_flags` classify items as FOOD, DRINK, or POTION, and a `food` block carries
the numbers such as `heals`, `restore_mana`, `restore_energy`,
`regen_per_second`, `regen_duration`, and `cooldown`. An item with a `food` block
or a food/drink/potion type bit is consumable without an explicit `consumable`
flag.

## Related

- [KBVESQLite](/docs/project/kbvesqlite) — the SQLite provider KBVEItemDB uses for durable inventory persistence.
