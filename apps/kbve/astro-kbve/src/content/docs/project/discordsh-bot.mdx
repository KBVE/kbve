...
It serves slash commands (`/github`, `/gh`, `/dungeon`, `/ping`, `/status`, `/health`, `/admin`, `/skills`, `/n8n`), runs an embed dungeon game with bevy_battle combat and bevy_inventory items, manages GitHub issues and PRs with SVG card rendering, persists players via Supabase, and forwards HMAC-signed requests into in-cluster n8n.

**How does the `/n8n` command stay secure?**
Every `/n8n` payload is signed with `X-KBVE-Signature: sha256=HMAC` over the timestamp and body so n8n workflows verify origin and reject replays. Requests are also filtered by a globset allowlist, and all three required env vars must be present or the command unregisters itself.

## Related

- [DiscordSH](/project/discordsh/) — HTTP server (Astro site, REST API).
- [bevy_inventory](/project/bevy-inventory-crate) — the inventory crate powering the embed dungeon game.
