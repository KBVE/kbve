...
version: "0.1.35"
source_path: apps/discordsh/bot
version_toml: apps/discordsh/bot/version.toml
version_target: apps/discordsh/bot/Cargo.toml
...
jsonld:
    faq:
        - question: What is the DiscordSH Bot?
          answer: discordsh-bot is the standalone Discord gateway bot extracted from the discordsh-api monolith. It handles all Discord interactions independently of the HTTP server, built on the poise/serenity gateway with shard support.
...
bento:
    ...
    lede: >-
        The standalone Discord gateway bot extracted from the discordsh-api
    ...
    ctas:
        - label: View source
          href: https://github.com/KBVE/kbve/tree/main/apps/discordsh/bot
          primary: true
          external: true
        - label: Features
          href: "#overview"
...
`discordsh-bot` is the standalone Discord gateway bot extracted from the discordsh-api monolith. It handles all Discord interactions independently of the HTTP server.
...
`/wm <command> [args]` runs an allowlisted Windmill script and returns its result inline. A blank invocation lists every available command. Scripts live under `services/windmill/f/discordsh/` and opt in by declaring `args` (array) plus an optional `discord` param; the bot POSTs `{args, discord}` to Windmill's `run_wait_result` endpoint.
