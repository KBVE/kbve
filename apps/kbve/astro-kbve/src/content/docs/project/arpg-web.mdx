...
# Pre-build sync: CI writes this MDX `version` into version_target BEFORE the docker
# build so the baked bundle (PUBLIC_ARPG_VERSION) matches the image tag instead of
# lagging a publish cycle (version.toml is otherwise only bumped post-publish).
