...
```bash
nx run arpg-e2e:e2e          # both: boots arpg-server + this web app, runs all specs
nx run arpg-e2e:e2e:server   # arpg-server only (/healthz, /ws JoinMatch handshake)
nx run arpg-e2e:e2e:web      # this app only (SPA shell, root mount, SPA fallback, clean boot)
```

The server specs sign a Supabase-style HS256 JWT (the server runs in local-secret mode) and assert the JSON wire handshake — `Welcome` carries `PROTOCOL_VERSION` 14, and protocol/auth/username failures each emit a `Reject`. The web specs boot the Vite dev server with the prod Supabase URL injected and assert the app shell loads without an uncaught console error.
