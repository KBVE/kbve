...
`QPhysics3D` exists in `q/src/rapier/bridge3d.rs` but is instantiated **nowhere in the game** — only in `test_net_game_client.gd`. Online gets its physics because the *server* runs `SimWorld`; the client only interpolates. Solo has no server, so it has no q physics at all.
