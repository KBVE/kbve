...
### Two simulations, and the plan to have one

Solo and online do not run the same simulation, and most confusing creature behaviour traces back to that.

| | Solo (`scenes/main.tscn`) | Online (`scenes/online.tscn`) |
| --- | --- | --- |
| Bodies | `CharacterBody3D` + `move_and_slide()` | rapier `SimWorld`, host-side |
| Steering | `QPatrol` per node | `PetRegistry` (`q/src/net/pets.rs`) |
| Robots | "mechs", `creature_spawner.gd` | "pets", `src/net/net_pet.gd` |
...
Online gets its physics because the *server* runs `SimWorld`; the client only interpolates. Solo has no server, so it has no q physics at all.

`QPhysics3D` in `q/src/rapier/bridge3d.rs` is **not** the way in, and wiring it up is not the migration. It is a rigid-body bridge — `spawn_ball`, `spawn_box`, `spawn_capsule`, `spawn_static_box`, `apply_impulse`, `body_position` — plus `track_capsule`, which registers a proxy for a body *Godot still moves*. There is no `SpawnCharacter` or `MoveCharacter` on it. The character controller lives in `sim3d::SimWorld` and is driven only from the net/host side, so it has no Godot-facing path at all. Moving solo onto it means writing that bridge, not instantiating a node.

The target is one simulation for both, with multiplayer differing only in *where* the registry runs. Staged: collapse tuning to one source; move neighbour indexing and slot assignment into a Rust registry, keeping Godot bodies; then move character motion into `SimWorld`.

**Things Godot only does because it owns motion in solo.** Each is a responsibility to hand back, not a feature to port.

`creature_patrol.gd` scales the whole wish vector by facing alignment, so a mech leans into its turn instead of sliding sideways:

```gdscript
wish *= lerpf(TURN_GATE_FLOOR, 1.0, gate)   # 0.35 .. 1.0
```

The intent is right and the placement is not: `wish` is goal **plus** avoidance summed, and Godot cannot tell them apart — so turning damps avoidance to 35% at the moment a group is most likely to converge. Only the solver knows which half is which.

Creatures also sit on `LAYER_CREATURE` and deliberately do not mask it, so they pass through each other. **Do not re-add depenetration.** It was removed because it fought the solver — the solver would open a gap, the engine would close it along the shortest axis, and on a slope that axis is often vertical, which parks one machine on top of another where it reports `is_on_floor` and stays. Unification is what makes separation safe again, because both halves then resolve in one tick against one state.
...
Tuning currently lives in three places — `PetConfig`, the `creature_patrol.gd` exports, and `Config::default()`. Every steering test runs the last one, and nothing ships it, which is how a probe comes back clean while the game misbehaves.
