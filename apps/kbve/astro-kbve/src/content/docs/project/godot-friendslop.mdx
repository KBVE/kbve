...
Not yet done: NPCs are still only in `main.tscn`, and they have no colliders in either scene. The moment one of them *does* something — produces goods, takes damage, holds a quest item — the clock stops being enough and the host has to own them, because that depends on what players did rather than what time it is.
