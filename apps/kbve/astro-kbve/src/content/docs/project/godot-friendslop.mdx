...
`sign_in` posts to GoTrue's password grant at `supabase.kbve.com`, keeps the access and refresh tokens in memory only, and reads the username out of the token for the title to draw. Nothing is written to `user://` — a refresh token on a shared machine is a login. `adopt_account` is the same seam an OAuth flow would land on.

The token is what the **server** verifies; the username decoded on this side is only for what the title screen prints before the join. See [friendslop-server](/project/friendslop-server) for the verification half.

**Password sign-in cannot succeed against production today.** GoTrue runs with `GOTRUE_SECURITY_CAPTCHA_ENABLED=true` (hCaptcha), and the password grant answers `captcha protection: request disallowed (no captcha_token found)` to any client that has no browser to solve one in. The form reports that as "not available yet" rather than as a wrong password. The two ways out are a loopback OAuth flow (`GOTRUE_URI_ALLOW_LIST` already permits `http://127.0.0.1:**` and `kbve://**`) or exempting the grant from captcha; the client half is otherwise finished and works the moment either lands.
