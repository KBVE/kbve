...
**What is the KBVE Minecraft Velocity proxy?**
A custom Velocity 3.4 proxy image that replaces the generic `itzg/bungeecord`
base and ships two pre-baked Kotlin plugins — `kbve-velocity-commands` and
`kbve-discord-relay` — built from source.

**What do the two plugins do?**
`kbve-velocity-commands` adds teleport aliases, a cross-server chat bridge, and
last-server persistence, while `kbve-discord-relay` bridges in-game chat to and
from a Discord channel with role-gated console commands.

**Are the plugins coupled?**
No. They have separate `@Plugin` IDs, versions, and no source dependency, so they
can be enabled, disabled, and rolled back independently even though they ship in
the same image.

## Related

- [mc (Fabric survival backend)](/docs/project/mc)
- [mc-lobby (PaperMC lobby)](/docs/project/mc-lobby)
