...
There is no account layer yet, so every player is a guest and the **server** names them. `SessionMsg::Join` carries a requested name; the host sanitizes it (ASCII alphanumerics plus single interior `_ - .`, 24 chars, no all-digit names) and falls back to a generated `Anon-XXXX` when the request is empty, unusable, or already taken. What the client asked for is never what it renders — `Welcome` carries the granted name, and a `Roster` message broadcasts `{peer, body, name}` for everyone on every join and leave.

That split is the point: a display name is drawn on other players' screens, so it is host-authored data that happens to have been suggested by a client.
