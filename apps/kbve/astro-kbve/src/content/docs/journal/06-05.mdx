---
title: 'June: 05'
category: Daily
date: 2026-06-05 12:00:00
client: Self
unsplash: 1717570994511-7abcd5f84183
img: https://images.unsplash.com/photo-1717570994511-7abcd5f84183?crop=entropy&cs=srgb&fm=jpg&ixid=MnwzNjM5Nzd8MHwxfHJhbmRvbXx8fHx8fHx8fDE2ODE3NDg2ODY&ixlib=rb-4.0.3&q=85
description: |
    June 5th. AstroPad
tags:
    - daily
template: splash
tableOfContents: false
editUrl: false
lastUpdated: false
prev: false
next: false
bento:
    badge: 'journal · daily log'
    title: 'June 5,'
    accent: every year.
    lede: 'The June 5th daily log — notes, tasks, and quotes collected across every year of the KBVE journal.'
    ctas:
        - label: Read the log
          href: '#2026'
          primary: true
        - label: '← Jun 4'
          href: /journal/06-04/
        - label: 'Jun 6 →'
          href: /journal/06-06/
    related:
        - title: 'Previous · Jun 4'
          copy: 'Step back a day to the June 4th log.'
          href: /journal/06-04/
          icon: calendar
        - title: 'Next · Jun 6'
          copy: 'Jump ahead to the June 6th log.'
          href: /journal/06-06/
          icon: arrow
        - title: 'Journal hub'
          copy: 'Every daily log, grouped by month, plus the blog archive and RSS feed.'
          href: /journal/
          icon: book
    theme:
        hero_bg: 'https://images.unsplash.com/photo-1717570994511-7abcd5f84183?q=80&w=2400&auto=format&fit=crop'
        accent: '#a78bfa'
        accent2: '#f472b6'
        btn_fg: '#1e1035'
---

import Adsense from '@/components/ads/Adsense.astro';
import BentoDoc from '@/components/hero/BentoDoc.astro';
import BentoProse from '@/components/hero/BentoProse.astro';

<BentoDoc data={frontmatter.bento}>

<BentoProse id="2026" heading="2026">

- [ ]

</BentoProse>

<BentoProse id="2025" heading="2025">

### Codex Test Run

-   05:25PM

    Testing out how good the GPT codex is and its ability to operate within a monorepo!
    That means I need to start adjusting the `kbve.sh` or better yet, I could make a new shell script to help bridge the codex changes.
    I wonder if there might be a safe way to handle it, maybe setting up an endpoint for it to communicate through?

### IObject Schema

-   04:25AM

    Updating the schemas for not only the IObjects but preparing them for the observability.
    `IObjectSchema` will be the single source of truth again for the ItemDBs.

### Steam RareIcon

-   05:08PM

    We approved and now have a straight path to success!
    Going to finish some of the schema changes and focus on making sure the editor can create the right quests next.
    Afterwards, I want to loop back around to the UI and maybe de-couple more of it from the scene itself.

-   05:30PM

    The marketing plan to reach over 1000 wishlists will not be too hard because of the friends of friends system, but going into the 10,000 mark will be a pain.
    I am thinking the best way to do that will be to ask if some communities would like to help in exchange for in game items, basically extend the lore out for them to be involved.

</BentoProse>

<section class="bento-section not-content">
	<div class="bento-frame bento-band">
		<Adsense />
	</div>
</section>

<BentoProse id="2024" heading="2024">

**Music** 

Adding the `Rock` genre to the jukebox and damn, it is such a pain to label music.
Do I want to include folk or maybe blues, ugh, I will just throw it all into this label for now and move forward.

We want the jukebox to focus on unique and outcast style music, things within the indie nature or music that is tough to find from the mainstream.

The overall goal of the jukebox is not to compete or replace any pandora or spotify, but rather help people find and listen to unique music that would be super fast.

```bash

./kbve.sh -nx kbve.com:music --args="--file=dnb --title='Ray Volpe - Laserbeam (ÆON:MODE Remix)' --ytid=jjoos7UMfe4"

```

Expanding the music collection will take some time but I am enjoying the little bits of adding unique songs here and there.


```bash

./kbve.sh -nx kbve.com:music --args="--file=nujazz --title='Lord Apex - Love Me or Hate Me' --ytid=86ukyi4w6rY"

```

Add in the Yamashita! 

```bash

./kbve.sh -nx kbve.com:music --args="--file=nujazz --title='MF DOOM X Yamashita - Rae Sparkle' --ytid=_b99ARr8Zmk"

```

The next couple songs that I need to add are the:

- Windy Ladies
- Dancer Ain't Nothing , ah found it! Yes!
- Melody Go Round - Also found this one, remastered too, yessir!
- Great Slider
- Air Book - got it!

Command example of adding these songs are below:

```bash

./kbve.sh -nx kbve.com:music --args="--file=nujazz --title='MF DOOM x Yamashita x Tanda - Dancer Aint Nothing' --ytid=qAnNPGUhOws"

```
---

```bash

./kbve.sh -nx kbve.com:music --args="--file=nujazz --title='MF DOOM x Tatsuro Yamashita x Tanda - Melody Go Round ' --ytid=ojGwEt5e44A"

```

---


```bash

./kbve.sh -nx kbve.com:music --args="--file=nujazz --title='MF DOOM x Tatsuro Yamashita x Tanda - Air Book' --ytid=kYu9GLhHme8"

```

So the only two songs that are missing are the Windy Ladies and the Great Slider, I will try to upload those into the KBVE YouTube channel and then come back around and throw them in here.

Oh! I remember this great nujazz style song that we could add into the document.

```bash

./kbve.sh -nx kbve.com:music --args="--file=nujazz --title='ZHU - Paradise Awaits (LCAW Remix)' --ytid=iKW41jbJcWM"

```

Perfect! This way I can continue to work on different areas at the same time too.

Let me add the white flag next :D

```bash

./kbve.sh -nx kbve.com:music --args="--file=dnb --title='Delta Heavy - White Flag VIP' --ytid=3YHNTXYLn8k"

```

Now I am testing the groq.com api to help me generate the commands, just to double check and it is doing a great job!

```bash

./kbve.sh -nx kbve.com:music --args="--file=nujazz --title='Matty Rico - Topic' --ytid=SfgLPHM-hMY"

```

Now there are some changes that I still need to make in the prompt but by providing more examples , it was able to handle it without any concern. 


**Graph**

Updating the Graph MDX under Projects because it might have been lost in a stash.

We can start with adding a basic outline and then add any additional information that we would need.

**Cryptocurrency**

Going to take some time and update the Cryptocurrency section on KBVE.

There are a couple cool tools that I could add into the page but for now getting the most basic information thrown onto it would be the move.

<Adsense />

</BentoProse>

<BentoProse id="2023" heading="2023">

- 7:00am - Most of the morning was spent looking into how I should handle svelte within this `kbve.com` repo, but I need to open up Unity and dive a bit deeper into that workflow.
I was able to clear up the invoice for Silver and we should be holding a meeting later today regarding the future of the game.

- 9:00am - Looking over the UI/UX and there are a couple things that we could improve upon, including the mini-map and general flow of information on the screen.

- 12:00pm - RJ Meeting! I will come back to this aspect and update the notes regarding this issue.

- 4:00pm - Going to try the Unity Cloud build and hopefully it will be able to build the application without any major issues.
I will have to look into how to setup the whole Unity DevOps, but from my basic understanding, it looks pretty straight forward!
We might try and have both repos active, one on Github and one of Unity cloud but it might end up becoming a bit too costly to run in the long term.

</BentoProse>

<BentoProse id="quote" heading="Quote">

> The person who lives life fully, glowing with life's energy, is the person who lives a successful life.
> — <cite>Daisaku Ikeda</cite>

---

</BentoProse>

<BentoProse id="tasks" heading="Tasks">

- [X] - Fix the Git LFS / Github build issue with Unity. UPDATE: This task was just removed from our log.
- [ ] - Integrate Unity Cloud builder for RougeJester.

</BentoProse>

</BentoDoc>
