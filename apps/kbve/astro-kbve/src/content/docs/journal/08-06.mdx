---
title: "August: 6th"
category: Daily
date: 2026-08-06 12:00:00
client: Self
unsplash: 1689308271349-e685f6ec405a
img: https://images.unsplash.com/photo-1689308271349-e685f6ec405a?crop=entropy&cs=srgb&fm=jpg&ixid=MnwzNjM5Nzd8MHwxfHJhbmRvbXx8fHx8fHx8fDE2ODE3NDg2ODY&ixlib=rb-4.0.3&q=85
description: August 6th. .
tags:
  - daily
template: splash
tableOfContents: false
editUrl: false
lastUpdated: false
prev: false
next: false
bento:
    badge: 'journal · daily log'
    title: 'August 6,'
    accent: every year.
    lede: 'The August 6th daily log — notes, tasks, and quotes collected across every year of the KBVE journal.'
    ctas:
        - label: Read the log
          href: '#2026'
          primary: true
        - label: '← Aug 5'
          href: /journal/08-05/
        - label: 'Aug 7 →'
          href: /journal/08-07/
    related:
        - title: 'Previous · Aug 5'
          copy: 'Step back a day to the August 5th log.'
          href: /journal/08-05/
          icon: calendar
        - title: 'Next · Aug 7'
          copy: 'Jump ahead to the August 7th log.'
          href: /journal/08-07/
          icon: arrow
        - title: 'Journal hub'
          copy: 'Every daily log, grouped by month, plus the blog archive and RSS feed.'
          href: /journal/
          icon: book
    theme:
        hero_bg: 'https://images.unsplash.com/photo-1689308271349-e685f6ec405a?q=80&w=2400&auto=format&fit=crop'
        accent: '#a78bfa'
        accent2: '#f472b6'
        btn_fg: '#1e1035'
---

import Adsense from '@/components/ads/Adsense.astro';
import BentoDoc from '@/components/hero/BentoDoc.astro';
import BentoProse from '@/components/hero/BentoProse.astro';

<BentoDoc data={frontmatter.bento}>

<BentoProse id="2026" heading="2026">

- [ ]

</BentoProse>

<BentoProse id="2025" heading="2025">

### Disoxide

-   10:35PM

    Another day in the dev world, this time we took a quick break from the general kubernetes and focused on the discord project.
    The template is almost done, added a new infinite scrolling library and improved the footer of the site!
    Next we need to add the actual server database and setup the voting and bumping for the site, both we have already written the code for.
    So the plan is just to link them together moving forward.

    The tables for the server could be done via the rust application deployment or having the schemas be split.
    We could have it in both areas too, just to be safe and of course the schemas have to be double checked regardless.

</BentoProse>

<section class="bento-section not-content">
	<div class="bento-frame bento-band">
		<Adsense />
	</div>
</section>

<BentoProse id="2024" heading="2024">

**RockHaus**
Nothing says we have entered the deep parts of the wowcraft than a bunch of us streaming ourselves playing all day.

**Gaming**
The gaming docs section of the KBVE collection needs to be updated, I will try to fix that up right now.
One of the things we can do is make sure to import everything from starlight that we might need for the document.
Here is the generic imports that we commonly use:

```js
import {
  Aside,
  Steps,
  Card,
  CardGrid,
  Code,
  FileTree,
  LinkCard,
  Tabs,
  TabItem,
} from '@astrojs/starlight/components';
```

We will try to inclue a couple different sections to describe the general gaming document.
The next update will be in the configuration for the website and place `gaming` inside of the `astro.config.mjs`

```js

				{
					label: 'Gaming',
					collapsed: true,
					autogenerate: { directory: 'gaming' },
				},

```

The menu for the gaming section will remain collapsed as a default setting.


**WoW Movie**
A great way to end the night!
Granted, with the south park episode and the movie, we been down this rabbit hole twice already and it feels a bit too much for me.
However, getting a chance to see Preacher again is always worth it.

**MapDB**


<Adsense />

</BentoProse>

<BentoProse id="2023" heading="2023">

- 12:05pm - I should find a way to combine the weekly reports that I get into one? I suppose we could do that via Postboy? Have it shift through the reports that get generated? Maybe make it a bit more modular and then work from there. I suppose we could utilize a bit of creativity to handle this. I believe the best way to approach this would to be a module that would initially scope out the data and then place it into a Google sheet based, this way we could organize and refine the data we would want to display onto our 
- 3:01pm - Getting ready for another night shift of programming and running around with the meme gang. 
- 4:07pm - The first report that could parse through would be the Google webmaster / search results data. This would be a bit easier to do from the current standpoint,  I believe the hardest part of this would be to manage the sessions, including the login data. I suppose setting up a chrome session with pre-defined instances might make this easier but there would be a security risk that I would have to account for. The tools we would use for this automation process might just be n8n, as it would make replication and migration a bit easier too incase someone else wanted to use them. 
- 4:34pm - Time to grab some coffee and prepare for general rounds. The bigger issue for me to tackle and shift this block towards will be to resolve the tags AND also make some minor updates to the visual novel.
- 4:40pm - The `Cover` widget is ready to move into this daily patch, with the purpose of providing a generic cover for the VN aka Mangas. The first demo of this will be used for the FinTech and the Conch series, both I should wrap up today! They are minor issues that could be resolved without too many pain points, at least getting the basics out of the way.
- 7:17pm - Let me sync these notes for now and I will start with the cover widget afterwards. 

</BentoProse>

<BentoProse id="quote" heading="Quote">

> Value your friendship. Value your relationships.
> — <cite>Barbara Bush</cite>

---

</BentoProse>

<BentoProse id="tasks" heading="Tasks">

- [ ]

</BentoProse>

</BentoDoc>
