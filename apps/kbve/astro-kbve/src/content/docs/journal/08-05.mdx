...
bento:
    ...
    title: 'August 5,'
    accent: every year.
    lede: 'The August 5th daily log — notes, tasks, and quotes collected across every year of the KBVE journal.'
    ctas:
        - label: Read the log
          href: '#2026'
          primary: true
        ...
        - label: 'Aug 6 →'
          href: /journal/08-06/
    related:
        - title: 'Previous · Aug 4'
          copy: 'Step back a day to the August 4th log.'
          href: /journal/08-04/
          icon: calendar
        - title: 'Next · Aug 6'
          copy: 'Jump ahead to the August 6th log.'
          href: /journal/08-06/
          icon: arrow
        - title: 'Journal hub'
...
-   09:15PM
    ...
    Spent the whole day working on the kong api gateway, dealing with CORS issues and general flow.
    There will be three API gateways that we plan to build out, one will be for the Supabase instance itself and the second will be for KBVE.
    Third API gateway will be for meme.sh, but that is still a while from now, I do need to start moving faster.

-   11:05PM

    I did start the process of hooking up the logflare but there are still some issues around it that we have not yet fully resolved.
    The logs are not being handled to our best of ability, it will be something that I have to sit down and walk through each step.
