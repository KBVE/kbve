---
title: "July: 24"
category: Daily
date: 2026-07-24 12:00:00
client: Self
unsplash: 1505236732171-72a5b19c4981
img: https://images.unsplash.com/photo-1505236732171-72a5b19c4981?crop=entropy&cs=srgb&fm=jpg&ixid=MnwzNjM5Nzd8MHwxfHJhbmRvbXx8fHx8fHx8fDE2ODE3NDg2ODY&ixlib=rb-4.0.3&q=85
description: July 24th. LootTable
tags:
  - daily
template: splash
tableOfContents: false
editUrl: false
lastUpdated: false
prev: false
next: false
bento:
    badge: 'journal · daily log'
    title: 'July 24,'
    accent: every year.
    lede: 'The July 24th daily log — notes, tasks, and quotes collected across every year of the KBVE journal.'
    ctas:
        - label: Read the log
          href: '#2026'
          primary: true
        - label: '← Jul 23'
          href: /journal/07-23/
        - label: 'Jul 25 →'
          href: /journal/07-25/
    related:
        - title: 'Previous · Jul 23'
          copy: 'Step back a day to the July 23rd log.'
          href: /journal/07-23/
          icon: calendar
        - title: 'Next · Jul 25'
          copy: 'Jump ahead to the July 25th log.'
          href: /journal/07-25/
          icon: arrow
        - title: 'Journal hub'
          copy: 'Every daily log, grouped by month, plus the blog archive and RSS feed.'
          href: /journal/
          icon: book
    theme:
        hero_bg: 'https://images.unsplash.com/photo-1505236732171-72a5b19c4981?q=80&w=2400&auto=format&fit=crop'
        accent: '#a78bfa'
        accent2: '#f472b6'
        btn_fg: '#1e1035'
---

import { Adsense } from '@/components/astropad';
import BentoDoc from '@/components/hero/BentoDoc.astro';
import BentoProse from '@/components/hero/BentoProse.astro';

<BentoDoc data={frontmatter.bento}>

<BentoProse id="2026" heading="2026">

- [ ]

</BentoProse>

<BentoProse id="2025" heading="2025">

### Hulk

-   07:20+PM

    Death comes in threes, did not expect to see hulk go out this fast.
    He holds a spot in my wrestling phase because I was a huge fan of his work!
    My grandpa and a bunch of his friends loved watching him perform too, so that also makes it hold a special part in my soul.

### Entity vs Entity.

-   08:29PM

    The entity vs entity system will be an important part of the Airship game because we want to create a world that exists without having to play.
    For now, I want the dragons and the navy ships to have constant battles, then I will start to introduce ground troops like goblins and navy guards afterwards. 
    These units will move from tile to tile, setting up tents and engaging in long standing wars, with the player being able to either fly over and help or if flying too close, take damage.

</BentoProse>

<section class="bento-section not-content">
	<div class="bento-frame bento-band">
		<Adsense />
	</div>
</section>

<BentoProse id="2024" heading="2024">

**LootTable**

The next major patch for the Laser package would be to build out a `LootTable`.
I am thinking the structure the table with an array of objects that represent the item, types of loot and the rolls for it?
We could include a chance on steal, drop from combat and tradable for a certain gold value.
The best idea would be to start small and then expand from there based upon the different options.

```ts

export interface LootableObject {
  ulid?: string;
  name?: string;
  chance: number; 
}

```

We also need to factor in the roll that the user gets and compare it to the chance table.
One way to handle this would be to take the rolls value and convert it over to a 100 point scale, basically a function to help us handle this.


<Adsense />

</BentoProse>

<BentoProse id="2023" heading="2023">

- 5:45am - The morning was a bit too hot for my blood, I made the mistake of not running the AC over night at a set temp, but I woke up in a pool of sodium-fused run off. The goal for today will be to keep things simple and hit a couple minor tasks.
- 7:00am - The bald spot has been increasing, I can no longer deny it, after getting out of the shower, I nearly saw my future bald self, I will have to... I am sorry, I must get stronger. The difference in self confidence will be depend on my ability to morph into the Rock or become a hermit buddha. 
- 12:00pm - Roll my weekly option contracts and then take the credits! I will have a bit of a slow week, I am only looking at around $500~ in profits, as I am slowly diminishing my cash reserves a bit too fast. Ideally I would want to slow it down, but there are only so many cuts that I can make and the generation of income has also been growing too slow. I made notes of this earlier but need to get back on the grind.
- 2:00pm - I am test casing the patch system but there are a couple things that I am already not a big fan of! The current bug that is driving me a bit nuts is why can't codesee's graph generation proc faster? I am thinking that I should experiment a bit with their action? Maybe I should merge their action with the patch pull action? It would be worth a shot to see what would happen. Furthermore, I need to start moving the needle on the backend auto-deployment and tackle some of the unique bugs that I know will come up.
- 3:20pm - Taking a bit of a programming / dev break and looking over my MTG card/deck. I am really thinking that I should build my trump and yoda deck before I play in person again. Both of the decks will be about 100 cards each and follow the EDH format, but I play with cool people, so proxy cards are always acceptable. I refuse to spend too much money on MTG cards and honestly `puttin` Trump onto the field and letting my friends destroy him, has its own unique taste to it. The coolest part about the deck is that I get to spawn Trump walls, which only make Trump stronger too lmfao. 
- 8:15pm - Mini Charles aka Charlie needs to come back online as well. I started to plug some of the base components within the older StormTrooper PC Tower, the good news is that it turns on but the neutral news is that we are not too sure how well it can run and there are way too many spider webs. If any PC gamer saw what I have done to this machine, dear lord.
- 8:32pm - Updating my daily journal and then going to push through the addition of codesee to the patch workflow action, let us see if that was worth wild. 

</BentoProse>

<BentoProse id="quote" heading="Quote">

> Technology is anything that wasn't around when you were born.
> — <cite>Man Ray</cite>

---

</BentoProse>

<BentoProse id="tasks" heading="Tasks">

- [ ] R&D The Pipeline

</BentoProse>

</BentoDoc>
