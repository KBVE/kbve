...
bento:
    ...
    title: 'January 4,'
    accent: every year.
    lede: 'The January 4th daily log — notes, tasks, and quotes collected across every year of the KBVE journal.'
    ctas:
        - label: Read the log
          href: '#2026'
          primary: true
        ...
        - label: 'Jan 5 →'
          href: /journal/01-05/
    related:
        - title: 'Previous · Jan 3'
          copy: 'Step back a day to the January 3rd log.'
          href: /journal/01-03/
          icon: calendar
        - title: 'Next · Jan 5'
          copy: 'Jump ahead to the January 5th log.'
          href: /journal/01-05/
          icon: arrow
        - title: 'Journal hub'
...
-   10:56AM
    ...
    Adding the custom prebuild script to create all 10K MDX files for the OSRS items.
    The plan is to migrate all the dynamic paths into the static paths for the OSRS flow.
    Then we pull the live data from our new API endpoints and render the chart/data from there!
    Later on, we can use the db workers to keep the data safely stored in the user's browser storage.
