...
bento:
    ...
    title: 'May 2,'
    accent: every year.
    lede: 'The May 2nd daily log — notes, tasks, and quotes collected across every year of the KBVE journal.'
    ctas:
        - label: Read the log
          href: '#quote'
          primary: true
        ...
        - label: 'May 3 →'
          href: /journal/05-03/
    related:
        - title: 'Previous · May 1'
          copy: 'Step back a day to the May 1st log.'
          href: /journal/05-01/
          icon: calendar
        - title: 'Next · May 3'
          copy: 'Jump ahead to the May 3rd log.'
          href: /journal/05-03/
          icon: arrow
        - title: 'Journal hub'
          ...
          icon: book
    theme:
        hero_bg: 'https://images.unsplash.com/photo-1679848148238-5ebc18399015?q=80&w=2400&auto=format&fit=crop'
        accent: '#a78bfa'
        accent2: '#f472b6'
        btn_fg: '#1e1035'
---
...
### Argo Syncs for Unity Cache

-   05:25AM

    Up all night doing the damn argo syncs, really wanting to get that near perfect score.
    Majority of the issues are just rbac and/or de-syncs because of updates, so it is just time consuming, ugh.

-   02:45PM

    The current goal is to get the unity game to build without breaking, hopefully we can get the cache to work this time around.
    This is a bit of a loop, we are trying to get the pipeline to become functional but also making optimizations at the same time.
    I would not recommend this method all the time, but sometimes it is what it is.

### KBVE Release v1.0.129

-   08:13AM
    
    The KBVE website release is being prepared, I did not get a chance to save it.
    We will have to update the arc runners as well, but we shall see.

### Arc Runner Self Hosted

-   08:12PM

    Wow it is already 8pm, but it seems that we still have not yet gotten the unity to run correctly in our self hosted runner using DinD.
    I am a bit surprised that we have these issues, I figured we would have solved them by now.
    One of the other issues is that our self hosted image did not have any e2e tests, which caused it to automatically fail.
