...
bento:
    ...
    title: 'June 17,'
    accent: every year.
    lede: 'The June 17th daily log — notes, tasks, and quotes collected across every year of the KBVE journal.'
    ctas:
        - label: Read the log
          href: '#2026'
          primary: true
        ...
        - label: 'Jun 18 →'
          href: /journal/06-18/
    related:
        - title: 'Previous · Jun 16'
          copy: 'Step back a day to the June 16th log.'
          href: /journal/06-16/
          icon: calendar
        - title: 'Next · Jun 18'
          copy: 'Jump ahead to the June 18th log.'
          href: /journal/06-18/
          icon: arrow
        - title: 'Journal hub'
...
### BlackJack Cryptothrone

-   09:15PM

    The goal will be to have a small casino inside of cryptothrone, so my players can enjoy some poker.
    I am thinking the two major games will be poker and blackjack, afterwards I might try to integrate some other games.
...
-   04:00AM
    ...
    The website looks terrible in light mode, so I am going to try and just strip all that out and keep it dark mode only.
    This should make things easier on the browser and avoid that terrible flashing that occurs when the site is loading.

### Unity Build failure

-   08:30AM

    Looks like the build failed but I am not too sure why it did, maybe it was because we had those extra files.
    Maybe it was because I included the assembly files by accident, hmm but that does not make sense.
    Let me think what else could have been the problem?

-   10:18PM

    Ended up taking a nap, which turned into a whole sleep but damn these two build failures are going to drive me insane.
    I have officially found the next level of hate in software development.
    It works in developement but breaks in production, yet the errors do not make sense.
    One could argue that unit tests would solve this, but this be like higher level testing maybe?
    When building the game in the unity editor, there is no error that 

### StrudelCC

-   09:17AM

    While I am working on the game, I been just messing with this site, creating some crazy music on the side.
    Honestly, this is going to become way too much fun if I dont stop soon, but this would be perfect for chips website.
    Hell, I might even import it into the music player that I was building, so that I could play music ontop of the youtube music that we already have.
    A jukebox that not only plays music but gives the option to add custom soundtracks.
