...
bento:
    ...
    title: 'August 19,'
    accent: every year.
    lede: 'The August 19th daily log — notes, tasks, and quotes collected across every year of the KBVE journal.'
    ctas:
        - label: Read the log
          href: '#2026'
          primary: true
        ...
        - label: 'Aug 20 →'
          href: /journal/08-20/
    related:
        - title: 'Previous · Aug 18'
          copy: 'Step back a day to the August 18th log.'
          href: /journal/08-18/
          icon: calendar
        - title: 'Next · Aug 20'
          copy: 'Jump ahead to the August 20th log.'
          href: /journal/08-20/
          icon: arrow
        - title: 'Journal hub'
...
-   06:19PM
    ...
    The chat function for the slop needs to be updated, so that it can not only connect to the IRC but as I am playing, I can talk.
    With the bridge, we can then link any additional game into the gaint relay system.
...
-   05:04AM
    ...
    Today is the day that I finally step into the world of elixir!
    I saw that there is a rustler crate, which is a ffi between rust and elixir.

-   08:39AM

    Moved the `seed.exs` from out of the config file, now it will be called via the kustomization yaml.
    Next, the plan will be to test if it deploys without any issues, then we can focus on improving the seed file, including handling migrations.

### Arcane Rewatch

-   06:26AM

    Arcane is a TV show that really has a great rewatch ability, the two seasons were done perfectly.
    As I have said before, Riot is better at media than balancing their own game.
    You can say they are a media company, with shows and music being their best work, while using crack to fuel their ventures.
