...
bento:
    ...
    title: 'August 3,'
    accent: every year.
    lede: 'The August 3rd daily log — notes, tasks, and quotes collected across every year of the KBVE journal.'
    ctas:
        - label: Read the log
          href: '#2026'
          primary: true
        ...
        - label: 'Aug 4 →'
          href: /journal/08-04/
    related:
        - title: 'Previous · Aug 2'
          copy: 'Step back a day to the August 2nd log.'
          href: /journal/08-02/
          icon: calendar
        - title: 'Next · Aug 4'
          copy: 'Jump ahead to the August 4th log.'
          href: /journal/08-04/
          icon: arrow
        - title: 'Journal hub'
...
-   08:06AM
    ...
    The issue with the arcade is that there are design problems that are holding our html gaming environment back.
    One of the things that I am having an issue with is if we keep track of the different mdx for the games we are making, verse getting them published.
    Afterwards, I know that we will need to integrate the credits and khash system, but we should aim at getting them into the eco-system without having it give value back to the user.
    By value, we do not want to create any of these games that might be a cheating risk, where a player abuses it to give themselves unlimited khash or credits. 
...
-   11:00PM
    ...
    Spent most of the day learning more about nix and getting the kilobase aka supabase fork to be as operational as we can get it.
    This means not only getting it to deploy but also to get it to cluster and backup without throwing too many issues.
    A part of me does not like nix because I am very much used to just writing docker files the classic way, but honestly, it is growing on me.
    I am understanding why it exists and the purpose that it serves, because going back to an older project makes it a breeze.
