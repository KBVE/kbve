...
`--bob` is the Y-wobble amplitude (fraction of model size); `0.06` (default) bobbed too
hard. Current values: `idle` (hover) `0.03`, `move` (flying) **`0.02`** (calmer — the
flying ship should barely undulate). `--lift 0.6` = resting hover height.
...
uses `--bob 0.02` (+ `--sway 8` roll); `lift`/`bank` ignore `--bob` (lift ramps to
`--lift`, bank holds it).
