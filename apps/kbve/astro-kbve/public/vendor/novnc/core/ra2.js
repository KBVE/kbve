...
export default class RSAAESAuthenticationState extends EventTargetMixin {
	...
	async negotiateRA2neAuthAsync() {
		...
		if (serverKeyLength < 1024) {
			throw new Error(
				'RA2: server public key is too short: ' + serverKeyLength,
			);
		} else if (serverKeyLength > 8192) {
			throw new Error(
				'RA2: server public key is too long: ' + serverKeyLength,
			);
		}
		...
		const clientExportedRSAKey = await legacyCrypto.exportKey(
			'raw',
			clientRSACipher,
		);
		...
		clientSessionKey = await window.crypto.subtle.digest(
			'SHA-1',
			clientSessionKey,
		);
		clientSessionKey = new Uint8Array(clientSessionKey).slice(0, 16);
		serverSessionKey = await window.crypto.subtle.digest(
			'SHA-1',
			serverSessionKey,
		);
		...
		let serverHash = new Uint8Array(
			8 + serverKeyBytes * 2 + clientKeyBytes * 2,
		);
		let clientHash = new Uint8Array(
			8 + serverKeyBytes * 2 + clientKeyBytes * 2,
		);
		...
		let username;
		if (subtype === 1) {
			username = encodeUTF8(this._getCredentials().username).slice(
				0,
				255,
			);
		} else {
		...
		const password = encodeUTF8(this._getCredentials().password).slice(
			0,
			255,
		);
