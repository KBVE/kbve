...
export default class ZRLEDecoder {
	constructor() {
		...
		this._pixelBuffer = new Uint8Array(
			ZRLE_TILE_WIDTH * ZRLE_TILE_HEIGHT * 4,
		);
		this._tileBuffer = new Uint8Array(
			ZRLE_TILE_WIDTH * ZRLE_TILE_HEIGHT * 4,
		);
	}
		...
		for (let ty = y; ty < y + height; ty += ZRLE_TILE_HEIGHT) {
			...
			for (let tx = x; tx < x + width; tx += ZRLE_TILE_WIDTH) {
				...
				if (subencoding === 0) {
					...
					const data = this._decodePaletteTile(
						subencoding,
						tileSize,
						tw,
						th,
					...
					const data = this._decodeRLEPaletteTile(
						subencoding - 128,
						tileSize,
					);
					display.blitImage(tx, ty, tw, th, data, 0, false);
