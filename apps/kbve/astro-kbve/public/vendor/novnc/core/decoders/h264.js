...
export class H264Parser {
	...
	_parseNalUnit(index) {
		...
		if (firstByte & 0x80) {
			throw new Error(
				'H264 parsing sanity check failed, forbidden zero bit is set',
			);
		}
	...
	parse() {
		...
		while (this._index < this._data.length) {
			...
			const { slice, key } = this._parseNalUnit(
				this._index + startSequenceLen,
			);

			let nextIndex = this._indexOfNextNalUnit(
				this._index + startSequenceLen,
			);
			if (nextIndex == -1) {
...
export class H264Context {
	...
	_handleFrame(frame) {
		...
		if (pending === undefined) {
			throw new Error(
				'Pending frame queue empty when receiving frame from decoder',
			);
		}
	...
	decode(payload) {
		...
		while (true) {
			...
			if (
				this._decoder === null ||
				this._decoder.state !== 'configured'
			) {
				if (!encodedFrame.key) {
					Log.Warn(
						"Missing key frame. Can't decode until one arrives",
					);
					continue;
				}
				if (self._profileIdc === null) {
					Log.Warn(
						'Cannot config decoder. Have not received SPS and PPS yet.',
					);
					continue;
				}
...
export default class H264Decoder {
	...
	_getContext(x, y, width, height) {
		...
		return context !== undefined
			? context
			: this._createContext(x, y, width, height);
	}
