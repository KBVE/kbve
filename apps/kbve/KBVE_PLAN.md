...
#### Phase 1 — Scaffold (POC: `nx run astro-kbve:dev` boots clean) ✅ PR #7415

- [x] Create directory structure
- [x] Write project.json with all targets (dev, build, build:osrs, preview, check, sync, proto, proto:lint, proto:clean)
- [x] Write astro.config.mjs with full Starlight config, React, Tailwind, mermaid, site-graph (PWA deferred to Phase 5)
- [x] Write tsconfig.json with strict mode, react-jsx, `@/*`, `@kbve/astro`, `@kbve/droid` path aliases
- [x] Write tailwind.config.mjs with purple theme (content scan: `./src/**/*.{astro,html,js,jsx,md,mdx,svelte,ts,tsx,vue}`)
- [x] Write postcss.config.cjs (`@tailwindcss/postcss` + `cssnano`)
- [x] Create minimal `src/content/docs/index.mdx` homepage
- [x] Create `src/styles/global.css` with Tailwind layers, cyan/teal theme vars, view transitions
- [x] Create `src/content.config.ts` with Starlight `docsLoader`/`docsSchema` (required for Starlight 0.37+)
- [x] Create Starlight component override stubs (SiteTitle, PageSidebar, Footer)
- [x] Write `buf.gen.yaml` with ts-proto plugin config
- [x] Verify dev server starts and renders Starlight landing page

#### Phase 2 — Proto Integration (POC: `nx run astro-kbve:proto` generates valid TypeScript) ✅ PR #7416

- [x] Write `buf.gen.yaml` with ts-proto plugin config (done in Phase 1)
- [x] Add proto, proto:lint, proto:clean targets to project.json (done in Phase 1)
- [x] Run `buf generate`, verify `src/generated/proto/*.ts` output compiles
- [x] Create `src/schemas/index.ts` barrel export
- [x] Fix buf.gen.yaml output path (must be relative to workspace root, not template)
- [x] Fix proto target command (removed `--path kbve/` flag)
- [x] Add `@bufbuild/protobuf` runtime dependency
- [x] Add `src/generated/.gitignore` to exclude generated proto files

#### Phase 3 — Content Migration (POC: guide pages render with navigation) ✅ PR #7416

- [x] Port `content.config.ts` with all collection definitions + `validateItemUniqueness()`
- [x] Port `src/data/` schema directory (IObjectSchema, IQuestSchema, IMapSchema, ICraftingSchema, IBonusSchema, IDeployableSchema, IScriptBindingSchema)
- [x] Port `src/data/types/` (ItemCategoryTypes, BonusTypes, QuestTypes, etc.)
- [x] Port `src/data/schema/osrs/` (IOSRSSchema — 869 lines of equipment, drops, shops, recipes, potions, food, cooking, treasure trails)
- [x] Migrate `src/content/docs/guides/` as first content batch (intro, getting-started, first-project-checklist)
...
#### Phase 1 — Scaffold (POC: `nx run astro-kbve-e2e:e2e` runs and passes) ✅ PR #7415

- [x] Create project.json with e2e, e2e:preview, e2e:docker targets
- [x] Write playwright.config.ts (dev), playwright.preview.config.ts, playwright.docker.config.ts
