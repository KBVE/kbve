...
describe('Discordsh — Smoke Tests', () => {
	...
	it('should return 401 when no auth header is provided', async () => {
		const res = await fetch(`${BASE_URL}/discordsh`, {
			...
			body: JSON.stringify({ command: 'vote.cast' }),
		});
	...
	it('should return 400 for bad command format (no dot)', async () => {
		const res = await fetch(`${BASE_URL}/discordsh`, {
			method: 'POST',
			headers: headers(),
			body: JSON.stringify({ command: 'cast' }),
		});
		expect(res.status).toBe(400);
		const body = await res.json();
	...
	it('should route vote.cast past the router', async () => {
		const res = await fetch(`${BASE_URL}/discordsh`, {
			method: 'POST',
			headers: headers(),
			body: JSON.stringify({ command: 'vote.cast' }),
	...
	it('should route server.submit past the router', async () => {
		const res = await fetch(`${BASE_URL}/discordsh`, {
			method: 'POST',
			headers: headers(),
			body: JSON.stringify({ command: 'server.submit' }),
		});
		expect(res.status).not.toBe(401);
		expect(res.status).not.toBe(405);
	});
