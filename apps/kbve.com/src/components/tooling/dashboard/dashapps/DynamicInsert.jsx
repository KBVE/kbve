...
import { kilobase, $profileStore } from '@kbve/laser';
...
const ContentBlock = () => {
  ...
  const $profile$ = useStore($profileStore);
	...
	//let username = kilobase.getUsername();
	useEffect(() => {
	 	const fetchProfile = async () => {
	 		const userProfile = await kilobase.getProfile();
			...
			<div className="flex flex-col justify-between p-6 space-y-4 bg-gray-300 dark:bg-gray-900/[.80] dark:text-gray-100">
				<div className="space-y-2">
					...
					<div className="dark:text-gray-100">
						Welcome { $profile$.username || 'Guest'} <br />
						Status: {plugin ? plugin.state : 'unknown'} <br />
						Message:{' '}
						{$atlas$.plugin?.[1]
							? $atlas$.plugin?.[1].message
							: 'No Message.'}{' '}
						<br />
						Error:{' '}
						{$atlas$.plugin?.[1]
							? $atlas$.plugin?.[1].error
							: 'No Error.'}{' '}
						<br />
					</div>
				</div>
...
const handleButtonClick = async () => {
	...
	try {
		...
		await updatePluginError(
			1,
			`Error with starting RuneLite Plugin ${error}`,
		);
