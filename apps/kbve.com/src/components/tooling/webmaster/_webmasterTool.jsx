...
const WebmasterTool = () => {
    ...
    const openSearch = (searchType) => {
        ...
        const searchUrl = searchUrls[searchType].replace('{url}', encodeURIComponent(url));
                ...
                <div className="p-1.5 min-w-full inline-block align-middle">
                    <div className="overflow-hidden">
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                            <thead>
                                <tr>
                                    <th scope="col" className="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase dark:text-neutral-500">Operation</th>
                            ...
                            <tbody className="divide-y divide-gray-200 dark:divide-neutral-700">
                                {operations.map((operation) => (
                                    <tr key={operation.type} className="hover:bg-gray-100 dark:hover:bg-neutral-700">
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200">{operation.name}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                            <button
                                                type="button"
                                                className="inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent text-blue-600 hover:text-blue-800 disabled:opacity-50 disabled:pointer-events-none dark:text-blue-500 dark:hover:text-blue-400"
                                                onClick={() => openSearch(operation.type)}
                                            >
                                                Run
                                            </button>
                                        </td>
