...
import { Aside, Steps, Card, CardGrid, Code } from '@astrojs/starlight/components';
...
After installing, verify your Bun installation by running the command below in your terminal:
```shell
bun --version
```
This command will display the installed version of Bun, confirming a successful installation.
</Aside>
...
To utilize Bun within Docker, use the **oven/bun** image. 
Execute Bun commands using the following single command: 
```shell
docker run --rm oven/bun --init --ulimit memlock=-1:-1 --$command
```
</Aside>
