...
The curl command that we will be using to hit the `rust_api_profile` docker image: `for i in {1..100000}; do curl -s -o /dev/null http://localhost:3000/api/v1/speed & done
wait`.
