#!/usr/bin/env bash
# seal-itch-api.sh — Seal the itch.io server-side API key into Secret chuckrpg-secrets.
#
...
#   # Output: apps/kube/chuckrpg/manifest/chuckrpg-secrets-sealedsecret.yaml

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"
OUTPUT_FILE="${SCRIPT_DIR}/manifest/chuckrpg-secrets-sealedsecret.yaml"
...
printf '%s' "${ITCH_API}" \
| kubectl create secret generic chuckrpg-secrets \
    --namespace=chuckrpg \
