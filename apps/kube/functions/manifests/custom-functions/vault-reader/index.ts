	...
	try {
		...
		if (command === 'get_by_guild') {
			...
			if (!server_id || !service) {
				return new Response(
					JSON.stringify({
						error: 'server_id and service are required for get_by_guild command',
					}),
					{
						status: 400,
						headers: {
			...
			if (error) {
				console.error('Error fetching guild token via RPC:', error);
				return new Response(JSON.stringify({ error: error.message }), {
					status: 500,
					headers: {
						...corsHeaders,
		...
		if (command === 'get_by_tag') {
			...
			if (error) {
				console.error('Error fetching guild token via tag RPC:', error);
				return new Response(JSON.stringify({ error: error.message }), {
					status: 500,
					headers: {
						...corsHeaders,
		...
		return new Response(
			JSON.stringify({
				error: 'Invalid command. Use "get", "set", "get_by_guild", or "get_by_tag"',
			}),
			{
				status: 400,
				headers: { ...corsHeaders, 'Content-Type': 'application/json' },
