...
// Purple/pink/blue aurora tuned to the dashboard accent palette — slow,
// low-contrast bands meant to sit behind the dashboard at low opacity.
...
`,
  ...
  let col =
      a1 * vec3f(0.65, 0.55, 0.98)
    + a2 * vec3f(0.91, 0.47, 0.98)
    + a3 * vec3f(0.38, 0.65, 0.98);
  let alpha = clamp(a1 + a2 + a3, 0.0, 0.55);
