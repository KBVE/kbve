...
// Capability bitmask (mirrors jobboard.member_applications.requested_capabilities).
const CAP_TAKER: i32 = 1;
const CAP_POSTER: i32 = 2;
    ...
    let row = tx
        .query_one(
            "INSERT INTO jobboard.member_applications
                 (user_id, requested_capabilities, statement, portfolio_links)
             VALUES ($1, $2, $3, $4)
             RETURNING id, status, created_at::text",
            &[&user.user_id, &caps, &body.statement, &body.portfolio_links],
        )
        .await
        .map_err(|e| {
...
async fn admin_decide(
    ...
    let new_status: i32 = if body.approve {
        ...
        if granted & CAP_TAKER != 0 {
            tx.execute(
                "INSERT INTO jobboard.talent_profiles (user_id) VALUES ($1) ON CONFLICT DO NOTHING",
                &[&applicant],
            )
            .await
            .map_err(pg_err)?;
