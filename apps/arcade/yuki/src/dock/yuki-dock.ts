 ...
 *
 * The dock itself is static HTML (see `index.html`). This file boots
 ...
 *
 * Idempotent: safe to call `initYukiDock()` multiple times. The bind
 * flag short-circuits when we are already wired against this exact
 * node, so a second call does not double-bind keyboard listeners.
 */
