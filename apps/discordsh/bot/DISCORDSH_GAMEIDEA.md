...
Profiles survive restarts: class, level, XP, gold, gear, inventory, completed
quests and lifetime counters (kills, gold, rooms, bosses, deaths, victories,
escapes). The cache hierarchy is in-memory LRU -> optional redb L2 (`bevy_db`)
-> Supabase RPC, each layer degrading on its own — with Supabase offline the bot
still serves cached profiles across pod restarts.

Live run state is separate and in-memory: a DashMap of `Arc<Mutex<SessionState>>`.

## Architecture

Rules and content live in the `bevy_dungeon` crate; `apps/discordsh/bot/src/discord/game/`
is the Discord shell over it, so `types.rs`, `content.rs` and `logic.rs` here are
thin re-exports.

- `bevy_dungeon` — game types, item/gear/enemy registries, loot and XP tables,
  combat resolution, effect ticking, room advancement
- `types.rs` — re-export plus the Discord snowflake <-> `PlayerId` mapping
- `render.rs` — Discord embed and component builders
- `card.rs` + `templates/game/card.svg` — Askama SVG template -> PNG game card
- `router.rs` — component interaction routing (`dng|<sid>|<action>|<arg>`)
- `session.rs` — DashMap-backed live session store
- `persistence.rs` — `ProfileStore`, the cached Supabase-backed profile layer
- `commands/dungeon.rs` — the `/dungeon` slash command (start, join, leave, status, end)

### Stack

- **Rust** — poise 0.6.1 over serenity 0.12.5
- **Askama** SVG templates rendered to PNG with resvg
- **Custom ID format** — `dng|<short_sid>|<action>|<arg>`, kept under Discord's
  100-char limit
