...
class VaultManager:
    ...
    async def get_vault_secret(self, secret_id: str) -> VaultOperationResponse:
        ...
        try:
            ...
            result = client.functions.invoke("vault-reader", invoke_options={"body": request.model_dump(mode="json")})
            ...
            if result:
                ...
                if secret_data:
                    if "error" in secret_data:
                        return VaultOperationResponse(success=False, error=secret_data["error"])

                    secret = VaultSecretResponse(**secret_data)
                    return VaultOperationResponse(success=True, data=secret.model_dump())

            return VaultOperationResponse(success=False, error="No data returned from vault")

        except Exception as e:
            logger.error(f"Error fetching vault secret: {e}")
            return VaultOperationResponse(success=False, error=str(e))

    async def set_vault_secret(
        self, secret_name: str, secret_value: str, secret_description: Optional[str] = None
        ...
        try:
            ...
            request = VaultSetRequest(
                secret_name=secret_name, secret_value=secret_value, secret_description=secret_description
            )
            ...
            if result:
                ...
                if response_data:
                    if "error" in response_data:
                        return VaultOperationResponse(success=False, error=response_data["error"])

                    return VaultOperationResponse(success=True, data=response_data)
