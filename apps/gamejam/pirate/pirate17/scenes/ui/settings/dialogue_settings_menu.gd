class_name DialogueSettingsMenu
extends Control
...
func _ready():
	...
	# Ensure the settings menu can receive input
	...
	# Store original pause state and set process mode first
	...
	# Try to connect to dialogue system but don't fail if it's not available
	...
	if dialogue_system:
		...
		print("SettingsMenu: DialogueSystem not found - running in standalone mode")
...
func _on_close_pressed():
	if dialogue_system and dialogue_system.has_method("is_active") and dialogue_system.is_active:
