...
namespace RareIcon
{
    ...
    public partial struct HarvestSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            if (!SystemAPI.TryGetSingleton<DietPreferencesSingleton>(out var diet)) return;
    ...
    public partial struct HarvestJob : IJobEntity
    {
        ...
        [ReadOnly] public DietPreferencesSingleton    Diet;
            ...
            switch ((HarvestRole)bestRole)
            {
                case HarvestRole.Forager:
                    ...
                    harvested = TryTakeCactus(ref res, movement.WanderStep,
                                              ...
                                              unitType, inventory, bags, ItemDb, Diet)
                             || TryTakeResource(ref res.Mushrooms, ResourceTag.Mushrooms, unitType, inventory, bags, ItemDb, Diet)
                             || TryTakeResource(ref res.Berries,   ResourceTag.Berries,   unitType, inventory, bags, ItemDb, Diet)
                             || TryTakeResource(ref res.Herbs,     ResourceTag.Herbs,     unitType, inventory, bags, ItemDb, Diet);
                    break;
                case HarvestRole.Lumberjack:
                    ...
                    bool gotWood     = TryTakeResource(ref res.Wood,     ResourceTag.Wood,     unitType, inventory, bags, ItemDb, Diet);
                    bool gotLeaves   = rollLeaves   < 0.6f && TryTakeResource(ref res.Leaves,   ResourceTag.Leaves,   unitType, inventory, bags, ItemDb, Diet);
                    bool gotBranches = rollBranches < 0.4f && TryTakeResource(ref res.Branches, ResourceTag.Branches, unitType, inventory, bags, ItemDb, Diet);
                    harvested = gotWood | gotLeaves | gotBranches;
                    break;
                case HarvestRole.Miner:
                    xpKind = SkillKind.Mining;
                    harvested = TryTakeResource(ref res.Stone,     ResourceTag.Stone,     unitType, inventory, bags, ItemDb, Diet);
                    break;
            }
        ...
        static bool TryTakeResource(ref byte amount, byte resourceTag, byte unitType,
                                    ...
                                    in DietPreferencesSingleton diet)
        {
            if (amount == 0) return false;
            ushort itemId = ResourceItemMap.ItemForResource(resourceTag);
            if (itemId == 0) return false;
            if (itemDb.GetHarvestRole(itemId) == (byte)HarvestRole.None) return false;
            if (diet.Get(unitType, itemId) == 0) return false;

            ushort take = amount < HarvestPerTick ? amount : HarvestPerTick;
            ushort added = inventory.AddItemCapped(bags, itemDb, itemId, take);
                                  ...
                                  in DietPreferencesSingleton diet)
        {
            ...
            if (diet.Get(unitType, (ushort)ItemId.RawCacti) == 0) return false;

            ushort gotCacti = inventory.AddItemCapped(bags, itemDb, (ushort)ItemId.RawCacti, 1);
            ...
            {
                if (d0 < 0.75f && diet.Get(unitType, (ushort)ItemId.CactiNeedle) > 0) inventory.AddItemCapped(bags, itemDb, (ushort)ItemId.CactiNeedle, 1);
                if (d1 < 0.90f && diet.Get(unitType, (ushort)ItemId.PricklyPear) > 0) inventory.AddItemCapped(bags, itemDb, (ushort)ItemId.PricklyPear, 1);
                if (d2 < 0.15f && diet.Get(unitType, (ushort)ItemId.CactiSeeds)  > 0) inventory.AddItemCapped(bags, itemDb, (ushort)ItemId.CactiSeeds,  1);
            }
            else
            {
                if (d0 < 0.50f && diet.Get(unitType, (ushort)ItemId.CactiNeedle) > 0) inventory.AddItemCapped(bags, itemDb, (ushort)ItemId.CactiNeedle, 1);
                if (d1 < 0.60f && diet.Get(unitType, (ushort)ItemId.Dragonfruit) > 0) inventory.AddItemCapped(bags, itemDb, (ushort)ItemId.Dragonfruit, 1);
                if (d2 < 0.10f && diet.Get(unitType, (ushort)ItemId.CactiSeeds)  > 0) inventory.AddItemCapped(bags, itemDb, (ushort)ItemId.CactiSeeds,  1);
            }
            return true;
        }
    }
