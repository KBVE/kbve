...
namespace RareIcon
{
    /// <summary>Authoritative logistics state: committed balances, in-flight reservations, per-frame deliveries + pending deltas, per-unit pack deliveries. PipelineHandle is the serialized handle across all pipeline phases; each system combines it into state.Dependency before scheduling and stores its new handle back.</summary>
