// Askama-rendered wrappers for dynamic Astro-style pages. Exposed for routes
// that want to wrap HTML content inside the shared layout (e.g. auth views,
// server-rendered error pages). Currently scaffolded; Astro's own 404.html is
// what `build_static_router` serves on not-found.
#![allow(dead_code)]
