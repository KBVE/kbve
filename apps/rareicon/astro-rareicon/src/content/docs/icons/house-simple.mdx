---
ref: house-simple
name: House Simple
title: House Simple Icon
description: House Simple glyph from the Phosphor Icons library (MIT), 1 weight — recolor via currentColor.
primary_category: navigation
categories: [navigation, library]
tags: [phosphor-generated, multi-weight]
search:
  keywords: ["house-simple","house","simple","house simple"]
  primary_category: navigation
default_license:
  license: mit
  attribution_required: false
  attribution_line: "Phosphor Icons (https://phosphoricons.com/) — MIT License"
  author: Phosphor contributors
  source_url: "https://phosphoricons.com/?q=house-simple"
  commercial_use: true
  modifications_allowed: true
default_offering:
  offering: free
icons:
  - ref: "regular"
    label: "Regular"
    style: outline
    format: svg
    viewbox: { min_x: 0, min_y: 0, width: 256, height: 256 }
    render:
      uses_current_color: true
      monochrome: true
    recommended_sizes: [16, 20, 24, 32]
    tags: [phosphor, regular]
    svg_body: |
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" fill="currentColor"><path d="M219.31,108.68l-80-80a16,16,0,0,0-22.62,0l-80,80A15.87,15.87,0,0,0,32,120v96a8,8,0,0,0,8,8H216a8,8,0,0,0,8-8V120A15.87,15.87,0,0,0,219.31,108.68ZM208,208H48V120l80-80,80,80Z"/></svg>
pagefindFilters: [phosphor, navigation]
---

import IconTermPanel from '@/components/icons/IconTermPanel.astro';

House Simple — 1 weight from the [Phosphor Icons library](https://phosphoricons.com/) (MIT).

<IconTermPanel
	termRef={frontmatter.ref}
	name={frontmatter.name}
	description={frontmatter.description}
	primary_category={frontmatter.primary_category}
	categories={frontmatter.categories}
	tags={frontmatter.tags}
	default_license={frontmatter.default_license}
	icons={frontmatter.icons}
/>
