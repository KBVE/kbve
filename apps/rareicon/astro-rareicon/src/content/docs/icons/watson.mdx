---
ref: watson
name: Watson
title: Watson Icon
description: Watson glyph from the carbon icon library, 1 variant — recolor via currentColor.
primary_category: tech
categories: [tech, library]
tags: [carbon-generated, carbon]
search:
  keywords: ["watson"]
  primary_category: tech
default_license:
  license: apache_2
  attribution_required: false
  attribution_line: "Carbon Icons (https://carbondesignsystem.com/guidelines/icons/library/) — Apache 2.0"
  author: IBM Corp.
  source_url: "https://carbondesignsystem.com/guidelines/icons/library/?icon=watson"
  commercial_use: true
  modifications_allowed: true
default_offering:
  offering: free
icons:
  - ref: "carbon"
    label: "Carbon"
    style: outline
    format: svg
    viewbox: { min_x: 0, min_y: 0, width: 32, height: 32 }
    render:
      uses_current_color: true
      monochrome: true
    recommended_sizes: [16, 20, 24, 32]
    tags: [carbon]
    svg_body: |
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" fill="currentColor"><path fill="currentColor" d="M21.74 9.49A11.4 11.4 0 0 0 16 8a.76.76 0 1 0 0 1.51a10 10 0 0 1 1.91.21c-2.26 1.08-4.76 3.58-6.73 7a22.5 22.5 0 0 0-2 4.44A9.6 9.6 0 0 1 7 17.22a3.43 3.43 0 0 1 .28-2.66c.79-1.37 2.44-2.15 4.63-2.2a.76.76 0 0 0 .74-.78a.75.75 0 0 0-.78-.74c-2.68.04-4.77 1.08-5.87 2.9a4.84 4.84 0 0 0-.44 3.79a12 12 0 0 0 3.2 5.22A11.4 11.4 0 0 0 8.52 26a10 10 0 0 1-2-3.48a.75.75 0 0 0-.95-.52a.76.76 0 0 0-.49 1a11.45 11.45 0 0 0 5.18 6.38A11.4 11.4 0 0 0 16 30.92a11.7 11.7 0 0 0 3-.39a11.48 11.48 0 0 0 2.77-21Zm-3.16 19.57a9.9 9.9 0 0 1-7.56-1c-.86-.49-1.21-2-.94-4a19 19 0 0 0 2.48 1.72a13.9 13.9 0 0 0 6.93 2a11 11 0 0 0 2.42-.28a9.8 9.8 0 0 1-3.33 1.56m6.06-4.66c-2 2-6.66 2.74-11.32.05a17.4 17.4 0 0 1-2.89-2.12a21 21 0 0 1 2.08-4.91c2.94-5.08 6.83-7.57 8.47-6.62a10 10 0 0 1 3.66 13.6M4.16 11.72L1.14 10a.76.76 0 1 0-.76 1.31L3.4 13a.86.86 0 0 0 .38.1a.77.77 0 0 0 .66-.38a.76.76 0 0 0-.28-1m4.13-4.13a.74.74 0 0 0 .65.41a.75.75 0 0 0 .38-.1a.76.76 0 0 0 .28-1l-1.74-3a.76.76 0 0 0-1-.27a.75.75 0 0 0-.28 1ZM16 6.08a.76.76 0 0 0 .76-.76V1.83a.76.76 0 0 0-1.52 0v3.49a.76.76 0 0 0 .76.76m6.68 1.79a.75.75 0 0 0 1-.28l1.75-3a.75.75 0 0 0-.28-1a.76.76 0 0 0-1 .27l-1.74 3a.76.76 0 0 0 .27 1.01m9.22 2.38a.76.76 0 0 0-1-.27l-3 1.74a.76.76 0 0 0-.28 1a.77.77 0 0 0 .66.38a.86.86 0 0 0 .38-.1l3-1.75a.76.76 0 0 0 .24-1"/></svg>
pagefindFilters: [carbon, tech]
---

import IconTermPanel from '@/components/icons/IconTermPanel.astro';

Watson — glyph from the [Carbon icon library](https://carbondesignsystem.com/guidelines/icons/library/).

<IconTermPanel
	termRef={frontmatter.ref}
	name={frontmatter.name}
	description={frontmatter.description}
	primary_category={frontmatter.primary_category}
	categories={frontmatter.categories}
	tags={frontmatter.tags}
	icons={frontmatter.icons}
/>
