...
public class BBCube extends BBObject implements BBFaceContainer {
    ...
    public BBCube(JsonObject element, BBModel model) {
        ...
        this.from = BBModelUtils.parseVector(element, "from");
        this.to = BBModelUtils.parseVector(element, "to");

        this.inflate = BBModelUtils.getIntElement(element, "inflate");
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (!BBModelUtils.isNull(faceObject, "texture")) {
                int id = BBModelUtils.getIntElement(faceObject, "texture");
                ...
                int rot = BBModelUtils.getIntElement(faceObject, "rotation");
                while (rot > 0) {
                    roll(u, i);
                    roll(v, i);
                    rot -= 90;
