  ...
  let mut recv_task = tokio::spawn(async move {
    while let Some(Ok(msg)) = socket_rx.next().await {
      if let Message::Text(text) = msg {
        let parsed_msg = parse_ws_command(&text).or_else(|_| {
          match serde_json::from_str::<jedi::wrapper::ThinWsCommand>(&text) {
            Ok(thin) => Ok(jedi::wrapper::ThinWsCommand::into(thin)),
            Err(e) => Err(e),
          }
        });
        ...
        match parsed_msg {
          Ok(ws_msg) => {
            if let Some(key) = extract_watch_command_key(&ws_msg) {
              ...
              match state_recv.temple.send_redis(cmd).await {
                Ok(response) => {
                ...
                Err(e) => {
                  tracing::warn!("WebSocket Redis error: {}", e);
