...
const CanvasManager = {
    ...
    bindCanvas(panelId: string, canvas: OffscreenCanvas, mode: CanvasDrawMode = 'animated') {
...
self.addEventListener('message', (event) => {
	const { type, panelId, canvas, mode } = event.data;

	switch (type) {
		case 'bindCanvas':
			if (canvas instanceof OffscreenCanvas) {
				CanvasManager.bindCanvas(panelId, canvas, mode ?? 'animated');
			} else {
				console.error('[CanvasWorker] Invalid canvas received for binding.');
			}
			break;

		case 'unbindCanvas':
			CanvasManager.unbindCanvas(panelId);
			break;

		default:
			console.warn(`[CanvasWorker] Unknown message type: ${type}`);
	}
});
