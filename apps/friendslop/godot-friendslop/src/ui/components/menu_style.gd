...
## The project draws against a 1280x720 viewport with `canvas_items` stretch, so without
## scaling a phone renders the menus at roughly one device pixel per unit and they read as
## unusably small. Every menu sizes itself against this height.
const LAYOUT_HEIGHT := 720.0
const SCALE_RANGE := Vector2(0.8, 1.7)
## A touchscreen is held closer and poked with a finger, so it gets a floor no desktop
## window needs; below this the controls stop being reliable targets.
...
## How much larger than the 1280x720 design a menu should draw itself.
...
## Margin, in viewport units, that anything pinned to an edge must keep clear.
##
## A phone reports the rectangle left over once the notch, status bar and home indicator
## are taken out. That rectangle is in screen pixels while the UI is laid out in viewport
## units, so it is converted through the ratio between the two rather than used directly.
## Desktop reports the whole window and this comes back as zero on every side.
