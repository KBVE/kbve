...
const FLAGS: Array[Dictionary] = [
	{"key": KEY_2, "node": ^"Terrain", "prop": "wake_enabled", "label": "wake"},
]
...
func _ready() -> void:
	...
	if player:
		if disable_ink_lines:
			var ink := player.get_node_or_null(^"Pivot/Camera3D/InkLines")
			if ink:
				ink.visible = false
		if unshade_player:
			_unshade(player)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	for layer in LAYERS:
		if key.keycode == layer["key"]:
			var node := get_node_or_null(layer["node"]) as Node3D
			if node:
				node.visible = not node.visible
				node.process_mode = (
					Node.PROCESS_MODE_INHERIT if node.visible else Node.PROCESS_MODE_DISABLED
				)
				_sync_flat_ground()
				_report()
			return
