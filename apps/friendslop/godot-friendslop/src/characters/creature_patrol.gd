...
## Gap at which it is running flat out.
@export var sprint_distance := 14.0
## Has to beat the leader's top speed by a fair margin, not match it.
...
## Works toward its slot, but treats being near the leader as good enough whenever the
## leader is not going anywhere.
...
## Ranks behind the leader, laid out on the leader's own axes so the formation turns
## with it.
func _slot() -> Vector3:
...
## Sum of the pushes away from every neighbour that is too close, falling off to nothing
## at the separation radius.
