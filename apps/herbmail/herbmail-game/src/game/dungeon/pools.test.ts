...
import { FLOOR, PIT, POOL } from '../geometry/grid';
...
describe('pool generation', () => {
	it('is deterministic and carves exactly the slot rect', () => {
		...
		expect(again.pools).toEqual(d.pools);

		let pit = 0;
		for (let i = 0; i < d.tiles.length; i++) if (d.tiles[i] & PIT) pit++;
		const expected = d.pools.reduce((s, p) => s + p.w * p.h, 0);
		expect(pit).toBe(expected);
		for (const p of d.pools)
			for (let r = p.row; r < p.row + p.h; r++)
				for (let c = p.col; c < p.col + p.w; c++)
					expect(d.tiles[r * d.cols + c]).toBe(POOL);
	});
	...
	it('keeps a walkable floor ring around every basin', () => {
		...
		for (const p of d.pools) {
			for (let c = p.col - 1; c <= p.col + p.w; c++) {
				expect(d.tiles[(p.row - 1) * d.cols + c]).toBe(FLOOR);
				expect(d.tiles[(p.row + p.h) * d.cols + c]).toBe(FLOOR);
			}
			for (let r = p.row - 1; r <= p.row + p.h; r++) {
				expect(d.tiles[r * d.cols + p.col - 1]).toBe(FLOOR);
				expect(d.tiles[r * d.cols + p.col + p.w]).toBe(FLOOR);
			}
