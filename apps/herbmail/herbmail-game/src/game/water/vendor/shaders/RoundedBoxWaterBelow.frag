...
void main() {
  ...
  // Translucent from below too: the swimmer's own body and the world above
  // the surface stay visible through the water sheet.
  float through = (1.0 - fresnel) * length(refractedRay);
  gl_FragColor = vec4(
    mix(reflectedColor, refractedColor, through),
    mix(0.75, 0.4, through)
  );
}
