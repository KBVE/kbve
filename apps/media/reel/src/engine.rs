...
impl Engine {
    ...
    fn spawn_completion_watcher(
        ...
        tokio::spawn(async move {
            ...
            // dropped from the session further down, but the move and the
            // stream drain in between can take minutes — we don't seed through
            // them. Raw streaming reads the file directly, so a paused torrent
            // never interrupts a viewer.
