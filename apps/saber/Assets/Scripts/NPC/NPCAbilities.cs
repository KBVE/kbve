...
private void InitializeCooldowns()
{
    ...
    public void CastSpell(string spellName, GameObject target)
    {
        ...
        if (spellToCast == null)
          ...
          {
              Debug.LogWarning($"Not enough mana to cast spell {spellName}. Current mana: {currentMana}, required: {spellToCast.manaCost}");
              return;
          }

          if (spellCooldowns[spellToCast] > 0)
          {
              Debug.LogWarning($"Spell {spellName} is on cooldown. Time left: {spellCooldowns[spellToCast]} seconds");
              return;
          }

        if (spellToCast != null && currentMana >= spellToCast.manaCost && spellCooldowns[spellToCast] <= 0)
        {
