...
bento:
    ...
    title: 'August 8,'
    accent: every year.
    lede: 'The August 8th daily log — notes, tasks, and quotes collected across every year of the KBVE journal.'
    ctas:
        - label: Read the log
          href: '#2026'
          primary: true
        ...
        - label: 'Aug 9 →'
          href: /journal/08-09/
    related:
        - title: 'Previous · Aug 7'
          copy: 'Step back a day to the August 7th log.'
          href: /journal/08-07/
          icon: calendar
        - title: 'Next · Aug 9'
          copy: 'Jump ahead to the August 9th log.'
          href: /journal/08-09/
          icon: arrow
        - title: 'Journal hub'
...
-   06:02AM
    ...
    The KBVE Desktop needs tmux improvements but I think with the correct tokio and actor system, we should be able to handle it.
    I updated the songbird api to support DAVE encoding, now I am getting all of yuki hooked in!
    The goal will be to make Yuki a solid bot aka application that the person can run without any major issues.

-   10:27AM

    Making some great progress with the kbve desktop application, as fudster suggested, it be basically an ADE.
    The agentic development environment would be the main goal for today, thus making it easier to call codex, claude and anything else within the eco system.
    Okay we need to figure out the processing of the LLMs next in the flow.

-   09:19PM

    Added better sidecar support for LLMs and now doing some improvements to the text to speech aka tts.
    The plan is to have both llama cpp support but also mlx support via c ffi or any stable sidecar library to help us.
    Each sidecar should not crash the whole application, but still provide solid multi threading support.

### Obsession

-   09:00PM

    Going to test out the reel update with a new media file, hopefully we are able to get this resolved.
    This was an amazing movie, definitely one of the best this year and what is amazing is that it was extremely low budget.
...
-   11:50PM
    ...
    End of the night notes!
    Most of the day was just updating the nginx and ingress, getting back into that flow since we moved away from traefik again.
    The cluster finally got the external secrets operator, which was something that we needed in our previous RKE2 instance!
    There are two or three more operators that I want to add int our cluster, but that will come at a later date.    
