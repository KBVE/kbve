...
bento:
    ...
    title: 'August 10,'
    accent: every year.
    lede: 'The August 10th daily log — notes, tasks, and quotes collected across every year of the KBVE journal.'
    ctas:
        - label: Read the log
          href: '#2026'
          primary: true
        ...
        - label: 'Aug 11 →'
          href: /journal/08-11/
    related:
        - title: 'Previous · Aug 9'
          copy: 'Step back a day to the August 9th log.'
          href: /journal/08-09/
          icon: calendar
        - title: 'Next · Aug 11'
          copy: 'Jump ahead to the August 11th log.'
          href: /journal/08-11/
          icon: arrow
        - title: 'Journal hub'
...
-   07:02PM
    ...
    The ravens do not have any clipping, so I am thinking that we would use an off thread to handle the flow fields.
    Our goal might be able to include a large instance of shader based enemies that could spawn into the game.
    So for pathfinding, we could do a combination of A* pathfinding with the kd tree  and then implement flow fields ontop?

### FriendSlop v1 Off Thread

-   09:01PM

    Moving all off the main thread, we are getting closer to our goal of the main thread just handling the rendering.
    I even wonder if we could optimize the GLSL calls that could move off the if branching, giving us even better performance.
...
-   10:00PM
    ...
    Wrapping up another unity session, this time the focus is on the supabase integration!
    The goal for this month will be to link the supabase with the unity client, giving us a better connection between the players and our server.
    Granted we are making an offline / single player game, I just wanted to make sure that we had the multiplayer connection ready so that as we build around the single player, we can extend it.
