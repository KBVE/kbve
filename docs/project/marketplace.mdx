...
next: false
bento:
    badge: gamedev · gaming · rust · proto · assets
    title: Open catalog for game
    accent: Assets
    lede: >-
        An open marketplace for game assets — think Fab / itch.io / CraftPix /
        GameDevMarket, scoped strictly to game assets: 3D models, 2D art, audio,
        VFX, shaders, tools/plugins, and complete projects. A decoupled Rust/Axum
        API sharing KBVE identity, media, transport, and vocabulary with the Job
        Board.
    ctas:
        - label: Architecture
          href: "#architecture"
        ...
        - label: Licensing
          href: "#licensing"
        - label: Preview gating
          href: "#preview-gating"
        - label: Protobuf
          href: "#protobuf"
    aside:
        icon: shield
        title: Private source, public previews
        body: >-
            The buyer never sees or receives the real asset before they're
            entitled to it. Listings show derived, watermarked, degraded previews
            generated server-side; source files stay private and grant-gated.
        points:
            - <strong>Game-native</strong> — engine/format facets, not generic categories.
            - <strong>Shared identity</strong> — one KBVE account across both products.
    stats:
        - { label: Status, value: Spec / arch, icon: book }
        - { label: Stack, value: Rust + Axum, icon: rust }
        - { label: Storage, value: Cloudflare R2, icon: database }
        - { label: Wire, value: protobuf, icon: network }
    features:
        - title: Game-native catalog
          icon: box
          body: Categories and facets speak game dev — engine version, render pipeline, tris/verts, humanoid rig, atlas size, sample rate — not generic graphics/audio.
        - title: Open supply, curated trust
          icon: shield
          body: Anyone can list (volume), but verification badges, ratings, and moderation surface quality. Trust is earned and displayed, not gatekept at the door.
        - title: Shared ecosystem
          icon: expand
          body: One KBVE account, one identity, one engine/tool vocabulary across the Job Board and the Asset Marketplace — a freelancer can take gigs and sell assets.
        - title: Preview protection
          icon: lock
          body: Source files live in private R2 with no public URL, served only via short-lived signed URLs gated on a LicenseGrant; the public only ever sees watermarked derivatives.
    related:
        - title: Job Board
          href: /project/jobboard/
          copy: The sibling product that sells services — shares identity, media, transport, and vocabulary.
          icon: server
...
<BentoProse id="overview" eyebrow="Status & positioning" heading="Asset Marketplace">

> Status: **Spec / architecture** — no app code yet. This is the document to review/edit before building.
> Sibling product to the [Job Board](/project/jobboard/) — the Job Board sells *services*, the Asset Marketplace sells *digital goods*. They share identity, media, transport, and technical vocabulary.
...
<BentoProse id="taxonomy" eyebrow="Vocabulary" heading="Asset taxonomy">
...
<BentoProse id="licensing" eyebrow="Terms" heading="Licensing">
...
<BentoProse id="revenue" eyebrow="Model" heading="Revenue">
...
<BentoProse id="preview-gating" eyebrow="The invariant" heading="Preview protection & download gating">
