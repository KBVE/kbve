...
version: "0.1.10"
source_path: services/kubectl
version_toml: services/kubectl/version.toml
version_target: services/kubectl/Cargo.toml
...
bento:
    ...
    ctas:
        - label: View source
          href: https://github.com/KBVE/kbve/tree/main/services/kubectl
          primary: true
          external: true
        - label: CLI
          href: "#overview"
...
When a new behavior needs to land across all rotator and KubeVirt jobs (RCON-aware save-flush probes, Agones SDK Health calls, retry-with-backoff, ConfigMap-driven rotation policy), extend the Rust CLI in `services/kubectl/src/`, bump `version.toml`, and every manifest above auto-bumps via the post-publish sync.
