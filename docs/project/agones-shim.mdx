...
| `AGONES_SHIM_LIVE_FAILURE_THRESHOLD` | Consecutive liveness failures before exiting, default `3` |
| `AGONES_SHIM_STALE_METRIC` | Gauge that must keep moving; arms the staleness watchdog |
| `AGONES_SHIM_STALE_METRICS_URL` | Endpoint to scrape it from; falls back to `_COUNTER_METRICS_URL` |
| `AGONES_SHIM_STALE_INTERVAL_SECS` | Staleness scrape interval, default `30` |
| `AGONES_SHIM_STALE_TIMEOUT_SECS` | Exit once the metric has not moved for this long, default `300` |
| `AGONES_SHIM_READY_TIMEOUT_SECS` | Give up waiting, default `900`. `0` means never |
