...
jsonld:
    faq:
        - question: What is Friendslop?
          answer: A Godot 4.7 co-op game built on a lightweight GDScript ECS with a Rust gdext core, featuring deterministic procedural grassland, toon-shaded rendering, and server-owned online sessions.
        - question: Where can I play Friendslop?
          answer: Windows, macOS, and Linux builds ship to itch.io at kbve.itch.io/friendslop through the CI godot pipeline; a Steam release is in preparation.
        - question: How does Friendslop handle multiplayer terrain?
          answer: Terrain is never shipped — the server sends a seed and every client bakes the identical heightmap from the deterministic generator, so all machines agree on the world.
bento:
    badge: godot · co-op · grassland
    title: Co-op slop in a living
    accent: grassland
    lede: >-
        A Godot 4.7 co-op game on a lightweight GDScript ECS with a Rust gdext
        core — deterministic worldgen, GPU-streamed grass, toon shading, and
        server-owned online sessions — shipping Windows, macOS, and Linux to
        itch.io on every release.
    ctas:
        - label: Play on itch.io
          href: https://kbve.itch.io/friendslop
          primary: true
          external: true
        - label: Steam art
          href: "#steam"
        - label: View source
          href: https://github.com/KBVE/kbve/tree/main/apps/friendslop/godot-friendslop
          external: true
    aside:
        icon: server
        title: Q — the Rust core
        body: >-
            The <code>q</code> gdext crate owns terrain, grass, physics, netcode
            and now the Steamworks bridge — <code>QSteam</code> registers on every
            build and simply answers unavailable outside the Steam flavor.
        points:
            - <strong>Deterministic worldgen</strong> — seed in, same hills out, on every machine.
            - <strong>One simulation</strong> — solo and online step the same Rust sim.
    stats:
        - { label: Engine, value: Godot 4.7.1, icon: box }
        - { label: Platforms, value: Win · macOS · Linux, icon: package }
        - { label: Status, value: beta, icon: activity }
        - { label: License, value: KBVE, icon: license }
    features:
        - title: GDScript ECS
          icon: zap
          body: <code>addons/GodotECS</code> — observer hubs and entity relations driving gameplay.
        - title: Procedural grassland
          icon: book
          body: GPU-streamed grass, flora and trees over deterministic terrain from the <code>q</code> crate.
        - title: Server-owned online
          icon: network
          body: Intent-only movement against an authoritative server; the seed is the only terrain ever shipped.
        - title: Steam-ready art
          icon: shield
          body: Every capsule renders from the live game world in one <code>nx</code> pass — see the gallery below.
    related:
        - title: Friendslop Server
          href: /project/friendslop-server/
          copy: The authoritative half — session, sim and verification.
          icon: server
...
<BentoProse id="build" eyebrow="CI" heading="Build & Ship">
...
<BentoProse id="identity" eyebrow="Front door" heading="Title screen & identity">
...
<BentoProse id="foliage" eyebrow="Rendering" heading="Shadows & foliage">
...
<BentoProse id="online" eyebrow="Multiplayer" heading="Online sessions">
...
<BentoProse id="pets" eyebrow="Companions" heading="Pet robots">
...
<BentoProse id="villagers" eyebrow="NPCs" heading="The villagers' day">
...
<BentoProse id="simulation" eyebrow="Physics" heading="One simulation, and what is left of the second">
...
<BentoProse id="local" eyebrow="Dev loop" heading="Local">

- `moon run godot-friendslop:test` — import + gdUnit4 headless suite
- `moon run godot-friendslop:test-smoke` — raw SceneTree smoke pass
- `moon run godot-friendslop:export` — all three release exports
...
<BentoProse id="steam" eyebrow="Store art" heading="Steam page assets">

Every image below is rendered from the live game world by `src/ui/steam/page_assets.gd` — pinned seed, real terrain, real foliage — and regenerated in one pass with `moon run godot-friendslop:steam-assets`. Each figure is anchored, so feedback can point at a slot by its link. The title art is drawn at render time from the Alagard face, so the whole set re-exports under a new name with one argument.

<div class="steam-gallery not-content" style="display:flex;flex-direction:column;gap:2rem">
	<figure id="steam-header-capsule">
		<a href="/assets/images/brand/rentearth/steam/header_capsule.png"><img src="/assets/images/brand/rentearth/steam/header_capsule.png" alt="Header capsule" loading="lazy" /></a>
		<figcaption>Header capsule — 920×430, store page top</figcaption>
	</figure>
	<figure id="steam-small-capsule">
		<a href="/assets/images/brand/rentearth/steam/small_capsule.png"><img src="/assets/images/brand/rentearth/steam/small_capsule.png" alt="Small capsule" loading="lazy" /></a>
		<figcaption>Small capsule — 462×174, search &amp; lists (Steam derives 231×87 and 120×45 from it)</figcaption>
	</figure>
	<figure id="steam-main-capsule">
		<a href="/assets/images/brand/rentearth/steam/main_capsule.png"><img src="/assets/images/brand/rentearth/steam/main_capsule.png" alt="Main capsule" loading="lazy" /></a>
		<figcaption>Main capsule — 1232×706, front-page features &amp; big picture</figcaption>
	</figure>
	<figure id="steam-vertical-capsule">
		<a href="/assets/images/brand/rentearth/steam/vertical_capsule.png"><img src="/assets/images/brand/rentearth/steam/vertical_capsule.png" alt="Vertical capsule" loading="lazy" /></a>
		<figcaption>Vertical capsule — 748×896, sale &amp; browse pages</figcaption>
	</figure>
	<figure id="steam-library-capsule">
		<a href="/assets/images/brand/rentearth/steam/library_capsule.png"><img src="/assets/images/brand/rentearth/steam/library_capsule.png" alt="Library capsule" loading="lazy" /></a>
		<figcaption>Library capsule — 600×900, library grid</figcaption>
	</figure>
	<figure id="steam-library-header">
		<a href="/assets/images/brand/rentearth/steam/library_header.png"><img src="/assets/images/brand/rentearth/steam/library_header.png" alt="Library header" loading="lazy" /></a>
		<figcaption>Library header — 920×430, recent-games shelf</figcaption>
	</figure>
	<figure id="steam-library-logo">
		<a href="/assets/images/brand/rentearth/steam/library_logo.png"><img src="/assets/images/brand/rentearth/steam/library_logo.png" alt="Library logo" loading="lazy" /></a>
		<figcaption>Library logo — transparent, Steam overlays it on the hero</figcaption>
	</figure>
	<figure id="steam-library-hero">
		<a href="/assets/images/brand/rentearth/steam/library_hero.png"><img src="/assets/images/brand/rentearth/steam/library_hero.png" alt="Library hero" loading="lazy" /></a>
		<figcaption>Library hero — 3840×1240, library detail banner; no title baked in by design</figcaption>
	</figure>
	<figure id="steam-page-background">
		<a href="/assets/images/brand/rentearth/steam/page_background.png"><img src="/assets/images/brand/rentearth/steam/page_background.png" alt="Store page background" loading="lazy" /></a>
		<figcaption>Store page background — 1438×810, sits behind the store page</figcaption>
	</figure>
	<figure id="steam-event-cover">
		<a href="/assets/images/brand/rentearth/steam/event_cover.png"><img src="/assets/images/brand/rentearth/steam/event_cover.png" alt="Event cover" loading="lazy" /></a>
		<figcaption>Event cover — 800×450, announcements</figcaption>
	</figure>
	<figure id="steam-event-header">
		<a href="/assets/images/brand/rentearth/steam/event_header.png"><img src="/assets/images/brand/rentearth/steam/event_header.png" alt="Event header" loading="lazy" /></a>
		<figcaption>Event header — 1920×622, event detail page</figcaption>
	</figure>
	<figure id="steam-icons">
		<a href="/assets/images/brand/rentearth/steam/icon_512.png"><img src="/assets/images/brand/rentearth/steam/icon_512.png" alt="Shortcut icon" style="max-width:8rem" loading="lazy" /></a>
		<a href="/assets/images/brand/rentearth/steam/app_icon_184.jpg"><img src="/assets/images/brand/rentearth/steam/app_icon_184.jpg" alt="App icon" style="max-width:4rem" loading="lazy" /></a>
		<figcaption>Icons — 512×512 shortcut PNG, 184×184 app JPG</figcaption>
	</figure>
	<figure id="steam-screenshot-01">
		<a href="/assets/images/brand/rentearth/steam/screenshot_01.png"><img src="/assets/images/brand/rentearth/steam/screenshot_01.png" alt="Screenshot: river crossing" loading="lazy" /></a>
		<figcaption>Screenshot 1 — 1920×1080, the crossing: bridge, mech, riverbank</figcaption>
	</figure>
	<figure id="steam-screenshot-02">
		<a href="/assets/images/brand/rentearth/steam/screenshot_02.png"><img src="/assets/images/brand/rentearth/steam/screenshot_02.png" alt="Screenshot: mechs on the hill" loading="lazy" /></a>
		<figcaption>Screenshot 2 — 1920×1080, mechs walking the hillside, river winding out</figcaption>
	</figure>
	<figure id="steam-screenshot-03">
		<a href="/assets/images/brand/rentearth/steam/screenshot_03.png"><img src="/assets/images/brand/rentearth/steam/screenshot_03.png" alt="Screenshot: meadow" loading="lazy" /></a>
		<figcaption>Screenshot 3 — 1920×1080, meadow with tree shadows and flowers</figcaption>
	</figure>
	<figure id="steam-screenshot-04">
		<a href="/assets/images/brand/rentearth/steam/screenshot_04.png"><img src="/assets/images/brand/rentearth/steam/screenshot_04.png" alt="Screenshot: bridge from downstream" loading="lazy" /></a>
		<figcaption>Screenshot 4 — 1920×1080, the bridge from downstream</figcaption>
	</figure>
	<figure id="steam-screenshot-05">
		<a href="/assets/images/brand/rentearth/steam/screenshot_05.png"><img src="/assets/images/brand/rentearth/steam/screenshot_05.png" alt="Screenshot: grove vista" loading="lazy" /></a>
		<figcaption>Screenshot 5 — 1920×1080, grove vista over the river valley</figcaption>
	</figure>
</div>
