...
```bash
moon run astro-kbve:sync-professiondb --skip-nx-cache 2>&1 | tail -15
```
...
```bash
BUILD_PROTO=1 moon run bevy_items:build 2>&1 | tail -8
...
```bash
moon run astro-kbve:sync-itemdb-uecpp --skip-nx-cache 2>&1 | tail -5
moon run astro-kbve:sync-itemdb --skip-nx-cache 2>&1 | tail -5
...
```bash
moon run bevy_items:check-desktop 2>&1 | tail -15
```
...
```bash
moon run astro-kbve:sync-professiondb --skip-nx-cache >/dev/null 2>&1
