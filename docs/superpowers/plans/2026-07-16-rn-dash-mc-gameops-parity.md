...
export interface RawMcPlayerList {
	online: number;
	max: number;
	players: { name: string; uuid: string | null; skin_url: string | null; server: string }[];
	servers: { server: string; online: number; max: number; reachable: boolean }[];
	cached_at: number;
}
export function mapPlayerList(raw: RawMcPlayerList): McServerItem[];
export interface McStreamOptions { baseUrl?: string; pollMs?: number }
export function createMcStream(opts?: McStreamOptions): StreamStore<McServerItem>;
```
...
describe('mapPlayerList', () => {
	...
	it('carries cached_at onto every item', () => {
		expect(mapPlayerList(raw).every((i) => i.cachedAt === 1750000000)).toBe(true);
	});
	it('handles empty payload', () => {
		expect(
			mapPlayerList({ online: 0, max: 0, players: [], servers: [], cached_at: 0 }),
		).toEqual([]);
	});
});
```
...
```ts
export interface RconExecRequest { command: string; args?: string[] }
export interface RconExecResponse { ok: boolean; output: string; latency_ms: number; error?: string }
export type RconExecFn = (server: string, body: RconExecRequest) => Promise<RconExecResponse>;
export function createRconExec(opts: { getToken: () => Promise<string | null>; baseUrl?: string }): RconExecFn;
```
