...
- **Extract the Linux engine image once**, shared by all Linux builders → restore concurrency (`maxRunners: 3` target: server + game + one plugin/engine Linux build).
...
- Confirm concurrent Linux builds reuse one image (`docker images` shows one engine; `df /var/mnt/ramdisk/docker` does not double) at `maxRunners: 3`.
...
A second adversarial audit read the spec **against the workflow it modifies** (`.github/workflows/ci-unreal-build.yml` @ `origin/dev`) and the full `values-ue.yaml`, not just the platform facts. It found three **blockers** that break the build the moment concurrency (`maxRunners > 1`) is restored, plus budget/rollback gaps. These are not optional polish — `maxRunners` stays at `1` until they are closed.

This work ships as **one PR** (#13278): the items below are workstreams (WS-1…WS-5) within this single PR, not separate follow-up PRs. They are sequenced, not independent — WS-1 gates the `maxRunners: 3` flip.
...
1. **`ci-unreal-build.yml` rewrite** — per-job path namespacing under the shared hostPath (project + output + clone, not just the git tree); **remove/rescope** the per-build `docker system prune -f` and centralize GC in `prepare` under `flock`; thread `prepare`/`flock` and clone-by-explicit-SHA (§4 SHA-skew). Closes B1, B2, B3. WS-1 is the gate for `maxRunners: 3`.
2. **RAM-budget worksheet + hard cap + alert** — the worksheet must count **all** RAM consumers: the shared tmpfs *plus* the runner pods' own RAM emptyDirs (`work` `medium: Memory` 24Gi + `shared-tmp` `medium: Memory` 16Gi, per `values-ue.yaml`, i.e. up to **3×40 = 120 GB** at `maxRunners: 3`, which live **outside** the tmpfs) + VM (20Gi) + etcd/CH/PG baseline + page-cache margin. On the corrected **~500 GB** node this sum closes with ~100 GB to spare (WS-2), so the gate is the **tmpfs `size=` ceiling (200 GB)**, not node RAM. Deliver: the worksheet, the hard tmpfs `size=200G`, and a **runtime** tmpfs-usage alert — the §4 pre-flight check runs *before* the cook and cannot see peak scratch (the unguarded etcd-flap path, #12987).
