...
namespace KBVE.MMExtensions.Ai
{
  public static class AiStateMachine
  {
    ...
    private static readonly Dictionary<
      AiStateType,
      Dictionary<AiStateType, int>
    > StateTransitionMemory = new();
        ...
        return stateType switch
        {
          ...
          _
            => throw new ArgumentOutOfRangeException(
              nameof(stateType),
              $"No string representation for state {stateType}"
            )
        };
      }
    ...
    public static async UniTask<AiStateType> StateOpsAsync(string fromStateName, string toStateName)
    {
      ...
      if (
        StateTransitionMemory.TryGetValue(fromState, out var transitions)
      ...
      Debug.Log(
        $"Shifted from {fromStateName} to {toStateName} with shift amount {shiftAmount}. Result: {result.ToStateName()}"
      );

      return result;
    }

    private static async UniTask CacheTransitionAsync(
      AiStateType fromState,
      AiStateType toState,
      int shiftAmount
    )
    {
