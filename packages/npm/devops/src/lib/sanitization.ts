 ...
 */
export async function markdownToJsonSafeString(
 ...
 */
export async function markdownToJsonSafeStringThenStrip(
	text: string,
): Promise<string> {
 ...
 */
export function sanitizePort(
	port: number,
	additionalRestrictedPorts: number[] = [],
): number {
	const defaultRestrictedPorts = [443, 80, 22];
	const restrictedPorts = [
		...defaultRestrictedPorts,
		...additionalRestrictedPorts,
	];
...
export function sanitizeContainerImage(image: string): string {
	...
	if (!sanitizedImage || sanitizedImage.length === 0) {
		throw new Error(
			'Invalid container image name. Image name must be alphanumeric and can include underscores, slashes, colons, and periods.',
		);
	}
