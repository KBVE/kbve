...
function createVNodeFromTile(tile: BentoTile): VirtualNode {
	...
	const datasetAttrs = tile.dataset
		? Object.fromEntries(Object.entries(tile.dataset).map(([k, v]) => [`data-${k}`, v]))
		: {};
	...
	const wrapper: VirtualNode = {
		...
		class: rootClasses,
		attrs: {
			...(tile.href ? { href: tile.href, target: tile.target ?? '_self' } : {}),
