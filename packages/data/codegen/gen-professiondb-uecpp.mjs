#!/usr/bin/env node
/**
 * Generate Unreal Engine C++ (USTRUCT + yyjson populate) from professiondb.proto.
 *
 * Mirrors gen-professiondb-zod.mjs but emits UE-native types instead of Zod:
 *   - one USTRUCT per item-package message (topo-ordered, definition-before-use)
 *   - a Populate(FStruct&, yyjson_val*) per message reading the camelCase JSON
 *     keys the data codegen emits.
 *
 * Output (into the KBVEProfessionDB plugin):
 *   Public/Generated/KBVEProfessionDBProtoTypes.h   (USTRUCTs, UHT-processed)
 *   Public/Generated/KBVEProfessionDBProtoParse.h   (yyjson populate fns)
 *
 * Usage: node packages/data/codegen/gen-professiondb-uecpp.mjs
 */
...
const DESCRIPTOR = resolve(__dirname, 'descriptors/professiondb.binpb');
const OUT_DIR = resolve(
	__dirname,
	'../../../packages/unreal/KBVEProfessionDB/Source/KBVEProfessionDB/Public/Generated',
);
const PKG_PREFIX = 'profession.';
...
function emitStructs(ordered, messages) {
	...
	lines.push('// AUTO-GENERATED by gen-professiondb-uecpp.mjs from profession/professiondb.proto — DO NOT EDIT', '');
	lines.push('#include "CoreMinimal.h"');
	lines.push('#include "KBVEProfessionDBProtoTypes.generated.h"', '');

	for (const msg of ordered) {
		lines.push('USTRUCT(BlueprintType)');
		lines.push(`struct KBVEPROFESSIONDB_API ${structName(msg.name)}`);
		lines.push('{');
		lines.push('\tGENERATED_BODY()', '');
		for (const f of msg.fields) {
			const ueType = fieldUEType(f, messages);
			lines.push('\tUPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "KBVE|ProfessionDB")');
...
function emitParsers(ordered, messages) {
	const lines = [];
	lines.push('#pragma once', '');
	lines.push('// AUTO-GENERATED by gen-professiondb-uecpp.mjs — DO NOT EDIT', '');
	lines.push('#include "CoreMinimal.h"');
	lines.push('#include "Misc/Base64.h"');
	lines.push('#include "Generated/KBVEProfessionDBProtoTypes.h"');
	lines.push('#include "KBVEYYJson.h"', '');
	lines.push('namespace KBVEProfessionDBProto', '{');
...
writeFileSync(resolve(OUT_DIR, 'KBVEProfessionDBProtoTypes.h'), emitStructs(ordered, messages) + '\n');
writeFileSync(resolve(OUT_DIR, 'KBVEProfessionDBProtoParse.h'), emitParsers(ordered, messages) + '\n');
