/**
 * AUTO-GENERATED by proto-to-zod — DO NOT EDIT
 *
 * Source: ../descriptors/meme.binpb
 * Config: ../meme-zod-config.json
 * Generated: 2026-05-03T09:31:28.491Z
 */

import { z } from 'zod';

export const MemeFormats = [
	'image',
	'gif',
	'video',
	'webp_anim',
] as const;

export type MemeFormatValue = (typeof MemeFormats)[number];

export const MemeFormatSchema = z.enum(MemeFormats);

export const MemeStatuses = [
	'draft',
	'pending',
	'published',
	'rejected',
	'archived',
	'flagged',
	'banned',
] as const;

export type MemeStatusValue = (typeof MemeStatuses)[number];

export const MemeStatusSchema = z.enum(MemeStatuses);

export const ReactionTypes = [
	'like',
	'dislike',
	'fire',
	'skull',
	'cry',
	'cap',
] as const;

export type ReactionTypeValue = (typeof ReactionTypes)[number];

export const ReactionTypeSchema = z.enum(ReactionTypes);

export const MemeRarities = [
	'common',
	'uncommon',
	'rare',
	'epic',
	'legendary',
	'mythic',
] as const;

export type MemeRarityValue = (typeof MemeRarities)[number];

export const MemeRaritySchema = z.enum(MemeRarities);

export const MemeElements = [
	'dank',
	'wholesome',
	'cursed',
	'deep_fried',
	'surreal',
	'meta',
	'edgy',
	'nostalgic',
] as const;

export type MemeElementValue = (typeof MemeElements)[number];

export const MemeElementSchema = z.enum(MemeElements);

export const AbilityTriggers = [
	'on_play',
	'on_attack',
	'on_defend',
	'on_death',
	'passive',
	'on_turn_start',
	'on_turn_end',
] as const;

export type AbilityTriggerValue = (typeof AbilityTriggers)[number];

export const AbilityTriggerSchema = z.enum(AbilityTriggers);

export const AbilityEffects = [
	'damage',
	'heal',
	'buff_atk',
	'buff_def',
	'debuff_atk',
	'debuff_def',
	'draw',
	'stun',
	'shield',
	'element_shift',
] as const;

export type AbilityEffectValue = (typeof AbilityEffects)[number];

export const AbilityEffectSchema = z.enum(AbilityEffects);

export const FeedSorts = [
	'hot',
	'new',
	'top',
	'random',
	'rising',
] as const;

export type FeedSortValue = (typeof FeedSorts)[number];

export const FeedSortSchema = z.enum(FeedSorts);

export const FeedTimeWindows = [
	'hour',
	'day',
	'week',
	'month',
	'all',
] as const;

export type FeedTimeWindowValue = (typeof FeedTimeWindows)[number];

export const FeedTimeWindowSchema = z.enum(FeedTimeWindows);

export const ReportReasons = [
	'spam',
	'nsfw',
	'hate_speech',
	'harassment',
	'copyright',
	'misinformation',
	'other',
] as const;

export type ReportReasonValue = (typeof ReportReasons)[number];

export const ReportReasonSchema = z.enum(ReportReasons);

export const BattleStatuses = [
	'waiting',
	'in_progress',
	'completed',
	'abandoned',
	'draw',
] as const;

export type BattleStatusValue = (typeof BattleStatuses)[number];

export const BattleStatusSchema = z.enum(BattleStatuses);

// BattleAction
export const BattleActionSchema = z
	.object({
		turn: z.number(),
		player_id: z.string(),
		card_id: z.string(),
		target_card_id: z.string().optional(),
		ability_name: z.string().optional(),
		damage_dealt: z.number(),
		healing_done: z.number(),
		effect_applied: z.string().optional(),
	});

export type BattleAction = z.infer<typeof BattleActionSchema>;

// BattleResult
export const BattleResultSchema = z
	.object({
		battle_id: z.string(),
		player_a_id: z.string(),
		player_b_id: z.string(),
		status: BattleStatusSchema,
		winner_id: z.string().optional(),
		total_turns: z.number(),
		actions: z.array(BattleActionSchema).optional(),
		elo_delta_a: z.number(),
		elo_delta_b: z.number(),
		created_at: z.string(),
		completed_at: z.string().optional(),
	});

export type BattleResult = z.infer<typeof BattleResultSchema>;

// ListBattleHistoryResponse
export const ListBattleHistoryResponseSchema = z
	.object({
		battles: z.array(BattleResultSchema).optional(),
		next_cursor: z.string().optional(),
		has_more: z.boolean(),
	});

export type ListBattleHistoryResponse = z.infer<typeof ListBattleHistoryResponseSchema>;

// ListBattleHistoryRequest
export const ListBattleHistoryRequestSchema = z
	.object({
		user_id: z.string(),
		limit: z.number(),
		cursor: z.string().optional(),
	});

export type ListBattleHistoryRequest = z.infer<typeof ListBattleHistoryRequestSchema>;

// GetBattleResponse
export const GetBattleResponseSchema = z
	.object({
		found: z.boolean(),
		battle: BattleResultSchema.optional(),
	});

export type GetBattleResponse = z.infer<typeof GetBattleResponseSchema>;

// GetBattleRequest
export const GetBattleRequestSchema = z
	.object({
		battle_id: z.string(),
	});

export type GetBattleRequest = z.infer<typeof GetBattleRequestSchema>;

// StartBattleResponse
export const StartBattleResponseSchema = z
	.object({
		result: ResultSchema.optional(),
		battle_id: z.string().optional(),
		status: BattleStatusSchema.optional(),
	});

export type StartBattleResponse = z.infer<typeof StartBattleResponseSchema>;

// StartBattleRequest
export const StartBattleRequestSchema = z
	.object({
		deck_id: z.string(),
	});

export type StartBattleRequest = z.infer<typeof StartBattleRequestSchema>;

// DeckCard
export const DeckCardSchema = z
	.object({
		card_id: z.string(),
		position: z.number().optional(),
	});

export type DeckCard = z.infer<typeof DeckCardSchema>;

// MemeDeck
export const MemeDeckSchema = z
	.object({
		id: z.string(),
		owner_id: z.string(),
		name: z.string(),
		cards: z.array(DeckCardSchema).optional(),
		is_active: z.boolean(),
		created_at: z.string(),
		updated_at: z.string().optional(),
	});

export type MemeDeck = z.infer<typeof MemeDeckSchema>;

// ListDecksResponse
export const ListDecksResponseSchema = z
	.object({
		decks: z.array(MemeDeckSchema).optional(),
	});

export type ListDecksResponse = z.infer<typeof ListDecksResponseSchema>;

// ListDecksRequest
export const ListDecksRequestSchema = z
	.object({
		owner_id: z.string(),
	});

export type ListDecksRequest = z.infer<typeof ListDecksRequestSchema>;

// UpdateDeckResponse
export const UpdateDeckResponseSchema = z
	.object({
		result: ResultSchema.optional(),
		deck: MemeDeckSchema.optional(),
	});

export type UpdateDeckResponse = z.infer<typeof UpdateDeckResponseSchema>;

// UpdateDeckRequest
export const UpdateDeckRequestSchema = z
	.object({
		deck_id: z.string(),
		name: z.string().optional(),
		card_ids: z.array(z.string()).optional(),
		is_active: z.boolean().optional(),
	});

export type UpdateDeckRequest = z.infer<typeof UpdateDeckRequestSchema>;

// CreateDeckResponse
export const CreateDeckResponseSchema = z
	.object({
		result: ResultSchema.optional(),
		deck: MemeDeckSchema.optional(),
	});

export type CreateDeckResponse = z.infer<typeof CreateDeckResponseSchema>;

// CreateDeckRequest
export const CreateDeckRequestSchema = z
	.object({
		name: z.string(),
		card_ids: z.array(z.string()).optional(),
	});

export type CreateDeckRequest = z.infer<typeof CreateDeckRequestSchema>;

// CardAbility
export const CardAbilitySchema = z
	.object({
		name: z.string(),
		description: z.string(),
		trigger: AbilityTriggerSchema,
		effect: AbilityEffectSchema,
		value: z.number(),
		cooldown: z.number().optional(),
		duration: z.number().optional(),
	});

export type CardAbility = z.infer<typeof CardAbilitySchema>;

// MemeCardStats
export const MemeCardStatsSchema = z
	.object({
		meme_id: z.string(),
		rarity: MemeRaritySchema,
		element: MemeElementSchema,
		attack: z.number(),
		defense: z.number(),
		hp: z.number(),
		energy_cost: z.number(),
		abilities: z.array(CardAbilitySchema).optional(),
		flavor_text: z.string().optional(),
		level: z.number(),
		xp: z.number(),
		evolves_from: z.string().optional(),
		evolves_into: z.string().optional(),
	});

export type MemeCardStats = z.infer<typeof MemeCardStatsSchema>;

// MintCardResponse
export const MintCardResponseSchema = z
	.object({
		result: ResultSchema.optional(),
		card: MemeCardStatsSchema.optional(),
	});

export type MintCardResponse = z.infer<typeof MintCardResponseSchema>;

// MintCardRequest
export const MintCardRequestSchema = z
	.object({
		meme_id: z.string(),
	});

export type MintCardRequest = z.infer<typeof MintCardRequestSchema>;

// ReportMemeResponse
export const ReportMemeResponseSchema = z
	.object({
		result: ResultSchema.optional(),
	});

export type ReportMemeResponse = z.infer<typeof ReportMemeResponseSchema>;

// ReportMemeRequest
export const ReportMemeRequestSchema = z
	.object({
		meme_id: z.string(),
		reason: ReportReasonSchema,
		detail: z.string().optional(),
	});

export type ReportMemeRequest = z.infer<typeof ReportMemeRequestSchema>;

// MemePlayerStats
export const MemePlayerStatsSchema = z
	.object({
		user_id: z.string(),
		total_battles: z.number(),
		wins: z.number(),
		losses: z.number(),
		draws: z.number(),
		elo_rating: z.number(),
		cards_owned: z.number(),
		highest_streak: z.number(),
		rank_title: z.string().optional(),
	});

export type MemePlayerStats = z.infer<typeof MemePlayerStatsSchema>;

// MemeUserProfile
export const MemeUserProfileSchema = z
	.object({
		user_id: z.string(),
		display_name: z.string().optional(),
		avatar_url: z.string().optional(),
		bio: z.string().optional(),
		total_memes: z.number(),
		total_reactions_received: z.number(),
		total_views_received: z.number(),
		follower_count: z.number(),
		following_count: z.number(),
		card_stats: MemePlayerStatsSchema.optional(),
		joined_at: z.string(),
		updated_at: z.string().optional(),
	});

export type MemeUserProfile = z.infer<typeof MemeUserProfileSchema>;

// ListFollowingResponse
export const ListFollowingResponseSchema = z
	.object({
		following: z.array(MemeUserProfileSchema).optional(),
		next_cursor: z.string().optional(),
		has_more: z.boolean(),
	});

export type ListFollowingResponse = z.infer<typeof ListFollowingResponseSchema>;

// ListFollowingRequest
export const ListFollowingRequestSchema = z
	.object({
		user_id: z.string(),
		limit: z.number(),
		cursor: z.string().optional(),
	});

export type ListFollowingRequest = z.infer<typeof ListFollowingRequestSchema>;

// ListFollowersResponse
export const ListFollowersResponseSchema = z
	.object({
		followers: z.array(MemeUserProfileSchema).optional(),
		next_cursor: z.string().optional(),
		has_more: z.boolean(),
	});

export type ListFollowersResponse = z.infer<typeof ListFollowersResponseSchema>;

// ListFollowersRequest
export const ListFollowersRequestSchema = z
	.object({
		user_id: z.string(),
		limit: z.number(),
		cursor: z.string().optional(),
	});

export type ListFollowersRequest = z.infer<typeof ListFollowersRequestSchema>;

// UnfollowResponse
export const UnfollowResponseSchema = z
	.object({
		result: ResultSchema.optional(),
	});

export type UnfollowResponse = z.infer<typeof UnfollowResponseSchema>;

// UnfollowRequest
export const UnfollowRequestSchema = z
	.object({
		target_user_id: z.string(),
	});

export type UnfollowRequest = z.infer<typeof UnfollowRequestSchema>;

// FollowResponse
export const FollowResponseSchema = z
	.object({
		result: ResultSchema.optional(),
	});

export type FollowResponse = z.infer<typeof FollowResponseSchema>;

// FollowRequest
export const FollowRequestSchema = z
	.object({
		target_user_id: z.string(),
	});

export type FollowRequest = z.infer<typeof FollowRequestSchema>;

// GetMemeProfileResponse
export const GetMemeProfileResponseSchema = z
	.object({
		found: z.boolean(),
		profile: MemeUserProfileSchema.optional(),
	});

export type GetMemeProfileResponse = z.infer<typeof GetMemeProfileResponseSchema>;

// GetMemeProfileRequest
export const GetMemeProfileRequestSchema = z
	.object({
		user_id: z.string(),
	});

export type GetMemeProfileRequest = z.infer<typeof GetMemeProfileRequestSchema>;

// TemplateCaptionSlot
export const TemplateCaptionSlotSchema = z
	.object({
		label: z.string(),
		default_x: z.number(),
		default_y: z.number(),
		default_font_size: z.number().optional(),
		placeholder: z.string().optional(),
		max_width: z.number().optional(),
		max_chars: z.number().optional(),
	});

export type TemplateCaptionSlot = z.infer<typeof TemplateCaptionSlotSchema>;

// MemeTemplate
export const MemeTemplateSchema = z
	.object({
		id: z.string(),
		name: z.string(),
		description: z.string().optional(),
		image_url: z.string(),
		thumbnail_url: z.string().optional(),
		width: z.number(),
		height: z.number(),
		slots: z.array(TemplateCaptionSlotSchema).optional(),
		usage_count: z.number(),
		tags: z.array(z.string()).optional(),
		created_at: z.string(),
		updated_at: z.string().optional(),
	});

export type MemeTemplate = z.infer<typeof MemeTemplateSchema>;

// ListTemplatesResponse
export const ListTemplatesResponseSchema = z
	.object({
		templates: z.array(MemeTemplateSchema).optional(),
		next_cursor: z.string().optional(),
		has_more: z.boolean(),
	});

export type ListTemplatesResponse = z.infer<typeof ListTemplatesResponseSchema>;

// ListTemplatesRequest
export const ListTemplatesRequestSchema = z
	.object({
		limit: z.number(),
		cursor: z.string().optional(),
		search: z.string().optional(),
	});

export type ListTemplatesRequest = z.infer<typeof ListTemplatesRequestSchema>;

// GetTemplateResponse
export const GetTemplateResponseSchema = z
	.object({
		found: z.boolean(),
		template: MemeTemplateSchema.optional(),
	});

export type GetTemplateResponse = z.infer<typeof GetTemplateResponseSchema>;

// GetTemplateRequest
export const GetTemplateRequestSchema = z
	.object({
		id: z.string(),
	});

export type GetTemplateRequest = z.infer<typeof GetTemplateRequestSchema>;

// MemeCaption
export const MemeCaptionSchema = z
	.object({
		text: z.string(),
		position_x: z.number(),
		position_y: z.number(),
		font_size: z.number().optional(),
		font_family: z.string().optional(),
		color: z.string().optional(),
		stroke_color: z.string().optional(),
		stroke_width: z.number().optional(),
		rotation: z.number().optional(),
		text_align: z.string().optional(),
		max_width: z.number().optional(),
	});

export type MemeCaption = z.infer<typeof MemeCaptionSchema>;

// Meme
export const MemeSchema = z
	.object({
		id: z.string(),
		author_id: z.string(),
		title: z.string().optional(),
		template_id: z.string().optional(),
		format: MemeFormatSchema,
		status: MemeStatusSchema,
		asset_url: z.string(),
		thumbnail_url: z.string().optional(),
		width: z.number().optional(),
		height: z.number().optional(),
		file_size: z.number().optional(),
		captions: z.array(MemeCaptionSchema).optional(),
		tags: z.array(z.string()).optional(),
		source_url: z.string().optional(),
		alt_text: z.string().optional(),
		view_count: z.number(),
		reaction_count: z.number(),
		share_count: z.number(),
		comment_count: z.number(),
		save_count: z.number(),
		created_at: z.string(),
		updated_at: z.string().optional(),
		published_at: z.string().optional(),
		card: MemeCardStatsSchema.optional(),
		content_hash: z.string().optional(),
	});

export type Meme = z.infer<typeof MemeSchema>;

// GetCollectionMemesResponse
export const GetCollectionMemesResponseSchema = z
	.object({
		memes: z.array(MemeSchema).optional(),
		next_cursor: z.string().optional(),
		has_more: z.boolean(),
	});

export type GetCollectionMemesResponse = z.infer<typeof GetCollectionMemesResponseSchema>;

// GetCollectionMemesRequest
export const GetCollectionMemesRequestSchema = z
	.object({
		collection_id: z.string(),
		limit: z.number(),
		cursor: z.string().optional(),
	});

export type GetCollectionMemesRequest = z.infer<typeof GetCollectionMemesRequestSchema>;

// MemeCollection
export const MemeCollectionSchema = z
	.object({
		id: z.string(),
		owner_id: z.string(),
		name: z.string(),
		description: z.string().optional(),
		cover_meme_id: z.string().optional(),
		is_public: z.boolean(),
		meme_count: z.number(),
		created_at: z.string(),
		updated_at: z.string().optional(),
	});

export type MemeCollection = z.infer<typeof MemeCollectionSchema>;

// ListCollectionsResponse
export const ListCollectionsResponseSchema = z
	.object({
		collections: z.array(MemeCollectionSchema).optional(),
		next_cursor: z.string().optional(),
		has_more: z.boolean(),
	});

export type ListCollectionsResponse = z.infer<typeof ListCollectionsResponseSchema>;

// ListCollectionsRequest
export const ListCollectionsRequestSchema = z
	.object({
		owner_id: z.string(),
		limit: z.number(),
		cursor: z.string().optional(),
	});

export type ListCollectionsRequest = z.infer<typeof ListCollectionsRequestSchema>;

// CreateCollectionResponse
export const CreateCollectionResponseSchema = z
	.object({
		result: ResultSchema.optional(),
		collection: MemeCollectionSchema.optional(),
	});

export type CreateCollectionResponse = z.infer<typeof CreateCollectionResponseSchema>;

// CreateCollectionRequest
export const CreateCollectionRequestSchema = z
	.object({
		name: z.string(),
		description: z.string().optional(),
		is_public: z.boolean(),
	});

export type CreateCollectionRequest = z.infer<typeof CreateCollectionRequestSchema>;

// UnsaveMemeResponse
export const UnsaveMemeResponseSchema = z
	.object({
		result: ResultSchema.optional(),
	});

export type UnsaveMemeResponse = z.infer<typeof UnsaveMemeResponseSchema>;

// UnsaveMemeRequest
export const UnsaveMemeRequestSchema = z
	.object({
		meme_id: z.string(),
		collection_id: z.string().optional(),
	});

export type UnsaveMemeRequest = z.infer<typeof UnsaveMemeRequestSchema>;

// SaveMemeResponse
export const SaveMemeResponseSchema = z
	.object({
		result: ResultSchema.optional(),
	});

export type SaveMemeResponse = z.infer<typeof SaveMemeResponseSchema>;

// SaveMemeRequest
export const SaveMemeRequestSchema = z
	.object({
		meme_id: z.string(),
		collection_id: z.string().optional(),
	});

export type SaveMemeRequest = z.infer<typeof SaveMemeRequestSchema>;

// MemeComment
export const MemeCommentSchema = z
	.object({
		id: z.string(),
		meme_id: z.string(),
		author_id: z.string(),
		body: z.string(),
		parent_id: z.string().optional(),
		reaction_count: z.number(),
		reply_count: z.number(),
		created_at: z.string(),
		updated_at: z.string().optional(),
	});

export type MemeComment = z.infer<typeof MemeCommentSchema>;

// ListCommentsResponse
export const ListCommentsResponseSchema = z
	.object({
		comments: z.array(MemeCommentSchema).optional(),
		next_cursor: z.string().optional(),
		has_more: z.boolean(),
	});

export type ListCommentsResponse = z.infer<typeof ListCommentsResponseSchema>;

// ListCommentsRequest
export const ListCommentsRequestSchema = z
	.object({
		meme_id: z.string(),
		limit: z.number(),
		cursor: z.string().optional(),
		parent_id: z.string().optional(),
	});

export type ListCommentsRequest = z.infer<typeof ListCommentsRequestSchema>;

// AddCommentResponse
export const AddCommentResponseSchema = z
	.object({
		result: ResultSchema.optional(),
		comment: MemeCommentSchema.optional(),
	});

export type AddCommentResponse = z.infer<typeof AddCommentResponseSchema>;

// AddCommentRequest
export const AddCommentRequestSchema = z
	.object({
		meme_id: z.string(),
		body: z.string(),
		parent_id: z.string().optional(),
	});

export type AddCommentRequest = z.infer<typeof AddCommentRequestSchema>;

// ReactResponse
export const ReactResponseSchema = z
	.object({
		result: ResultSchema.optional(),
	});

export type ReactResponse = z.infer<typeof ReactResponseSchema>;

// ReactRequest
export const ReactRequestSchema = z
	.object({
		meme_id: z.string(),
		reaction: ReactionTypeSchema,
	});

export type ReactRequest = z.infer<typeof ReactRequestSchema>;

// DeleteMemeResponse
export const DeleteMemeResponseSchema = z
	.object({
		result: ResultSchema.optional(),
	});

export type DeleteMemeResponse = z.infer<typeof DeleteMemeResponseSchema>;

// DeleteMemeRequest
export const DeleteMemeRequestSchema = z
	.object({
		id: z.string(),
	});

export type DeleteMemeRequest = z.infer<typeof DeleteMemeRequestSchema>;

// UpdateMemeResponse
export const UpdateMemeResponseSchema = z
	.object({
		result: ResultSchema.optional(),
		meme: MemeSchema.optional(),
	});

export type UpdateMemeResponse = z.infer<typeof UpdateMemeResponseSchema>;

// UpdateMemeRequest
export const UpdateMemeRequestSchema = z
	.object({
		id: z.string(),
		title: z.string().optional(),
		tags: z.array(z.string()).optional(),
		alt_text: z.string().optional(),
		status: MemeStatusSchema.optional(),
	});

export type UpdateMemeRequest = z.infer<typeof UpdateMemeRequestSchema>;

// GetMemeResponse
export const GetMemeResponseSchema = z
	.object({
		found: z.boolean(),
		meme: MemeSchema.optional(),
	});

export type GetMemeResponse = z.infer<typeof GetMemeResponseSchema>;

// GetMemeRequest
export const GetMemeRequestSchema = z
	.object({
		id: z.string(),
	});

export type GetMemeRequest = z.infer<typeof GetMemeRequestSchema>;

// SubmitMemeResponse
export const SubmitMemeResponseSchema = z
	.object({
		result: ResultSchema.optional(),
		meme: MemeSchema.optional(),
	});

export type SubmitMemeResponse = z.infer<typeof SubmitMemeResponseSchema>;

// SubmitMemeRequest
export const SubmitMemeRequestSchema = z
	.object({
		title: z.string().optional(),
		template_id: z.string().optional(),
		format: MemeFormatSchema,
		asset_url: z.string(),
		captions: z.array(MemeCaptionSchema).optional(),
		tags: z.array(z.string()).optional(),
		source_url: z.string().optional(),
		alt_text: z.string().optional(),
	});

export type SubmitMemeRequest = z.infer<typeof SubmitMemeRequestSchema>;

// FeedResponse
export const FeedResponseSchema = z
	.object({
		memes: z.array(MemeSchema).optional(),
		next_cursor: z.string().optional(),
		has_more: z.boolean(),
	});

export type FeedResponse = z.infer<typeof FeedResponseSchema>;

// FeedRequest
export const FeedRequestSchema = z
	.object({
		limit: z.number(),
		cursor: z.string().optional(),
		sort: FeedSortSchema,
		time_window: FeedTimeWindowSchema.optional(),
		tag: z.string().optional(),
		author_id: z.string().optional(),
		element: MemeElementSchema.optional(),
		cards_only: z.boolean().optional(),
	});

export type FeedRequest = z.infer<typeof FeedRequestSchema>;

// MemeReport
export const MemeReportSchema = z
	.object({
		id: z.string(),
		meme_id: z.string(),
		reporter_id: z.string(),
		reason: ReportReasonSchema,
		detail: z.string().optional(),
		resolved: z.boolean(),
		resolved_by: z.string().optional(),
		resolution_note: z.string().optional(),
		created_at: z.string(),
		resolved_at: z.string().optional(),
	});

export type MemeReport = z.infer<typeof MemeReportSchema>;

// MemeFollow
export const MemeFollowSchema = z
	.object({
		follower_id: z.string(),
		following_id: z.string(),
		created_at: z.string(),
	});

export type MemeFollow = z.infer<typeof MemeFollowSchema>;

// MemeSave
export const MemeSaveSchema = z
	.object({
		meme_id: z.string(),
		user_id: z.string(),
		collection_id: z.string().optional(),
		created_at: z.string(),
	});

export type MemeSave = z.infer<typeof MemeSaveSchema>;

// MemeReaction
export const MemeReactionSchema = z
	.object({
		meme_id: z.string(),
		user_id: z.string(),
		reaction: ReactionTypeSchema,
		created_at: z.string(),
	});

export type MemeReaction = z.infer<typeof MemeReactionSchema>;

// ElementMatchup
export const ElementMatchupSchema = z
	.object({
		attacker: MemeElementSchema,
		defender: MemeElementSchema,
		damage_multiplier: z.number(),
	});

export type ElementMatchup = z.infer<typeof ElementMatchupSchema>;
