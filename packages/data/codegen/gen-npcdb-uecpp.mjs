#!/usr/bin/env node
/**
 * Generate Unreal Engine C++ (USTRUCT + yyjson populate) from npcdb.proto.
 *
 * Mirrors gen-itemdb-uecpp.mjs but emits UE-native types instead of Zod:
 *   - one USTRUCT per npc-package message (topo-ordered, definition-before-use)
 *   - a Populate(FStruct&, yyjson_val*) per message reading the camelCase JSON
 *     keys the data codegen emits.
 *
 * Output (into the KBVENPCDB plugin):
 *   Public/Generated/KBVENPCDBProtoTypes.h   (USTRUCTs, UHT-processed)
 *   Public/Generated/KBVENPCDBProtoParse.h   (yyjson populate fns)
 *
 * Usage: node packages/data/codegen/gen-npcdb-uecpp.mjs
 */
...
const DESCRIPTOR = resolve(__dirname, 'descriptors/npcdb.binpb');
const OUT_DIR = resolve(
	__dirname,
	'../../../packages/unreal/KBVENPCDB/Source/KBVENPCDB/Public/Generated',
);
const PKG_PREFIX = 'npc.';
...
function emitStructs(ordered, messages) {
	...
	lines.push('// AUTO-GENERATED by gen-npcdb-uecpp.mjs from npc/npcdb.proto — DO NOT EDIT', '');
	lines.push('#include "CoreMinimal.h"');
	lines.push('#include "KBVENPCDBProtoTypes.generated.h"', '');

	for (const msg of ordered) {
		lines.push('USTRUCT(BlueprintType)');
		lines.push(`struct KBVENPCDB_API ${structName(msg.name)}`);
		lines.push('{');
		lines.push('\tGENERATED_BODY()', '');
		for (const f of msg.fields) {
			const ueType = fieldUEType(f, messages);
			lines.push('\tUPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "KBVE|NPCDB")');
...
function emitParsers(ordered, messages) {
	const lines = [];
	lines.push('#pragma once', '');
	lines.push('// AUTO-GENERATED by gen-npcdb-uecpp.mjs — DO NOT EDIT', '');
	lines.push('#include "CoreMinimal.h"');
	lines.push('#include "Generated/KBVENPCDBProtoTypes.h"');
	lines.push('#include "KBVEYYJson.h"', '');
	lines.push('namespace KBVENPCDBProto', '{');
...
writeFileSync(resolve(OUT_DIR, 'KBVENPCDBProtoTypes.h'), emitStructs(ordered, messages) + '\n');
writeFileSync(resolve(OUT_DIR, 'KBVENPCDBProtoParse.h'), emitParsers(ordered, messages) + '\n');
