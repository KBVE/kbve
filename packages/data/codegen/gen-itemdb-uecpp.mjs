#!/usr/bin/env node
/**
 * Generate Unreal Engine C++ (USTRUCT + yyjson populate) from itemdb.proto.
 *
 * Mirrors gen-itemdb-zod.mjs but emits UE-native types instead of Zod:
 ...
 *
 * Output (into the KBVEItemDB plugin):
 *   Public/Generated/KBVEItemDBProtoTypes.h   (USTRUCTs, UHT-processed)
 *   Public/Generated/KBVEItemDBProtoParse.h   (yyjson populate fns)
 *
 * Usage: node packages/data/codegen/gen-itemdb-uecpp.mjs
 */
...
const DESCRIPTOR = resolve(__dirname, 'descriptors/itemdb.binpb');
const OUT_DIR = resolve(
	__dirname,
	'../../../packages/unreal/KBVEItemDB/Source/KBVEItemDB/Public/Generated',
);
const PKG_PREFIX = 'item.';
...
function emitStructs(ordered, messages) {
	...
	lines.push('// AUTO-GENERATED by gen-itemdb-uecpp.mjs from item/itemdb.proto — DO NOT EDIT', '');
	lines.push('#include "CoreMinimal.h"');
	lines.push('#include "KBVEItemDBProtoTypes.generated.h"', '');

	for (const msg of ordered) {
		lines.push('USTRUCT(BlueprintType)');
		lines.push(`struct KBVEITEMDB_API ${structName(msg.name)}`);
		lines.push('{');
		lines.push('\tGENERATED_BODY()', '');
		for (const f of msg.fields) {
			const ueType = fieldUEType(f, messages);
			lines.push('\tUPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "KBVE|ItemDB")');
...
function emitParsers(ordered, messages) {
	const lines = [];
	lines.push('#pragma once', '');
	lines.push('// AUTO-GENERATED by gen-itemdb-uecpp.mjs — DO NOT EDIT', '');
	lines.push('#include "CoreMinimal.h"');
	lines.push('#include "Misc/Base64.h"');
	lines.push('#include "Generated/KBVEItemDBProtoTypes.h"');
	lines.push('#include "KBVEYYJson.h"', '');
	lines.push('namespace KBVEItemDBProto', '{');
...
writeFileSync(resolve(OUT_DIR, 'KBVEItemDBProtoTypes.h'), emitStructs(ordered, messages) + '\n');
writeFileSync(resolve(OUT_DIR, 'KBVEItemDBProtoParse.h'), emitParsers(ordered, messages) + '\n');
