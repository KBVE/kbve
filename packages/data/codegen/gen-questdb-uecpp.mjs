#!/usr/bin/env node
/**
 * Generate Unreal Engine C++ (USTRUCT + yyjson populate) from quest/questdb.proto.
 *
 * Mirrors gen-itemdb-uecpp.mjs but emits the quest proto-mirror types instead:
 *   - one USTRUCT per quest-package message (topo-ordered, definition-before-use)
 *   - a Populate(FStruct&, yyjson_val*) per message reading the camelCase JSON
 *     keys the data codegen emits.
 *
 * Adds map<string, V> support (e.g. QuestRewards.bonuses) which the itemdb
 * generator did not need.
 *
 * Output (into the KBVEQuestDB plugin):
 *   Public/Generated/KBVEQuestDBProtoTypes.h   (USTRUCTs, UHT-processed)
 *   Public/Generated/KBVEQuestDBProtoParse.h   (yyjson populate fns)
 *
 * Usage: node packages/data/codegen/gen-questdb-uecpp.mjs
 */
...
const DESCRIPTOR = resolve(__dirname, 'descriptors/questdb.binpb');
const OUT_DIR = resolve(
	__dirname,
	'../../../packages/unreal/KBVEQuestDB/Source/KBVEQuestDB/Public/Generated',
);
const PKG_PREFIX = 'quest.';
...
function fieldDefault(field, messages) {
	...
	return ''; // enum FString, message struct, list TArray, map TMap — default-construct
}
...
function emitStructs(ordered, messages) {
	...
	lines.push('// AUTO-GENERATED by gen-questdb-uecpp.mjs from quest/questdb.proto — DO NOT EDIT', '');
	lines.push('#include "CoreMinimal.h"');
	lines.push('#include "KBVEQuestDBProtoTypes.generated.h"', '');

	for (const msg of ordered) {
		lines.push('USTRUCT(BlueprintType)');
		lines.push(`struct KBVEQUESTDB_API ${structName(msg.name)}`);
		lines.push('{');
		lines.push('\tGENERATED_BODY()', '');
		for (const f of msg.fields) {
			const ueType = fieldUEType(f, messages);
			lines.push('\tUPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "KBVE|QuestDB")');
...
function emitParsers(ordered, messages) {
	const lines = [];
	lines.push('#pragma once', '');
	lines.push('// AUTO-GENERATED by gen-questdb-uecpp.mjs — DO NOT EDIT', '');
	lines.push('#include "CoreMinimal.h"');
	lines.push('#include "Generated/KBVEQuestDBProtoTypes.h"');
	lines.push('#include "KBVEYYJson.h"', '');
	lines.push('namespace KBVEQuestDBProto', '{');
...
writeFileSync(resolve(OUT_DIR, 'KBVEQuestDBProtoTypes.h'), emitStructs(ordered, messages) + '\n');
writeFileSync(resolve(OUT_DIR, 'KBVEQuestDBProtoParse.h'), emitParsers(ordered, messages) + '\n');
