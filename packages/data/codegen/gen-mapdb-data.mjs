...
import { syncCsharp } from './lib/csharp-sync.mjs';
...
function main() {
	...
	// C# classes so Unity's MapdbLoaderSystem stays in sync with the proto shape.
	//
	// The classes come from packages/proto, the same module the artifacts above
	// are encoded against. They used to be generated by protoc from
	// packages/data/proto, which is how Unity ended up parsing one schema's bytes
	// with another schema's classes. The runtime Google.Protobuf DLL ships via
	// nuget inside Assets/Packages.
	const csharpTargets = [
		{
			name: 'rareicon',
			dir: resolve(
	...
	for (const t of csharpTargets) {
		const copied = syncCsharp(
			kbveProtoDescriptor(),
			'kbve.map.v1.MapRegistry',
			t.dir,
		);
		if (copied) console.log(`Synced ${copied} C# protos for ${t.name} → ${t.dir}`);
	}
}
