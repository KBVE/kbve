...
-- ===============================================
-- /gh claim resolver — Discord snowflake → KBVE user_id +
-- linked GitHub login/id + profile.username. Single round-trip
-- used by discordsh-bot /gh claim. Backend/service_role only;
-- never exposed to anon/authenticated because it surfaces linked
-- identity metadata from auth.identities.
--
-- Semantics:
--   - Input gated by ^[0-9]{15,25}$ in a PL/pgSQL early-return guard,
--     so non-snowflake probes short-circuit to an empty result
--     without ever touching auth.identities.
--   - Empty result        = no auth.identities row for the snowflake
--                           (or invalid input shape).
--   - NULL github_login   = KBVE account exists, no GitHub identity
--                           linked. Bot surfaces a link prompt.
--   - NULL kbve_username  = no profile.username row yet (rare; user
--                           signed in but never set a handle).
--   - LATERAL join picks the latest GitHub identity per user using
--     (created_at DESC NULLS LAST, id DESC) as a fully deterministic
--     tie-breaker on collisions. Same tie-breaker on outer discord
--     lookup for safety against duplicate provider_id rows.
--   - github_login extracted via COALESCE(user_name, preferred_username,
--     login) since the claim name differs across GitHub OAuth scopes.
--
-- Promoted from dbmate migration
