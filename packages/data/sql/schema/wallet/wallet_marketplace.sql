...
--       → TABLE(...)  authenticated + service_role
--   public.proxy_market_my_ledger_readonly(
--       INTEGER, TIMESTAMPTZ, BIGINT,
--       wallet.source_kind[], wallet.currency_kind
--   )
--       → TABLE(...)  authenticated + service_role
--       Caller-scoped wallet.ledger history. Defaults source_kinds to
--       the marketplace trio (market_buy, market_sell, market_fee) so
--       the out-of-the-box response is the "market activity" view;
--       pass NULL to widen to the full ledger, or a narrower array to
--       scope further (e.g. ARRAY['market_sell']). p_currency is
--       optional (NULL = both currencies). Cursor pair
--       (before_created_at, before_id) is required together; partial
--       cursor raises 22023. Backed by
--       wallet_ledger_account_currency_idx. Body lives in migration
--       20260601173847_wallet_marketplace_my_ledger_proxy.
--
