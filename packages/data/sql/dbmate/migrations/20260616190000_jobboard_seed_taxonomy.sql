-- migrate:up
-- Seed jobboard verticals + taxonomy from the MDX source of truth.
-- Source: apps/kbve/astro-kbve/src/content/docs/project/jobboard.mdx (verticals block)
-- Generated by: nx run astro-kbve:sync:jobboard-taxonomy (see generated/jobboard-taxonomy-seed.sql)
-- Idempotent upsert: re-running syncs labels/status. Future taxonomy changes
-- land as a NEW migration (dbmate never re-runs applied ones) — regenerate the
-- seed, then paste the diff into a fresh migration.
...
-- migrate:down
-- Bootstrap seed — only safe before any gigs/talent reference these tags.
delete from jobboard.taxonomy where vertical_id in
    (select id from jobboard.verticals where slug = 'game-dev');
delete from jobboard.verticals where slug = 'game-dev';
