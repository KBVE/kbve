-- migrate:up
-- Profile consolidation Phase 0: Create central extended_profiles table
-- Issue: #13465
-- Schema source: packages/data/sql/schema/profile/extended_profiles.sql
-- Proto source: packages/data/proto/kbve/profile.proto (ExtendedProfile message)
...
comment on table profile.extended_profiles is
    'Central extended profile data (bio, location, avatar, links) shared across all apps. Single source of truth for user identity fields. See issue #13465.';
...
-- migrate:down
drop policy if exists "user_insert_own" on profile.extended_profiles;
drop policy if exists "user_update_own" on profile.extended_profiles;
drop policy if exists "user_read_own" on profile.extended_profiles;
drop policy if exists "service_role_full_access" on profile.extended_profiles;

drop trigger if exists extended_profiles_set_updated_at on profile.extended_profiles;

drop table if exists profile.extended_profiles;

drop function if exists profile.set_updated_at();
drop function if exists profile.is_valid_profile_links(jsonb);
