...
begin
    ...
    loop
        execute format('drop policy if exists jobboard_service on jobboard.%I', t);
    end loop;
end $$;

do $$ begin
    if exists (select 1 from pg_roles where rolname = 'jobboard') then
        execute 'drop owned by jobboard';
        execute 'drop role jobboard';
    end if;
end $$;
