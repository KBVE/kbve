...
public partial class IRCBridge : MonoBehaviour, IInitializable
{
    ...
    [EventfulProperty] ObservableList<IRCMessage> _jsMessagesList = new(); // ObservableList for efficient updates
    ...
    private void InitializeBindings()
    {
        ...
        _ircService.OnMessageReceived.Subscribe(message =>
        {
            ...
            // Add to ObservableList efficiently
    ...
    private void RefreshMessagesList()
    {
        if (_ircService == null)
        {
            ...
            // Clear and add status message to ObservableList
            JsMessagesList.Clear();
            JsMessagesList.Add(statusMessage);
            JsLatestMessage = statusMessage;
            return;
        }
        ...
        try
        {
            ...
            {
                // Efficiently update ObservableList with diff-based approach
                // Only add new messages not already in the list
                var currentCount = JsMessagesList.Count;

                // If we have more messages than needed, clear and rebuild
                if (messages.Length != currentCount)
                {
                    JsMessagesList.Clear();
                    foreach (var msg in messages)
                    {
                        JsMessagesList.Add(msg);
                    }
                ...
                Debug.Log($"[IRCBridge] Refreshed messages list with {JsMessagesList.Count} messages");
            }
        }
        catch (Exception ex)
        {
            ...
            JsMessagesList.Clear();
            JsMessagesList.Add(errorMessage);
            JsLatestMessage = errorMessage;
        }
    }

    /// <summary>
    /// Add a single message to the ObservableList and trim if needed
    /// </summary>
    private void AddMessageToList(IRCMessage message)
    {
        if (message == null) return;

        JsMessagesList.Add(message);
