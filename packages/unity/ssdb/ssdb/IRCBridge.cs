...
public partial class IRCBridge : MonoBehaviour, IInitializable
{
    ...
    private void LogDiagnosticWarning(string message)
    {
        if (enableDiagnostics)
        {
            Debug.LogWarning($"[IRCBridge] {message}");
        }
    }
    ...
    private void LogDiagnosticError(string message)
    {
        if (enableDiagnostics)
        {
            Debug.LogError($"[IRCBridge] {message}");
        }
    }
    ...
    public void Initialize()
    {
        ...
        {
            LogDiagnostic("Attempting to resolve IIRCService through injected container");
            if (_container.TryResolve<IIRCService>(out var ircService))
            {
                _ircService = ircService;
                LogDiagnostic("✓ Successfully resolved IIRCService through injected container");
            }
            else
            {
                LogDiagnosticWarning("✗ Failed to resolve IIRCService through injected container");
            }
        }
        ...
        {
            LogDiagnostic("Attempting to resolve SteamUserProfiles through injected container");
            if (_container.TryResolve<SteamUserProfiles>(out var steamProfiles))
            {
                _steamUserProfiles = steamProfiles;
                LogDiagnostic("✓ Successfully resolved SteamUserProfiles through injected container");
            }
            else
            {
                LogDiagnosticWarning("✗ Failed to resolve SteamUserProfiles through injected container");
            }
        }
#endif
        ...
        if (_ircService != null)
        {
            LogDiagnostic("IRC Service available, initializing with injected service");
            InitializeWithInjectedService().Forget();
        }
        else
        {
            LogDiagnosticWarning("IRC Service not available after VContainer initialization, will fall back to manual search");
        }
    }
    ...
    private async UniTaskVoid InitializeIRCConnection()
    {
        ...
        {
            LogDiagnosticWarning("IRC connection initialization was cancelled or timed out");
        }
        catch (Exception ex)
        {
            LogDiagnosticError($"Failed to initialize IRC connection: {ex.Message}");
        }
    }
    ...
    private async UniTask WaitForIRCServiceAsync()
    {
        ...
        try
        {
            LogDiagnostic("Starting async search for services...");
            ...
            {
                LogDiagnosticWarning("Timeout finding services");
                return;
            }
        ...
        await UniTask.WaitUntil(() =>
        {
            try
            {
                ...
                if (_container != null)
                {
                    ...
                    {
                        _ircService = ircService;
                        LogDiagnostic("Found IRCService through injected container");
                    }
                    ...
                    {
                        _steamUserProfiles = steamProfiles;
                        LogDiagnostic("Found SteamUserProfiles through injected container");
                    }
#endif
                ...
                if (ssdbScope?.Container != null)
                {
                    ...
                    {
                        _ircService = ircService;
                        LogDiagnostic("Found IRCService through SSDBLifetimeScope");
                    }
                    ...
                    {
                        _steamUserProfiles = steamProfiles;
                        LogDiagnostic("Found SteamUserProfiles through SSDBLifetimeScope");
                    }
#endif
                ...
                LogDiagnostic($"Searching through {allScopes.Length} LifetimeScopes for IRC services");
                ...
                foreach (var scope in allScopes)
                {
                    if (scope?.Container != null)
                    {
                        ...
                        {
                            _ircService = ircService;
                            LogDiagnostic($"Found IRCService in LifetimeScope: {scope.name}");
                        }
                        ...
                        {
                            _steamUserProfiles = steamProfiles;
                            LogDiagnostic($"Found SteamUserProfiles in LifetimeScope: {scope.name}");
                        }
#endif
        ...
        try
        {
            ...
            foreach (var scope in allScopes)
            {
                ...
                if (scope.Container != null)
                {
                    ...
                    LogDiagnostic($"    IRC Service: {(hasIRC ? "✓" : "✗")}");
                    LogDiagnostic($"    Steam Profiles: {(hasSteam ? "✓" : "✗")}");
                }
                else
                {
    ...
    private async UniTask WaitForSteamReadyAsync(CancellationToken cancellationToken)
    {
        ...
        LogDiagnostic("Waiting for Steam to be ready...");
        ...
        try
        {
            ...
            if (completedTaskIndex == 0) // Steam ready
            {
                ...
                JsSteamReady = true;
                LogDiagnostic($"Steam ready! Username: {JsSteamUsername}");
            }
            else
            {
                LogDiagnostic("Steam username not ready, proceeding without it");
                JsSteamReady = false;
            }
        }
        catch (OperationCanceledException)
        {
            LogDiagnostic("Steam wait cancelled");
            JsSteamReady = false;
        }
    }
#endif
    ...
    private void InitializeBindings()
    {
        ...
        if (_steamUserProfiles != null)
        {
            ...
            {
                LogDiagnostic($"Steam username changed: {value}");
        ...
        {
            LogDiagnostic($"IRC Connection state changed: {state}");
        ...
        {
            LogDiagnostic($"✓ Received IRC message: {message.nickname}: {message.message}");
            ...
            LogDiagnostic($"Updated message count to: {JsMessageCount}");
        }).AddTo(_disposables);
        
        // Subscribe to raw messages for debugging
        _ircService.OnRawMessageReceived.Subscribe(rawMessage => 
        {
            LogDiagnostic($"Raw IRC message received: {rawMessage}");
            ...
            {
                LogDiagnostic($"Updating message count from {JsMessageCount} to {newCount}");
        ...
        LogDiagnostic($"✓ Initial values set - Messages: {JsMessageCount}, Connected: {JsIsConnected}, Channel: {JsCurrentChannel}");
        
        // Subscribe to errors
        _ircService.OnError.Subscribe(error => 
        {
            JsLastError = error; // Auto-generated property
            LogDiagnosticWarning($"IRC Error: {error}");
        }).AddTo(_disposables);
    ...
    private void RefreshMessagesList()
    {
        if (_ircService == null)
        {
            LogDiagnosticWarning("Cannot refresh messages list - IRCService is null");
        ...
        try
        {
            ...
            else
            {
                ...
                LogDiagnostic($"Refreshed messages list with {JsMessagesList.Count} formatted messages");
            }
        }
        catch (Exception ex)
        {
            LogDiagnosticError($"Error refreshing messages list: {ex.Message}");
    ...
    public async UniTask<bool> JsConnectAsync()
    {
        ...
        if (_ircService == null)
        {
            LogDiagnosticWarning("IRCService not found, attempting to find it...");
            ...
            if (_ircService == null)
            {
                LogDiagnosticError("IRCService still not available after search - cannot connect");
                JsLastError = "IRCService not available";
                return false;
            }
            else
            {
                LogDiagnostic("Found IRCService, proceeding with connection");
        ...
        try
        {
            LogDiagnostic($"Connecting to IRC server...");
            LogDiagnostic($"Current IRCService state - IsConnected: {_ircService.IsConnectedValue}");
            var result = await _ircService.ConnectAsync(this.GetCancellationTokenOnDestroy());
            LogDiagnostic($"Connection result: {result}");
            return result;
        }
        catch (Exception ex)
        {
            LogDiagnosticError($"Connection failed: {ex.Message}");
    ...
    public async UniTask<bool> JsConnectWithConfigAsync(string nickname, string channel = null)
    {
        if (_ircService == null)
        {
            LogDiagnosticError("IRCService not available");
            return false;
        }
        ...
        {
            LogDiagnosticError($"Connection with config failed: {ex.Message}");
            JsLastError = ex.Message;
            return false;
        }
    }
    ...
    public void JsDisconnect()
    {
        if (_ircService == null)
        {
            ...
            if (lifetimeScope?.Container != null)
            {
                ...
                {
                    _ircService = ircService;
                    LogDiagnostic("Found IRCService for disconnect");
                    InitializeBindings();
                }
            }
            ...
            if (_ircService == null)
            {
                LogDiagnosticError("IRCService still not available - cannot disconnect");
                JsLastError = "IRCService not available";
                return;
            }
        }
        
        LogDiagnostic($"Disconnecting from IRC - Current state: {_ircService.IsConnectedValue}");
        _ircService.Disconnect();
        LogDiagnostic("Disconnected from IRC");
    }
    ...
    public async UniTask<bool> JsReconnectAsync()
    {
        ...
        if (_ircService == null)
        {
            ...
            if (_ircService == null)
            {
                LogDiagnosticError("IRCService still not available after search - cannot reconnect");
                JsLastError = "IRCService not available";
                return false;
            }
            else
            {
                LogDiagnostic("Found IRCService, proceeding with reconnection");
                // Re-initialize bindings since we just found the service
                InitializeBindings();
            }
        }
        ...
        try
        {
            LogDiagnostic("Starting reconnection process using IRCService.ReconnectAsync()...");
            LogDiagnostic($"Current IRCService state - IsConnected: {_ircService.IsConnectedValue}");
            ...
            if (result)
            {
                LogDiagnostic("Reconnection successful");
            }
            else
            {
                LogDiagnosticWarning("Reconnection failed");
            }
        ...
        {
            LogDiagnosticError($"Reconnection error: {ex.Message}");
            JsLastError = ex.Message;
            return false;
        }
    }
    ...
    private async UniTask TryFindIRCServiceAsync()
    {
        try
        {
            ...
            if (_container != null)
            {
                LogDiagnostic("Using injected container to resolve services");
                
                if (_ircService == null && _container.TryResolve<IIRCService>(out var ircService))
                {
                    _ircService = ircService;
                    LogDiagnostic("Successfully found IRCService through injected container");
                }
                ...
                {
                    _steamUserProfiles = steamProfiles;
                    LogDiagnostic("Also found SteamUserProfiles through injected container");
                }
#endif
            }
            ...
            if (_ircService == null)
            {
                ...
                if (ssdbScope?.Container != null)
                {
                    LogDiagnostic("Using SSDBLifetimeScope to resolve services");
                    
                    if (ssdbScope.Container.TryResolve<IIRCService>(out var ircService))
                    {
                        _ircService = ircService;
                        LogDiagnostic("Successfully found IRCService through SSDBLifetimeScope");
                        ...
                        {
                            _steamUserProfiles = steamProfiles;
                            LogDiagnostic("Also found SteamUserProfiles through SSDBLifetimeScope");
                        }
#endif
                    }
                }
        ...
        {
            LogDiagnosticError($"Error finding IRCService: {ex.Message}");
        }
    }
    ...
    public void JsSendMessage(string message)
    {
        if (_ircService != null && !string.IsNullOrEmpty(JsCurrentChannel))
        {
            ...
            LogDiagnostic($"Sent message to {JsCurrentChannel}: {message}");
            ...
            LogDiagnostic($"Added sent message to local history. Total messages: {JsMessageCount}");
        }
        else
        {
            LogDiagnosticWarning("Cannot send message - no channel joined");
        }
    }
    ...
    public void JsSendMessageToChannel(string channel, string message)
    {
        if (_ircService != null)
        {
            ...
            LogDiagnostic($"Sent message to {channel}: {message}");
    ...
    public void JsSendPrivateMessage(string nickname, string message)
    {
        if (_ircService != null)
        {
            ...
            LogDiagnostic($"Sent private message to {nickname}: {message}");
        }
    }
    ...
    public void JsSendRawCommand(string command)
    {
        if (_ircService != null)
        {
            ...
            LogDiagnostic($"Sent raw command: {command}");
        }
    }
    ...
    public void JsJoinChannel(string channel)
    {
        if (_ircService != null)
        {
            ...
            LogDiagnostic($"Joining channel: {channel}");
        }
    }
    ...
    public void JsLeaveChannel(string channel)
    {
        if (_ircService != null)
        {
            ...
            LogDiagnostic($"Leaving channel: {channel}");
        }
    }
    ...
    public void JsLeaveCurrentChannel()
    {
        if (_ircService != null && !string.IsNullOrEmpty(JsCurrentChannel))
        {
            ...
            LogDiagnostic($"Left channel: {JsCurrentChannel}");
        }
    }
    ...
    public void JsSetNickname(string nickname)
    {
        ...
        LogDiagnostic($"Setting nickname: {nickname}");
    ...
    {
        LogDiagnostic($"JsGetRecentMessages called with count={count}");
        
        if (_ircService == null)
        {
            LogDiagnosticError("IRCService is null in JsGetRecentMessages!");
            return new string[0];
        }
        
        var messages = _ircService.GetRecentMessages(count);
        LogDiagnostic($"IRCService returned {messages.Length} messages");
        
        if (messages.Length == 0)
        {
            // Additional diagnostics
            LogDiagnostic($"No messages found. Service stats:");
            LogDiagnostic($"  - Total message count: {_ircService.GetMessageCount()}");
            LogDiagnostic($"  - Is connected: {_ircService.IsConnectedValue}");
            LogDiagnostic($"  - Current channel: {_ircService.CurrentChannelValue}");
            LogDiagnostic($"  - Pending messages: {_ircService.PendingMessagesValue}");
        }
        ...
        for (int i = 0; i < messages.Length; i++)
        {
            ...
            LogDiagnostic($"Message {i}: {formattedMessages[i]}");
        }
    ...
    public async UniTask<string> JsUsernameFromSteamID(string steamId)
    {
        if (_steamUserProfiles == null)
        {
            LogDiagnosticWarning("Steam integration not available");
            return null;
        }
        
        if (string.IsNullOrEmpty(steamId))
        {
            LogDiagnosticWarning("Empty Steam ID provided");
            return null;
        }
        
        try
        {
            LogDiagnostic($"Looking up username for Steam ID: {steamId}");
            ...
            {
                LogDiagnostic($"Found username for {steamId}: {username}");
            }
            else
            {
                LogDiagnostic($"Could not find username for {steamId}");
            }
        ...
        {
            LogDiagnosticError($"Error looking up Steam ID {steamId}: {ex.Message}");
            return null;
        }
    }
    ...
    public void JsClearSteamUsernameCache()
    {
        if (_steamUserProfiles != null)
        {
            ...
            LogDiagnostic("Steam username cache cleared");
        }
    }
