...
public partial class IRCBridge : MonoBehaviour, IInitializable
{
    ...
    [EventfulProperty] List<IRCMessage> _jsMessagesList = new(); // Live reactive message list for OneJS
    [EventfulProperty] List<IRCMessage> _jsNewMessages = new(); // NEW messages since last update only
    ...
    private void InitializeBindings()
    {
        ...
        _ircService.OnMessageReceived.Subscribe(message => 
        {
            ...
            // Add to the new messages list for incremental updates
            var newMessagesList = new List<IRCMessage> { message };
            JsNewMessages = newMessagesList;
            
            // Only refresh full messages list on initial connection
            // RefreshMessagesList(); // Commented out to reduce processing
            
            Debug.Log($"[IRCBridge] Updated message count to: {newCount}");
        }).AddTo(_disposables);
    ...
    private void RefreshMessagesList()
    {
        if (_ircService == null) 
        {
            ...
            // Create dummy message for status
            var statusMessage = new IRCMessage
            {
            ...
            JsMessagesList = new List<IRCMessage> { statusMessage };
            return;
        }

        try
        {
            // Get recent messages from service (limit to reasonable number for UI performance)
            var messages = _ircService.GetRecentMessages(50);
            var messagesList = new List<IRCMessage>();
            ...
            if (messages.Length == 0)
            {
                ...
                var statusMessage = new IRCMessage
                {
                    message = _ircService.IsConnectedValue ? "Connected to IRC - No messages yet..." : "Use Connect to join IRC chat",
                    command = "STATUS",
                    timestamp = DateTime.Now,
                    isServerMessage = true
                };
                messagesList.Add(statusMessage);
            }
            else
            {
                // Add messages directly - no formatting needed in bridge
                messagesList.AddRange(messages);
                Debug.Log($"[IRCBridge] Refreshed messages list with {messagesList.Count} raw messages");
            }
            
            // Update the reactive property - this will trigger OneJS UI updates
            JsMessagesList = messagesList;
        }
        catch (Exception ex)
        {
            Debug.LogError($"[IRCBridge] Error refreshing messages list: {ex.Message}");
            var errorMessage = new IRCMessage
            {
            ...
            JsMessagesList = new List<IRCMessage> { errorMessage };
        }
    }
