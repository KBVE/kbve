...
use crate::entity::regex::{
    ...
    extract_email_from_regex, extract_general_input_from_regex, extract_github_username_from_regex,
    extract_hex_code_from_regex, extract_instagram_username_from_regex,
    extract_markdown_image_href_link_from_regex, extract_markdown_standalone_href_link_from_regex,
    extract_service_from_regex, extract_ulid_from_regex, extract_unsplash_photo_id_from_regex,
...
impl<T, E> Default for ValidatorBuilder<T, E>
where
    T: Sync + Send + Default + Clone + 'static,
    E: Send + 'static,
{
    fn default() -> Self {
        Self::new()
    ...
    E: Send + 'static,
{
    ...
    where
        F: Fn(T) -> Result<T, E> + Sync + Send + 'static,
    {
...
impl ValidatorBuilder<String, String> {
    ...
    pub fn password(&mut self) -> &mut Self {
        self.add_rule(
            |s: String| match validate_only_input_password_without_regex(&s) {
                Ok(()) => Ok(s),
