...
impl VectorSpace {
    pub fn new(provider: impl Into<String>, model: impl Into<String>, dim: usize) -> VectorSpace {
        VectorSpace { provider: provider.into(), model: model.into(), dim }
    }
}
...
impl VectorFilter {
    pub fn kind(ref_kind: impl Into<String>) -> VectorFilter {
        VectorFilter { ref_kind: Some(ref_kind.into()) }
    }
}
...
fn check_dim(space: &VectorSpace, vec: &[f32]) -> Result<()> {
    if vec.len() != space.dim {
        return Err(EmbedError::VectorDim { expected: space.dim, actual: vec.len() });
    }
    Ok(())
}
...
impl TopK {
    fn new(k: usize) -> TopK {
        TopK { k, items: Vec::with_capacity(k.saturating_add(1)) }
    }
...
impl EmbedDb {
    ...
    pub async fn vector_search(
        ...
        let mut rows = match kind.as_deref() {
            ...
            None => self.conn().query(SEARCH_SQL, (space.model.as_str(),)).await?,
        };
        ...
        while let Some(row) = rows.next().await? {
            ...
            if stored.len() != space.dim {
                return Err(EmbedError::VectorDim { expected: space.dim, actual: stored.len() });
            }
            top.push(VectorHit {
    ...
    pub async fn vector_delete_model(&self, model: &str) -> Result<u64> {
        self.execute("DELETE FROM _embeddb_vectors WHERE model = ?", (model,)).await
    }

    pub async fn vector_count(&self, model: &str) -> Result<i64> {
        let mut rows = self
            .conn()
            .query("SELECT count(*) FROM _embeddb_vectors WHERE model = ?", (model,))
            .await?;
        let mut n = 0_i64;
...
const SEARCH_SQL: &str =
    "SELECT ref_kind, ref_id, vec FROM _embeddb_vectors WHERE model = ?";
...
fn value_text(v: turso::Value) -> Result<String> {
    match v {
        ...
        other => Err(EmbedError::VectorBlob(format!("expected text column, got {:?}", other))),
    }
}
...
mod tests {
    ...
    fn normalize_rejects_zero_vector() {
        assert!(matches!(normalize(&[0.0, 0.0]).unwrap_err(), EmbedError::VectorZeroNorm));
    }
    ...
    fn topk_zero_returns_empty() {
        ...
        top.push(VectorHit { ref_kind: "m".into(), ref_id: "a".into(), score: 1.0 });
    ...
    async fn upsert_then_search_ranks_by_cosine() {
        ...
        let s = space();
        db.vector_upsert(&s, "msg", "a", &[1.0, 0.0, 0.0]).await.unwrap();
        db.vector_upsert(&s, "msg", "b", &[0.0, 1.0, 0.0]).await.unwrap();
        db.vector_upsert(&s, "msg", "c", &[0.9, 0.1, 0.0]).await.unwrap();
        let hits = db.vector_search(&s, &[1.0, 0.0, 0.0], 2, None).await.unwrap();
        assert_eq!(hits.len(), 2);
    ...
    async fn search_scores_are_magnitude_invariant() {
        ...
        let s = space();
        db.vector_upsert(&s, "msg", "small", &[1.0, 0.0, 0.0]).await.unwrap();
        db.vector_upsert(&s, "msg", "large", &[100.0, 0.0, 0.0]).await.unwrap();
        let hits = db.vector_search(&s, &[5.0, 0.0, 0.0], 2, None).await.unwrap();
    ...
    async fn upsert_replaces_same_ref_and_model() {
        ...
        let s = space();
        db.vector_upsert(&s, "msg", "a", &[1.0, 0.0, 0.0]).await.unwrap();
        db.vector_upsert(&s, "msg", "a", &[0.0, 1.0, 0.0]).await.unwrap();
        assert_eq!(db.vector_count("test-model").await.unwrap(), 1);
        let hits = db.vector_search(&s, &[0.0, 1.0, 0.0], 1, None).await.unwrap();
        assert!((hits[0].score - 1.0).abs() < 1e-5);
    }
    ...
    async fn models_do_not_mix_in_search() {
        ...
        db.vector_upsert(&a, "msg", "x", &[1.0, 0.0, 0.0]).await.unwrap();
        db.vector_upsert(&b, "msg", "y", &[1.0, 0.0, 0.0]).await.unwrap();
        let hits = db.vector_search(&a, &[1.0, 0.0, 0.0], 10, None).await.unwrap();
    ...
    async fn same_ref_can_hold_one_row_per_model() {
        ...
        let a = VectorSpace::new("local", "model-a", 3);
        let b = VectorSpace::new("api", "model-b", 3);
        db.vector_upsert(&a, "msg", "same", &[1.0, 0.0, 0.0]).await.unwrap();
        db.vector_upsert(&b, "msg", "same", &[0.0, 1.0, 0.0]).await.unwrap();
    ...
    async fn filter_restricts_to_ref_kind() {
        ...
        let s = space();
        db.vector_upsert(&s, "msg", "a", &[1.0, 0.0, 0.0]).await.unwrap();
        db.vector_upsert(&s, "doc", "b", &[1.0, 0.0, 0.0]).await.unwrap();
        let hits = db
    ...
    async fn dim_mismatch_errors_on_upsert_and_search() {
        ...
        let s = space();
        let err = db.vector_upsert(&s, "msg", "a", &[1.0, 0.0]).await.unwrap_err();
        assert!(matches!(err, EmbedError::VectorDim { expected: 3, actual: 2 }));
        let err = db.vector_search(&s, &[1.0, 0.0], 1, None).await.unwrap_err();
        assert!(matches!(err, EmbedError::VectorDim { expected: 3, actual: 2 }));
    }
    ...
    async fn zero_vector_rejected_on_upsert() {
        ...
        let s = space();
        let err = db.vector_upsert(&s, "msg", "a", &[0.0, 0.0, 0.0]).await.unwrap_err();
    ...
    async fn search_on_empty_store_returns_empty() {
        ...
        let hits = db.vector_search(&space(), &[1.0, 0.0, 0.0], 5, None).await.unwrap();
    ...
    async fn search_k_larger_than_corpus_returns_all() {
        ...
        db.vector_upsert(&s, "msg", "a", &[1.0, 0.0, 0.0]).await.unwrap();
        db.vector_upsert(&s, "msg", "b", &[0.0, 1.0, 0.0]).await.unwrap();
        let hits = db.vector_search(&s, &[1.0, 0.0, 0.0], 50, None).await.unwrap();
        assert_eq!(hits.len(), 2);
    }
    ...
    async fn search_survives_writer_reader_interleave() {
        ...
        for i in 0..25 {
            ...
            db.vector_upsert(&s, "msg", &i.to_string(), &v).await.unwrap();
        }
        ...
        let hits = db.vector_search(&s, &[1.0, 0.0, 0.0], 40, None).await.unwrap();
