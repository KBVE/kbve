...
fn main() {
    ...
    // Walk up to the workspace root (4 levels: bevy_chat → bevy → rust → packages → root)
    ...
    let chat_proto = proto_root.join("kbve/chat.proto");
    ...
    prost_build::Config::new()
        .out_dir(&out_dir)
        .compile_protos(
            &[chat_proto.to_str().unwrap()],
            &[
                proto_root.join("kbve").to_str().unwrap(),
                proto_root.to_str().unwrap(),
            ],
        )
        .expect("Failed to compile chat.proto");
}
