...
fn pick_enemy_target(session: &SessionState, actor: PlayerId) -> PlayerId {
    if session.mode == SessionMode::Solo {
        return actor;
    }

    let alive_players: Vec<PlayerId> = session
        .players
        .iter()
        .filter(|(_, p)| p.alive)
        .map(|(uid, _)| *uid)
...
fn validate_actor(session: &SessionState, actor: PlayerId) -> Result<(), String> {
...
fn maybe_first_strike(session: &mut SessionState, actor: PlayerId) -> (Vec<String>, bool) {
...
fn resolve_combat_turn_party(
    ...
    let actions: Vec<(PlayerId, GameAction)> = session.pending_actions.drain().collect();
    ...
    let mut bridge_actions: Vec<(PlayerId, battle_bridge::PlayerAction)> = Vec::new();
...
fn handle_enemy_deaths(session: &mut SessionState, actor: PlayerId) -> Vec<String> {
    let mut logs = Vec::new();
    let mut rng = rand::rng();
...
fn single_enemy_turn(
    ...
    let all_uids: Vec<PlayerId> = session.players.keys().copied().collect();
    for uid in all_uids {
        let player = session.player_mut(uid);
...
#[allow(dead_code)] // Used only in tests; production uses bevy_battle ECS
fn enemy_turns(session: &mut SessionState, default_target: PlayerId) -> Vec<String> {
    let mut logs = Vec::new();
...
fn resolve_flee(session: &mut SessionState, actor: PlayerId) -> Vec<String> {
    let mut logs = Vec::new();
...
fn arrive_at_tile(session: &mut SessionState, pos: MapPos) -> Vec<String> {
    ...
    let alive_ids: Vec<PlayerId> = session
        .players
        .iter()
        .filter(|(_, p)| p.alive)
        .map(|(id, _)| *id)
...
fn apply_buy(session: &mut SessionState, item_id: &str, actor: PlayerId) -> Result<String, String> {
    let offer = session
        .room
        .merchant_stock
        .iter()
...
#[allow(dead_code)] // Used only in tests; production uses bevy_battle ECS
fn effective_accuracy(session: &SessionState, actor: PlayerId) -> f32 {
...
mod tests {
    ...
    fn wrong_actor_rejected() {
        let mut session = test_session();
        let result = apply_action(&mut session, GameAction::Explore, PlayerId::new(999));
        assert!(result.is_err());
