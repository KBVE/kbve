...
pub struct SessionState {
    ...
    pub show_items: bool,
    #[serde(skip)]
    pub pending_actions: HashMap<PlayerId, GameAction>,
    pub map: MapState,
    pub show_map: bool,
...
impl SessionState {
    ...
    pub fn player_mut(&mut self, uid: PlayerId) -> &mut PlayerState {
        self.players
            .get_mut(&uid)
            .expect("player must exist in session")
    }
    ...
    pub fn roster(&self) -> Vec<(PlayerId, &PlayerState)> {
...
///
/// Behind the `testing` feature so front-end test suites can build sessions
/// without the helper landing in a production build.
