...
pub async fn update_reset_token(
    ...
    task::spawn_blocking(move || {
        ...
        diesel::update(auth::table.filter(auth::email.eq(email)))
            .set((
                auth::password_reset_token.eq(token),
...
pub async fn handle_recovery_process(
    email: String,
    pool: Arc<Pool>,
...
pub async fn process_password_recovery(
    ...
    .await
    {
        ...
        Ok(false) => create_custom_response(
