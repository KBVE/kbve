...
use crate::runes::{
    ...
    UpdateProfileSchema, WizardResponse,
};
...
pub async fn auth_player_register(
    ...
    match crate::guild::hazardous_boolean_email_exist(body.email.clone(), pool.clone()).await {
        ...
        Err(e) => {
            return create_error_response("x-kbve", "email_exists", e);
        }
    }
    ...
    match crate::guild::hazardous_create_user(body.username.clone(), pool.clone()).await {
        ...
        Err(_e) => {
            return create_error_response("x-kbve", "process_user_failed", "process_user_failed");
            //return spellbook_error!(axum::http::StatusCode::BAD_REQUEST, &e);
        }
    }
    ...
    //	[#]	Obtain UUID
    //	!	Migration to ULID
    let ulid = match crate::guild::task_fetch_userid_by_username(
        body.username.clone(),
        pool.clone(),
    )
    .await
    {
        Ok(value) => value,
        Err(_) => {
            return create_error_response("x-kbve", "process_ulid_failed", "process_uild_failed");
    ...
    //	[&] Create Auth
    match crate::guild::hazardous_create_auth_from_ulid(
    ...
    .await
    {
        Ok(true) => {}
        Ok(false) => {
            return create_error_response("x-kbve", "process_auth_failed", "process_auth_failed");

            //return spellbook_error!(axum::http::StatusCode::BAD_REQUEST, "process-auth-failed");
        }
        Err(e) => {
            return create_error_response("x-kbve", "process_ulid_failed", e);
    ...
    //	[&]	Create Profile
    match crate::guild::hazardous_create_profile_from_ulid(
        body.username.clone(),
        ulid.clone(),
        pool.clone(),
    )
    .await
    {
        ...
        Err(e) => {
            return create_error_response("x-kbve", "process_profile_failed", e);
    ...
    match crate::guild::hazardous_boolean_email_exist(body.email.clone(), pool.clone()).await {
        ...
        Err(e) => {
            return create_error_response("x-kbve", "internal_process", e);
...
pub async fn auth_player_login(
    ...
    let clean_email = match crate::utility::sanitize_email(&body.email) {
        ...
        Err(error_message) => {
            ...
            let header_value =
                axum::http::HeaderValue::from_str(&format!("shield_{}", error_message)).unwrap();
    ...
    match crate::utility::validate_password(&body.password) {
        ...
        Err(_) => {
            ...
            let header_name = axum::http::header::HeaderName::from_str("x-kbve").unwrap();
            let header_value =
                axum::http::HeaderValue::from_str(&format!("shield_{}", "invalid_password"))
                    .unwrap();
    ...
    let auth_verification_data = match auth::table
        ...
        .first::<AuthVerificationSchema>(&mut conn)
    {
        ...
        Err(_) => {
            ...
            let header_name = axum::http::header::HeaderName::from_str("x-kbve").unwrap();
            let header_value =
                axum::http::HeaderValue::from_str(&format!("shield_{}", "auth_error")).unwrap();
    ...
    let userid_ulid_string =
        match crate::utility::convert_ulid_bytes_to_string(&auth_verification_data.userid) {
            ...
            Err(_e) => {
                ...
                let header_name = axum::http::header::HeaderName::from_str("x-kbve").unwrap();
                let header_value =
                    axum::http::HeaderValue::from_str(&format!("shield_{}", "invalid_ulid"))
                        .unwrap();
    ...
    let operational_vaild_password = match PasswordHash::new(&db_user_hash_password) {
        Ok(process_hash) => Argon2::default()
    ...
    if !operational_vaild_password {
        ...
        let header_name = axum::http::header::HeaderName::from_str("x-kbve").unwrap();
        let header_value =
            axum::http::HeaderValue::from_str(&format!("shield_{}", "invalid_password")).unwrap();
    ...
    let jwt_secret = match spellbook_get_global!("jwt_secret", "invalid_jwt") {
        ...
        Err(_) => {
            ...
            let header_name = axum::http::header::HeaderName::from_str("x-kbve").unwrap();
            let header_value =
                axum::http::HeaderValue::from_str(&format!("shield_{}", "invalid_jwt")).unwrap();
...
pub async fn auth_jwt_update_profile(
    ...
    let clean_ulid_bytes =
        match crate::utility::convert_ulid_string_to_bytes(clean_user_ulid_string) {
            Ok(bytes) => bytes,
            Err(error_message) => {
                // Handle the error, e.g., return an appropriate response
                return (
...
pub async fn graceful(
    ...
    let privatedata = match jsonwebtoken::decode::<TokenRune>(
        &token,
            ...
            Some(action) => match action.as_str() {
                "deploy" => {
                    ...
                    let response: axum::response::Response =
                        shieldwall_action_portainer_stack_deploy()
                            .await
                            .into_response();
                    response
                }
...
pub async fn shieldwall_action_portainer_stack_deploy() -> impl IntoResponse {
        ...
        match global_map.get(portainer_stack_url_from_map) {
            ...
            None => {
                return axum::response::Response::builder()
                    ...
                    .body(
                        Json(serde_json::json!({"error": "URL not found in global map"}))
                            .into_response()
                            .into_body(),
                    )
                    .unwrap();
    ...
    Ok(response) => {
      match response.text().await {
        Ok(text) => (StatusCode::OK, Json(serde_json::json!({ "response": text }))).into_response(),
        Err(e) =>
          (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(
