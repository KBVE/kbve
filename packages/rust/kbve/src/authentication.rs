...
pub async fn auth_player_register(
  ...
  //	!	Migration to ULID
  let ulid = match
    crate::guild::task_fetch_userid_by_username(body.username.clone(), pool.clone()).await
  {
    Ok(value) => value,
    Err(_) => {
      return create_error_response("x-kbve",
...
pub async fn auth_player_login(
  ...
  let userid_ulid_string = match
    crate::utility::convert_ulid_bytes_to_string(&auth_verification_data.userid)
  {
...
pub async fn graceful_jwt_profile(
  ...
  (axum::http::StatusCode::OK, axum::Json(serde_json::to_value(privatedata.claims).unwrap()))
}
...
pub async fn graceful<B>(
  ...
  let jwt_secret = match spellbook_get_global!("jwt_secret", "invalid_jwt") {
    Ok(secret) => secret,
    Err(_) => {
      return (StatusCode::UNAUTHORIZED, Json(json!({"error": "invalid_jwt"}))).into_response();
    }
  };
