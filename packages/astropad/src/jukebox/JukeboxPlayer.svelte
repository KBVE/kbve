...
<script lang="ts">
  ...
  async function loadVideoScripts() {
    try {
      ...
      await new Promise((resolve, reject) => {
        const script = document.createElement('script');
        script.src = 'https://unpkg.com/videojs-youtube/dist/Youtube.min.js';
        script.onload = resolve;
        script.onerror = () =>
  ...
  function initializePlayer() {
    if (typeof videojs !== 'undefined' && videojs.getTech('youtube')) {
      ...
      if (
        allVideos.length === 0 &&
        (!queryVideoId || !isValidYouTubeId(queryVideoId))
      )
        return; // Prevent initialization if no videos are available or the query ID is invalid

      const initialVideoId =
        queryVideoId && isValidYouTubeId(queryVideoId)
          ? queryVideoId
          : allVideos[Math.floor(Math.random() * allVideos.length)]; // Use query parameter if valid or select a random index
  ...
  function getActiveVideos() {
    ...
    $musicData$.items.forEach((item) => {
      ...
      const isActive = item.tags.some((tag) => $tagSetting$[tag] === true);
