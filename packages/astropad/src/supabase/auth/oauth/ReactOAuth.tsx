  ...
  return (
    <button
      ...
      className={cn(
        ...
        "flex items-center justify-center gap-2 w-full py-2 px-4 min-h-[44px] rounded-lg font-semibold shadow-md transition-all duration-300",
        "bg-[#24292f] hover:bg-[#2f363d] text-white",
        "border border-[var(--sl-color-gray-5,#383838)] hover:border-[var(--sl-color-accent,#06b6d4)]",
        "hover:shadow-lg hover:shadow-[var(--sl-color-accent,#06b6d4)]/20 hover:-translate-y-0.5 active:translate-y-0",
    ...
    >
      ...
      {isLoading ? (
        ...
        <>
          <svg className="w-5 h-5 transition-transform duration-300 group-hover:scale-110 drop-shadow-sm" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    ...
    <div className="relative w-full">
      <button
        ...
        className={cn(
          ...
          "border border-[var(--sl-color-gray-5,#383838)]",
          isDisabled 
            ? "opacity-50 cursor-not-allowed border-[var(--sl-color-gray-4,#8b8b8b)]" 
            : "hover:border-[var(--sl-color-accent,#06b6d4)] hover:from-[#8A3FD9] hover:to-[#00D4AA] hover:shadow-lg hover:shadow-[var(--sl-color-accent,#06b6d4)]/20 hover:-translate-y-0.5 active:translate-y-0"
        )}
        type="button"
      >
        <div className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/10 to-transparent group-hover:translate-x-full transition-transform duration-500" />
        ...
        {loading ? (
          ...
          <>
            <svg className="w-5 h-5 transition-transform duration-300 group-hover:scale-110 drop-shadow-sm" fill="currentColor" viewBox="0 0 397.7 311.7" aria-hidden="true">
              <defs>
            ...
            <span className="relative z-10">Continue with Solana</span>
          </>
        )}
      </button>
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      <div className="text-center mb-6">
        <h2 className="text-xl font-bold mb-2" style={{ color: 'var(--sl-color-white, #fff)' }}>{title}</h2>
        <p className="text-sm" style={{ color: 'var(--sl-color-gray-3, #8b8b8b)' }}>{description}</p>
      </div>

      {error && (
        <div 
          className="p-3 rounded-lg text-center text-sm mb-4 border bg-red-500/10 border-red-500/20 text-red-400"
          style={{
            backgroundColor: 'var(--sl-color-red-high, rgba(239, 68, 68, 0.1))',
            borderColor: 'var(--sl-color-red-low, rgba(239, 68, 68, 0.2))',
            color: 'var(--sl-color-red-low, #f87171)'
          }}
        >
          {error}
        </div>
      ...
      {success && (
        <div 
          className="p-3 rounded-lg text-center text-sm mb-4 border bg-green-500/10 border-green-500/20 text-green-400"
          style={{
            backgroundColor: 'var(--sl-color-green-high, rgba(34, 197, 94, 0.1))',
            borderColor: 'var(--sl-color-green-low, rgba(34, 197, 94, 0.2))',
            color: 'var(--sl-color-green-low, #4ade80)'
          }}
        >
          {success}
        </div>
