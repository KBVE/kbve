...
<script lang="ts">
	...
	export const redirect = false;                                    // Exporting 'redirect', initially set to false.
	...
	$: if (mounted && loaded) {
		widgetID = hcaptcha.render(`h-captcha-${id}`, {               // Rendering the captcha widget.
			sitekey,
	...
	let usernameRegex = new RegExp(/^[a-z0-9]+$/i);                  // Regular expression for validating usernames.
