...
@click.command()
@click.option(
    "--scope", type=click.Choice(["monorepo", "app", "package"]), required=True, help="Scope of graph to build"
)
...
def export_main(input: str, output: str):
    ...
    try:
        ...
        # Add metadata
        web_export = {
            "metadata": {"source": "graphify", "version": "0.1.0", "type": "semantic-knowledge-graph"},
            "graph": graph_data,
        }
