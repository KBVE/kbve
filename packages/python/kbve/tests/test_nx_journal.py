...
def test_journal_build_scaffolds_year(tmp_path):
    ...
    text = (journal_root(root) / "07-19.mdx").read_text(encoding="utf-8")
...
def test_journal_build_idempotent(tmp_path):
    ...
    after_first = (journal_root(root) / "07-19.mdx").read_text(encoding="utf-8")

    result2 = builder.build_one("journal")
    assert result2.skipped is True
    after_second = (journal_root(root) / "07-19.mdx").read_text(encoding="utf-8")
