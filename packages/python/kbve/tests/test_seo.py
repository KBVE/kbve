...
def test_desc_too_short_flags_warn():
    p = _page("api", {"title": "KBVE API Layer", "description": "short", "tags": ["a"], "sem": 1})
    findings = _run(p, build_ctx([p]))
    assert findings["desc-length"].severity == "warn"
...
def test_good_page_passes():
    p = _page(
        "edge",
        {
            ...
            "description": (
                "Cloudflare edge worker that fronts kbve.com, handles "
...
def test_missing_sem_is_info():
    p = _page("api", {"title": "KBVE API Layer", "description": "x" * 100, "tags": ["a"]})
    findings = _run(p, build_ctx([p]))
    assert findings["sem-tracked"].severity == "info"


def test_duplicate_description_is_error():
    a = _page("a", {"title": "Alpha Project", "description": "y" * 100, "tags": ["t"], "sem": 1})
    b = _page("b", {"title": "Beta Project", "description": "y" * 100, "tags": ["t"], "sem": 1})
    ctx = build_ctx([a, b])
    findings = _run(a, ctx)
