...
from kbve.content.activity import aggregate, since_day, since_iso
...
def test_aggregate_leaderboard():
    ...
    agg = aggregate(commits, {"total": 4, "items": []}, {"total": 2}, {"total": 1}, "2026-07-12T00:00:00Z", 7)
    assert agg["commits"]["total"] == 3
    assert agg["commits"]["authors"] == 2
    assert agg["commits"]["leaderboard"][0] == {"author": "h0lybyte", "commits": 2}
    ...
    seam = {
        "activity": {
            ...
            "prs": {
                "total": 1,
                "items": [
                    {"number": 5, "title": "PR", "user": {"login": "h0lybyte"}, "html_url": "https://github.com/x/5"}
                ],
            },
