...
def security_main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--mdx-out", help="Path to write Starlight MDX output")
    parser.add_argument("--json-out", help="Path to write structured JSON output")
    parser.add_argument("--timestamp", required=True, help="ISO 8601 timestamp for the report")
    args = parser.parse_args(argv)

    if not args.mdx_out and not args.json_out:
        print("Error: at least one of --mdx-out or --json-out required", file=sys.stderr)
        return 1
...
def graph_main(argv: list[str] | None = None) -> int:
    ...
    print(f"Generated {args.output} — {len(graph.nodes)} projects, {len(graph.edges)} edges")
    return 0
...
def alerts_main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--out", required=True, help="Where to write the filtered JSON array.")
    parser.add_argument("--per-page", type=int, default=100, help="Pagination size (max 100 per GitHub).")
    ...
    try:
        ...
        if e.code in (403, 404):
            print(f"::notice::{args.endpoint} returned HTTP {e.code} — assuming zero alerts", file=sys.stderr)
            with open(args.out, "w") as f:
                json.dump([], f)
            return 0
        print(f"::error::{args.endpoint} HTTP {e.code}: {e.reason}", file=sys.stderr)
        with open(args.out, "w") as f:
            json.dump([], f)
        return 2
...
def router_main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Select an explicit route by name (repeatable); bypasses --cadence.",
    )
    parser.add_argument("--json", action="store_true", help="Print the matrix JSON (default behavior).")
    parser.add_argument("--content-root", help="Override the docs content root.")
    args = parser.parse_args(argv)
...
def build_main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Compute edits without writing.")
    parser.add_argument("--date", help="Target date as MM-DD.")
    parser.add_argument("--year", help="Target 4-digit year.")
    parser.add_argument("--content-root", help="Override the docs content root.")
    parser.add_argument("--timestamp", help="ISO 8601 timestamp for the report.")
    parser.add_argument("--workdir", help="Scratch dir for acquired raw JSON.")
    parser.add_argument("--public-dir", help="Astro public data dir for JSON output.")
    args = parser.parse_args(argv)
