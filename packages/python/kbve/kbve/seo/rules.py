...
def rule_title_length(page, ctx, profile):
    ...
    if n < profile["title_min"]:
        return [Finding("title-length", WARN, "title %d chars < %d" % (n, profile["title_min"]))]
    if n > profile["title_max"]:
        return [Finding("title-length", WARN, "title %d chars > %d" % (n, profile["title_max"]))]
    return []
...
def rule_desc_length(page, ctx, profile):
    ...
    if n < profile["desc_min"]:
        return [Finding("desc-length", WARN, "description %d chars < %d" % (n, profile["desc_min"]))]
    if n > profile["desc_max"]:
        return [
            Finding("desc-length", WARN, "description %d chars > %d (truncated in SERP)" % (n, profile["desc_max"]))
        ]
    return []
...
def rule_title_duplicate(page, ctx, profile):
    ...
    if t and len(ctx["titles"].get(t, ())) > 1:
        return [Finding("title-duplicate", ERROR, "title shared with %d other pages" % (len(ctx["titles"][t]) - 1))]
    return []
...
def rule_desc_duplicate(page, ctx, profile):
    ...
    if d and len(ctx["descs"].get(d, ())) > 1:
        return [Finding("desc-duplicate", ERROR, "description shared with %d other pages" % (len(ctx["descs"][d]) - 1))]
    return []
...
def rule_jsonld_present(page, ctx, profile):
    ...
    if not (fm.get("source_path") or fm.get("app_name")):
        return [Finding("jsonld-present", WARN, "no source_path/app_name -> no SoftwareSourceCode JSON-LD")]
    return []
...
def rule_body_thin(page, ctx, profile):
    ...
    if floor and len(body) < floor:
        return [Finding("body-thin", WARN, "body %d chars < %d" % (len(body), floor))]
    return []
