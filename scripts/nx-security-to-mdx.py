...
def write_mdx(data: dict, timestamp: str, path: str):
    ...
    with open(path, "w") as out:
        ...
        # Mermaid severity distribution chart
        has_findings = any(
            summary[s] > 0 for s in SEVERITY_ORDER[:4]
        )
        if has_findings:
            ...
            for sev in SEVERITY_ORDER[:4]:
                if summary[sev] > 0:
                    out.write(
                        f'    "{sev.capitalize()}" : {summary[sev]}\n'
                    )
            out.write("```\n\n")

        # Mermaid ecosystem breakdown chart
        eco_totals = {
            ECOSYSTEM_LABELS[e]: ecosystems.get(e, {}).get("total", 0)
            for e in ["npm", "cargo", "python", "codeql", "dependabot"]
        }
        has_eco_findings = any(v > 0 for v in eco_totals.values())
        if has_eco_findings:
